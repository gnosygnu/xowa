...
public class Db_qry_dml_tst {
	...
	@Test public void Insert_basic() {
		tst_XtoSql(new Db_qry_insert("tbl0").Val_int("id", 0).Val_str("name", "me").Val_date("time", GfoDateUtl.ParseGplx("2007-12-23"))
	...
	@Test public void Update_all() {
		Db_qry_update qry = new Db_qry_update();
		qry.From_("tbl0");
		qry.Val_int("id", 1).Val_str("name", "me").Val_date("startTime", GfoDateUtl.ParseGplx("2007-12-23"));
		qry.Where_(Criteria_.And(Db_crt_.New_eq("id", 0), Db_crt_.New_mt("startTime", GfoDateUtl.ParseGplx("2005-01-01"))));
	...
	void tst_XtoSql(Db_qry qry, String expd) {GfoTstr.EqObj(expd, qry.ToSqlExec(SqlQryWtrUtl.NewBasic()));}
}
