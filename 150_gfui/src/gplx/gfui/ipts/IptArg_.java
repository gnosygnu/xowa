...
public class IptArg_ {
	...
	public static IptArg Wildcard = new IptKey(IntUtl.MaxValue, Wildcard_key);
	...
	public static IptArg parse(String raw) {
		if		(StringUtl.Has(raw, ","))				return IptKeyChain.parse(raw);
		String bgn = StringUtl.GetStrBefore(raw, ".");
		if		(StringUtl.Eq(bgn, "wheel"))			return IptMouseWheel_.parse(raw);
		else if (StringUtl.Eq(bgn, "mouse"))			return IptMouseBtn_.parse(raw);
		else if (StringUtl.Eq(bgn, "key"))			return IptKey_.parse(raw);
	...
	public static IptEventType EventType_default(IptArg arg) {
	...
	public static boolean EventType_match(IptArg arg, IptEventType match) {
		Class<?> type = arg.getClass();
		if		(	type == IptKey.class
...
class IptMacro {
	...
	public IptArg parse(String raw) {
		if (regy == null) Init();
		String[] plusAry = StringUtl.Split(raw, "+");
		String[] dotAry	= StringUtl.Split(plusAry[0], ".");
		String bgn = dotAry[0], end = dotAry[1];
		Ordered_hash list = (Ordered_hash)regy.GetByOrNull(bgn);
		if (list == null) throw parse_err(raw, "list not found").ArgsAdd("list", bgn);
		IptKey rv = (IptKey)list.GetByOrNull(end);
		if (rv == null) throw parse_err(raw, "arg not found").ArgsAdd("arg", end);
	...
	Ordered_hash regy;
	static Err parse_err(String raw, String loc) {return ErrUtl.NewArgs("could not parse IptArg", "raw", raw, "loc", loc);}
