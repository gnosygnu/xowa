...
public class IptKeyStrMgr_tst {
	...
	}	void tst_FetchByKeyPress(char c, IptKey expd) {GfoTstr.EqObj(expd.Key(), IptKeyStrMgr.Instance.FetchByKeyPress((int)c).Key());}
	void tst_XtoUiStr(IptKey key, String expd) {GfoTstr.EqObj(expd, key.XtoUiStr());}
	void tst_XtoUiStrShifted(IptKey key, String expdNormal, String expdShifted) {
		GfoTstr.EqObj(expdNormal, key.XtoUiStr());
		GfoTstr.EqObj(expdShifted, IptKey_.Shift.Add(key).XtoUiStr());
	}
}
