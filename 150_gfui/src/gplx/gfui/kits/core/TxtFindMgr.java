...
public class TxtFindMgr {
	...
	public TxtFindMgr Text_(String v) {
		if (!caseSensitive) v = StringUtl.Lower(v);
		text = v;
		return this;
	}	String text;
	...
	public int[] FindByUi(String findText, int selBgn, int selLen, boolean keyIsEnter) {
		int[] rv = new int[2];
		if (StringUtl.Eq(findText, "")) return rv;		// make newSel = 0 b/c all text deleted; else, find will continue from last selBgn; easy way to "reset"
		...
		if (findPos == StringUtl.FindNone) {				// nothing found; set selLen to 0 and return
			rv[1] = 0;
			return rv;
		}
		rv[0] = findPos;
		rv[1] = StringUtl.Len(findText);
		return rv;
	}
	public int FindNext(String find, int guiPos) {
		if (!caseSensitive) find = StringUtl.Lower(find);
		int findPos = StringUtl.FindFwd(text, find, guiPos);
		if (findPos == StringUtl.FindNone && guiPos != 0)
			findPos = StringUtl.FindFwd(text, find, 0);
		return findPos;
	}
}
