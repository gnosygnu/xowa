...
public class SizeAdpF_ {
	...
	public static SizeAdpF parse_(String s) {
		try {
			String[] ary = String_.Split(s, ","); if (ary.length != 2) throw Err_.new_("SizeAdf should only have 2 numbers separated by 1 comma");
			float val1 = Float_.parse_(ary[0]);
			float val2 = Float_.parse_(ary[1]);
			return new_(val1, val2);
		}	catch (Exception e) {throw Err_.parse_type_exc_(e, SizeAdpF.class, s);}
	}
}
