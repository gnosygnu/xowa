...
public class IoItmDir_FetchDeepOrNull_tst {
	...
	@Test public void FetchDeepOrNull() {
		...
		tst_FetchDeepOrNull(rootDir.SubDirs().GetAt(0), drive.GenSubDir("sub1"), true);
		tst_FetchDeepOrNull(rootDir.SubDirs().GetAt(0), drive.GenSubDir("sub2"), false);
	}
	void tst_FetchDeepOrNull(Object rootDirObj, Io_url find, boolean expdFound) {
		...
		if (actlDir == null) {
			if (expdFound) GfoTstr.Fail("actlDir is null, but expd dir to be found");
			else return;    // actlDir is null but expdFound was false; return;
		}
		GfoTstr.EqObj(find.Raw(), actlDir.Url().Raw());
	}
}
