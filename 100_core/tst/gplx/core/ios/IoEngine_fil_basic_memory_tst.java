...
public class IoEngine_fil_basic_memory_tst extends IoEngine_fil_basic_base_tst {
	...
	@Test @Override public void SaveFilText_overwrite() {
		...
		IoItmFil_mem filItm = (IoItmFil_mem)dirItm.SubFils().GetAt(0);
		GfoTstr.EqObj(filItm.Text(), "changed");
	}
	@Test public void RecycleFil() {
		...
		List_adp list = GfoIoTstr.RscDir.XtoNames();
//            foreach (String s in list)
//                Tfds.Write(s);
		list.DelAt(0); // remove drive
		IoEngine_xrg_recycleFil recycleXrg = bin.Send_xrg(fil)
			.RootDirNames_(list)
			.AppName_("gplx.test").Time_(GfoDateUtl.ParseGplx("20100102_115559123")).Uuid_(GfoGuidUtl.Parse("467ffb41-cdfe-402f-b22b-be855425784b"));
		recycleXrg.Exec();
