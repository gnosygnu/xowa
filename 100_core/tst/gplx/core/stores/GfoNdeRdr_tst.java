...
public class GfoNdeRdr_tst {
	...
	@Test public void Subs_rdr() {
		...
		GfoTstr.EqBoolY(rdr.MoveNextPeer());
		GfoTstr.EqObj(0, rdr.ReadAt(0));
		GfoTstr.EqBoolN(rdr.MoveNextPeer());
	}
	@Test public void MoveNextPeer_implicit() {
		...
		tst_RdrVals(subRdr, ObjectUtl.Ary(0, 1, 2));
	}
	@Test public void MoveNextPeer_explicit() {
		...
		GfoNdeRdr rootRdr = GfoNdeRdr_.root_parseNot_(root);
		DataRdr subsRdr = rootRdr.Subs();        // pos=-1; bof
		GfoTstr.EqBoolY(subsRdr.MoveNextPeer());    // explicitly moves to pos=0
	...
	@Test public void Xpath_null() {
		...
		GfoTstr.EqBoolN(sub.MoveNextPeer());
	}
	...
	void tst_RdrVals(DataRdr rdr, Object[] exptVals) {
		...
		while (rdr.MoveNextPeer()) {
			...
			GfoTstr.EqObj(actl, exptVals[count++]);
		}
		GfoTstr.EqObj(count, exptVals.length);
	}
