...
public class List_adp_tst {
	...
	@Test public void Add() {
		GfoTstr.EqObj(0, list.Len());

		list.Add("0");
		GfoTstr.EqObj(1, list.Len());
	}
	@Test public void Add_changeCapacity() {
		int capacity = 8;
		for (int i = 0; i < capacity; i++)
			list.Add("0");
		GfoTstr.EqObj(capacity, list.Len());
		GfoTstr.EqObj(capacity, listBase.Capacity());

		list.Add(capacity);    // forces resize
		GfoTstr.EqObj(capacity + 1, list.Len());
		GfoTstr.EqObj(capacity * 2, listBase.Capacity());
	}
	@Test public void Get_at() {
		list.Add("0");

		GfoTstr.EqObj("0", list.GetAt(0));
	}
	@Test public void Fetch_many() {
		list_AddMany("0", "1");

		GfoTstr.EqObj("0", list.GetAt(0));
		GfoTstr.EqObj("1", list.GetAt(1));
	}
	@Test public void FetchAt_fail() {
		try {list.GetAt(0);}
		catch (Exception exc) {return;}
		GfoTstr.Fail("Get_at should fail for out of bound index");
	}
	...
	@Test public void DelAt_shiftDown() {
		list_AddMany("0", "1");
		GfoTstr.EqObj(list.Len(), 2);

		list.DelAt(0);
		GfoTstr.EqObj(1, list.Len());
		GfoTstr.EqObj("1", list.GetAt(0));
	}
	@Test public void DelAt_fail() {
		try {list.DelAt(0);}
		catch (Exception exc) {return;}
		GfoTstr.Fail("Del_at should fail for out of bound index");
	}
	...
	@Test public void Enumerator_recursive() { // confirm separate enumerator objects are used
		...
		for (Object valObj : list) {
			...
			GfoTstr.EqObj(IntUtl.ToStr(pos++), val);
			tst_Enumerator("0", "1", "2");
		}
	}
	@Test public void Clear() {
		...
		list.Clear();
		GfoTstr.EqObj(0, list.Len());
		GfoTstr.EqObj(16, listBase.Capacity()); // check that capacity has increased
	}
	...
	@Test public void Xto_bry() {
		...
		GfoTstr.EqNotNull(ary);
		GfoTstr.EqObj(2, ArrayUtl.Len(ary));
	}
	@Test public void XtoAry_empty() {
		String[] ary = (String[])list.ToAry(String.class);
		GfoTstr.EqNotNull(ary);
		GfoTstr.EqObj(0, ArrayUtl.Len(ary));
	}
	@Test public void Shuffle() {
		...
		for (int i = 0; i < list.Len(); i++) {
			int val = IntUtl.Cast(list.GetAt(i));
			if (val != i) hasMovedCount++;
		}
		GfoTstr.EqBoolY(hasMovedCount > 0, "all documents have the same index");    // NOTE: may still fail occasionally (1%)

		int count = list.Len();
		for (int i = 0; i < count; i++)
			list.Del(i);
		GfoTstr.EqObj(0, list.Len(), "shuffled list does not have the same contents as original list");
	}
	...
	void tst_DelRange(int bgn, int end, String... expd) {
		...
		GfoTstr.EqLines(expd, list.ToStrAry());
	}
	...
	void tst_Enumerator(String... expd) {
		...
		for (int i = 0; i < expdLength; i++) {
			String val = expd[i];
			GfoTstr.EqObj(expd[pos++], val);
		}
		GfoTstr.EqObj(pos, expdLength);
	}
}
