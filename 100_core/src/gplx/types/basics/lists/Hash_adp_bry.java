...
public class Hash_adp_bry extends gplx.core.lists.Hash_adp_base implements Hash_adp {
	...
	public int Get_as_int(byte[] key, int bgn, int end) {
		int rv = Get_as_int_or(key, bgn, end, IntUtl.MinValue); if (rv == IntUtl.MinValue) throw ErrUtl.NewArgs("unknown key", "key", key);
		return rv;
	}
	...
	public Hash_adp_bry Add_byte_int(byte key, int val)                {this.Add_base(new byte[]{key}, new IntVal(val)); return this;}
	public Hash_adp_bry Add_bry_byte(byte[] key, byte val)            {this.Add_base(key, ByteVal.New(val)); return this;}
	public Hash_adp_bry Add_bry_int(byte[] key, int val)            {this.Add_base(key, new IntVal(val)); return this;}
	public Hash_adp_bry Add_bry_bry(byte[] key)                        {this.Add_base(key, key); return this;}
	public Hash_adp_bry Add_str_byte(String key, byte val)            {this.Add_base(BryUtl.NewU8(key), ByteVal.New(val)); return this;}
	public Hash_adp_bry Add_str_int(String key, int val)            {this.Add_base(BryUtl.NewU8(key), new IntVal(val)); return this;}
	public Hash_adp_bry Add_str_obj(String key, Object val)            {this.Add_base(BryUtl.NewU8(key), val); return this;}
...
class Hash_adp_bry_itm_ci_u8 extends Hash_adp_bry_itm_base {
	...
	@Override public int hashCode() {
		...
		for (int i = src_bgn; i < src_end; i++) {
			...
			Gfo_case_itm itm = case_mgr.GetOrNull(b, src, i, src_end);
			if (itm == null) {                // unknown itm; byte is a number, symbol, or unknown; just use the existing byte
				...
				int b_len = Utf8Utl.LenOfCharBy1stByte(b);    // NOTE: must calc b_len for langs with asymmetric upper / lower; PAGE:tr.w:Zvishavane DATE:2015-09-07
				i += b_len - 1;
			}
			rv = (31 * rv) + b_int;
		}
	...
	@Override public boolean equals(Object obj) {
		...
		while    (    src_c_bgn < src_end
			...
			int src_c_len = Utf8Utl.LenOfCharBy1stByte(src_c);
			int trg_c_len = Utf8Utl.LenOfCharBy1stByte(trg_c);
			int src_c_end = src_c_bgn + src_c_len;
			int trg_c_end = trg_c_bgn + trg_c_len;
			Gfo_case_itm src_c_itm = case_mgr.GetOrNull(src_c, src, src_c_bgn, src_c_end);
			Gfo_case_itm trg_c_itm = case_mgr.GetOrNull(trg_c, trg, trg_c_bgn, trg_c_end);
			if        (src_c_itm != null && trg_c_itm == null)    return false;                        // src == ltr; trg != ltr; EX: a, 1
				...
				if (!BryLni.Eq(src, src_c_bgn, src_c_end, trg, trg_c_bgn, trg_c_end)) return false;// syms do not match; return false;
			}
			else {
		...
		switch (case_mgr.Tid()) {
			...
			default:                            throw ErrUtl.NewUnhandled(case_mgr.Tid());
		}
	}
