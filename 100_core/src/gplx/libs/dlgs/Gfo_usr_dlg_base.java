...
public class Gfo_usr_dlg_base implements Gfo_usr_dlg {
	...
	private final BryFmtr tmp_fmtr = BryFmtr.NewTmp().FailWhenInvalidEscapesSet(false);    // do not fail b/c msgs may contain excerpt of random text; EX:[[User:A|~A~]] DATE:2014-11-28
	...
	public Err Fail_many(String grp_key, String msg_key, String fmt, Object... args) {
		Err rv = ErrUtl.NewArgs(Bld_msg_many(grp_key, msg_key, fmt, args));
		log_wkr.Log_to_err(rv.ToStrLog());
		return rv;
	}
	private String Bld_msg_many(String grp_key, String msg_key, String fmt, Object[] args) {
		synchronized (tmp_fmtr) {
			try {
				tmp_fmtr.FmtSet(fmt).BldToBfrMany(tmp_bfr, args);
	...
	private String Bld_msg_one(String grp_key, String msg_key, String fmt, Object val) {
		synchronized (tmp_fmtr) {
			tmp_fmtr.FmtSet(fmt).BldToBfrObj(tmp_bfr, val);
			return tmp_bfr.ToStrAndClear();
		}
	}
