...
public class Gfo_usr_dlg__log_base implements Gfo_usr_dlg__log {
	...
	private final BryFmtr fmtr = BryFmtr.NewTmp();
	...
	private void Flush() {
		...
		for (int i = 0; i < queued_len; i++) {
			Usr_log_fil fil = (Usr_log_fil)queued_list.GetAt(i);
			if (fil.Url() == null) {
	...
	public void Log_dir_(Io_url log_dir) {
		...
		if (enabled) {
			session_dir = log_dir.GenSubDir(GfoDateNow.Get().ToStrFmt_yyyyMMdd_HHmmss_fff());
	...
	public void Log_msg_to_url_fmt(Io_url url, String fmt, Object... args) {
		if (!enabled) return;
		String msg = Bld_msg(StringUtl.NewU8(fmtr.FmtSet(fmt).BldToBryMany(tmp_bfr, args)));
		Log_msg(url, msg);
		Log_msg(session_fil, msg);
	}
	public void Log_to_session_fmt(String fmt, Object... args) {Log_to_session(StringUtl.NewU8(fmtr.FmtSet(fmt).BldToBryMany(tmp_bfr, args)));}
	...
	public void Log_to_err(String s) {
		...
		catch (Exception e) {}            // java.lang.StringBuilder can throw exceptions in some situations when called on a different thread; ignore errors
	}
	private String Bld_msg(String s) {
		synchronized (thread_lock) { // THREAD:synchronized neded b/c multiple threads can still mutate the same sb; ISSUE#:646; DATE:2020-03-16
			return sb.Add(GfoDateNow.GetForce().ToUtc().ToStrFmt_yyyyMMdd_HHmmss_fff()).Add(" ").Add(s).AddCharNl().ToStrAndClear();
		}
	}
...
class Usr_log_fil {
	...
	public void Flush(boolean enabled) {
		...
		if (sb.Len() == 0) return;
		try {
			Io_mgr.Instance.AppendFilStr(url, sb.ToStrAndClear());
		}
		catch (Exception e) {
