...
public class Gfo_evt_mgr {
	...
	public void AddSub(Gfo_evt_mgr_owner pub, String pubEvt, Gfo_evt_itm sub, String subPrc) {
	...
	public void Lnk(Gfo_evt_mgr_owner pub) {
	...
	public void AddPub(String pubEvt, GfoEvLnk lnk) {
		if (pubsRegy == null) pubsRegy = Ordered_hash_.New();
		AddInList(pubsRegy, pubEvt, lnk);
	}
	public void Pub(GfsCtx ctx, String evt, GfoMsg m) {
		ctx.MsgSrc_(sender);
		...
		if (lnks != null) {
			for (int i = 0; i < lnks.Len(); i++) {
				Gfo_evt_mgr lnk = (Gfo_evt_mgr)lnks.GetAt(i);
				lnk.Pub(ctx, evt, m);
			}
		}
	}
	public void RlsSub(Gfo_evt_mgr_owner eobj) {
		RlsRegyObj(pubsRegy, eobj, true);
		RlsRegyObj(subsRegy, eobj, false);
	}
	public void RlsPub(Gfo_evt_mgr_owner eobj) {
		RlsRegyObj(pubsRegy, eobj, true);
		RlsRegyObj(subsRegy, eobj, false);
	}
	public void RlsRegyObj(Ordered_hash regy, Gfo_evt_mgr_owner eobj, boolean pub) {
		if (regy == null) return;
		List_adp delList = List_adp_.New();
		for (int i = 0; i < regy.Len(); i++) {
			GfoEvLnkList pubsList = (GfoEvLnkList)regy.GetAt(i);
			delList.Clear();
			...
			for (int j = 0; j < delList.Len(); j++) {
				GfoEvLnk del = (GfoEvLnk)delList.GetAt(j);
				del.End(pub).Evt_mgr().RlsLnk(!pub, pubsList.Key(), del.End(!pub));
				pubsList.Del(del);
			}
		}
	}
	public void RlsLnk(boolean pubEnd, String key, Gfo_evt_mgr_owner endObj) {
		...
		for (int i = 0; i < delList.Len(); i++) {
			GfoEvLnk lnk = (GfoEvLnk)delList.GetAt(i);
			list.Del(lnk);
		}
		delList.Clear();
	}
...
class GfoEvLnkList {
	...
	public GfoEvLnk Get_at(int i) {return (GfoEvLnk)list.GetAt(i);}
