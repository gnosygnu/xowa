...
public class Gfo_msg_grp implements Gfo_msg_obj {
	public Gfo_msg_grp(Gfo_msg_grp owner, int uid, byte[] key) {
		this.owner = owner; this.uid = uid; this.key = key; this.key_str = StringUtl.NewA7(key);
		if (owner != null) {
			owner.subs.Add(this);
	...
	public String Path_str() {return StringUtl.NewA7(path);}
	public Gfo_msg_obj Subs_get_by_key(String sub_key) {
		int subs_len = subs.Len();
		for (int i = 0; i < subs_len; i++) {
			Gfo_msg_obj sub = (Gfo_msg_obj)subs.GetAt(i);
			if (StringUtl.Eq(sub_key, sub.Key_str())) return sub;
		}
		return null;
	}
