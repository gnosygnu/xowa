...
public class Gfo_msg_root {
	...
	public Gfo_msg_data Data_new_note_many(String owner_key, String key, String fmt, Object... vals) {return Data_new_many(Gfo_msg_itm_.Cmd_note, BryUtl.Empty, -1, -1, owner_key, key, fmt, vals);}
	public Gfo_msg_data Data_new_many(byte cmd, String owner_key, String key, String fmt, Object[] vals) {return Data_new_many(cmd, BryUtl.Empty, -1, -1, owner_key, key, fmt, vals);}
	public Gfo_msg_data Data_new_many(byte cmd, byte[] src, int bgn, int end, String owner_key, String key, String fmt, Object[] vals) {
		...
		if (itm == null)
			itm = new Gfo_msg_itm(owner, uid_item_next++, cmd, BryUtl.NewU8(key), fmt == null ? BryUtl.Empty : BryUtl.NewA7(fmt), false);
	...
	Gfo_msg_grp New_list_by_key(String key) {
		String[] segs = StringUtl.Split(key, '.');
		...
		for (int i = 0; i < segs_last; i++) {
			...
			if (sub_list == null)
				sub_list = new Gfo_msg_grp(cur_list, uid_list_next++, BryUtl.NewA7(key));
			cur_list = sub_list;
		}
		return cur_list;
	}
