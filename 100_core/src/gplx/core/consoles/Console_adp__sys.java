...
public class Console_adp__sys implements Console_adp {
	...
	public boolean Canceled_chk() {if (canceled) throw ErrUtl.NewCanceled(); return canceled;}
	...
	public void Write_fmt_w_nl(String fmt, Object... args)        {Clear_tmp(); Write_str_w_nl_lang(StringUtl.Format(fmt, args));}
	...
	public void Write_tmp(String s) {
		Clear_tmp();
		if (StringUtl.Has(s, "\r")) s = StringUtl.Replace(s, "\r", " ");
...
//            if (String_.Len(s) >= chars_per_line_max) s = String_.Mid(s, 0, chars_per_line_max - String_.Len("...") - 1) + "...";    // NOTE: >= and -1 needed b/c line needs to be 1 less than max; ex: default cmd is 80 width, but writing 80 chars will automatically create lineBreak
	...
	private void Clear_tmp() {
		...
		if (Env_.Mode_debug()) {Write_str_lang(StringUtl.CrLf); return;}
		int count = backspace_by_bytes ? BryUtl.NewU8(tmp_txt).length : StringUtl.Len(tmp_txt);
		String moveBack = StringUtl.Repeat("\b", count);
		this.Write_str_lang(moveBack);                        // move cursor back to beginning of line
		this.Write_str_lang(StringUtl.Repeat(" ", count));    // overwrite tmp_txt with space
	...
	private char Read_key_lang() {
		...
		return StringUtl.Len(text) == 0 ? '\0' : StringUtl.CharAt(text, 0);
	}
	public void Write_str_w_nl_utf8(String s) {
				java.io.PrintStream ps;
		try {ps = new java.io.PrintStream(System.out, true, "UTF-8");}
		catch (java.io.UnsupportedEncodingException e) {throw ErrUtl.NewArgs("unsupported exception");}
		ps.println(s);
			}
