...
public class RegxPatn_cls_like_tst {
	...
	void tst_Match(String raw, String regx, char escape, boolean expd) {
		...
		GfoTstr.EqObj(expd, actl, "raw={0} regx={1} expd={2}", raw, regx, expd);
	}
	void tst_EscapeRegxChar(char regexChar) {
		RegxPatn_cls_like like = RegxPatn_cls_like_.parse(ObjectUtl.ToStrOrEmpty(regexChar), '|');
		GfoTstr.EqObj(true, like.Matches(ObjectUtl.ToStrOrEmpty(regexChar)));
		GfoTstr.EqObj(false, like.Matches("a"));                // catches errors for improper escaping of wildcard
	}
}
