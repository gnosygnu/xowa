...
public class Btrie_u8_mgr implements Btrie_mgr {
	...
	Btrie_u8_mgr(Gfo_case_mgr case_mgr) {
		...
		this.root = new Btrie_u8_itm(BryUtl.Empty, null);
	}
	public int Count() {return count;} private int count;
	public Object MatchAt(Btrie_rv rv, byte[] src, int bgn_pos, int end_pos) {return Match_at_w_b0(rv, src[bgn_pos], src, bgn_pos, end_pos);}
	public Object Match_at_w_b0(Btrie_rv rv, byte b, byte[] src, int bgn_pos, int end_pos) {
		...
		while (true) {
			int c_len = Utf8Utl.LenOfCharBy1stByte(b);
	...
	public Object MatchBgn(byte[] src, int bgn_pos, int end_pos) {return Match_bgn_w_byte(src[bgn_pos], src, bgn_pos, end_pos);}
	...
	public Btrie_mgr AddObj(byte[] key, Object val) {
		...
		while (c_bgn < key_len) {
			byte c = key[c_bgn];
			int c_len = Utf8Utl.LenOfCharBy1stByte(c);
			int c_end = c_bgn + c_len;
			Btrie_u8_itm nxt = cur.Nxts_find(key, c_bgn, c_end, false);
			if (nxt == null)
				nxt = cur.Nxts_add(case_mgr, BryLni.Mid(key, c_bgn, c_end), null);
			c_bgn = c_end;
			if (c_bgn == key_len)
				nxt.Val_set(val);
			cur = nxt;
