...
class Gfo_poolable_mgr_tstr {
	private final Gfo_poolable_mgr mgr = new Gfo_poolable_mgr(new Sample_poolable_itm(null, -1, ObjectUtl.AryEmpty), ObjectUtl.Ary("make"), 2, 8);
	...
	public void Test__get(int expd_idx) {
		...
		GfoTstr.EqObj(expd_idx, actl_itm.Pool__idx(), "pool_idx");
	}
	public void Test__free__ary(int... expd)    {GfoTstr.EqAry(expd, mgr.Free_ary(), "mgr.Free_ary()");}
	public void Test__free__len(int expd)            {GfoTstr.EqObj(expd, mgr.Free_len(), "mgr.Free_len()");}
	public void Test__pool__len(int expd)            {GfoTstr.EqObj(expd, mgr.Pool_len(), "mgr.Pool_len()");}
	public void Test__pool__nxt(int expd)            {GfoTstr.EqObj(expd, mgr.Pool_nxt(), "mgr.Pool_nxt()");}
