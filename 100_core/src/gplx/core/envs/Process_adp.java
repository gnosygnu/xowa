...
public class Process_adp implements Gfo_invk, Rls_able {
	...
	public BryFmtr Args_fmtr() {return args_fmtr;} BryFmtr args_fmtr = BryFmtr.New("");
	...
	public Process_adp Cmd_args(String cmd, String args) {this.Exe_url_(Io_url_.new_fil_(cmd)); this.args_fmtr.FmtSet(args); return this;}
	...
	public Process_adp Run(Object... args) {
		if (StringUtl.IsNullOrEmpty(exe_url.Raw())) return this;    // noop if exe_url is "";
		if (!args_fmtr.FmtNull()) {
			BryWtr tmp_bfr = BryWtr.New();
			args_fmtr.BldToBfrMany(tmp_bfr, args);
			args_str = tmp_bfr.ToStrAndClear();
		}
		...
		switch (run_mode) {
			...
			default:                    throw ErrUtl.NewUnhandled(run_mode);
		}
	}
	public String[] Xto_process_bldr_args(String... args) {
		String args_str = args_fmtr.BldToStrMany(args);
	...
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		...
		else if    (ctx.Match(k, Invk_cmd_))                        this.Exe_url_(BryFmtrEvalMgrUtl.Eval_url(cmd_url_eval, m.ReadBry("cmd")));
		else if    (ctx.Match(k, Invk_args))                        return StringUtl.NewU8(args_fmtr.Fmt());
		else if    (ctx.Match(k, Invk_args_))                        args_fmtr.FmtSet(m.ReadBry("v"));
		else if    (ctx.Match(k, Invk_cmd_args_))                    {this.Exe_url_(BryFmtrEvalMgrUtl.Eval_url(cmd_url_eval, m.ReadBry("cmd"))); args_fmtr.FmtSet(m.ReadBry("args"));}
	...
	BryFmtrEvalMgr cmd_url_eval;
	public static Process_adp ini_(Gfo_invk owner, Gfo_usr_dlg usr_dlg, Process_adp process, BryFmtrEvalMgr cmd_url_eval, byte run_mode, int timeout, String cmd_url_fmt, String args_fmt, String... args_keys) {
		process.Run_mode_(run_mode).Thread_timeout_seconds_(timeout);
		process.cmd_url_eval = cmd_url_eval;
		Io_url cmd_url = BryFmtrEvalMgrUtl.Eval_url(cmd_url_eval, BryUtl.NewU8(cmd_url_fmt));
		process.Exe_url_(cmd_url).Tmp_dir_(cmd_url.OwnerDir());
		process.Args_fmtr().FmtSet(args_fmt).KeysSet(args_keys);
		process.owner = owner;
	...
	public static String Escape_ampersands_if_process_is_cmd(boolean os_is_wnt, String exe_url, String exe_args) {
		return (    os_is_wnt
				&&    StringUtl.Eq(exe_url, "cmd"))
			? StringUtl.Replace(exe_args, "&", "^&")    // escape ampersands
			: exe_args
			;
	}

	public void Exe_and_args_(String exe, String args) {
		Io_url exe_url = BryFmtrEvalMgrUtl.Eval_url(this.cmd_url_eval, BryUtl.NewU8(exe));
		this.Exe_url_(exe_url).Tmp_dir_(exe_url.OwnerDir());
		this.Args_fmtr().FmtSet(args);
	}
	public static Process_adp New(Gfo_usr_dlg usr_dlg, BryFmtrEvalMgr eval_mgr, byte run_mode, int timeout, String exe, String args, String... args_keys) {
		...
		Io_url exe_url = BryFmtrEvalMgrUtl.Eval_url(eval_mgr, BryUtl.NewU8(exe));
		rv.Exe_url_(exe_url).Tmp_dir_(exe_url.OwnerDir());
		rv.Args_fmtr().FmtSet(args).KeysSet(args_keys);
		return rv;
	}
		private BryFmtr notify_fmtr = BryFmtr.New("", "process_exe_name", "process_exe_args", "process_seconds"); BryWtr notify_bfr = BryWtr.NewAndReset(255);
	...
	//        thread_timeout = 15000;
			...
			notify_fmtr.FmtSet(prog_fmt);
			while (thread.isAlive()) {
				thread_run = true;
				long prv = SystemUtl.Ticks();
...
//                catch (InterruptedException e) {throw Err_.err_key_(e, "gplx.ProcessAdp", "thread interrupted at join");}
				...
				if (prog_dlg != null) {
					if (elapsed > notify_checkpoint) {
						...
						notify_fmtr.BldToBfrMany(notify_bfr, exe_url.NameAndExt(), args_str, elapsed / 1000);
						prog_dlg.Prog_none(GRP_KEY, "notify.prog", notify_bfr.ToStrAndClear());
					}
				}
				...
				if (elapsed > thread_timeout) {
					...
					thread.Cancel();
					try {thread.join();}
					catch (InterruptedException e) {throw ErrUtl.NewArgs(e, "thread interrupted at timeout");}
					break;
				}
			}
			if (!thread_run) {
				try {thread.join();}
				catch (InterruptedException e) {throw ErrUtl.NewArgs(e, "thread interrupted at join 2");}
			}
		} catch (Exception exc) {
			GfoTstr.Write(ErrUtl.ToStrFull(exc));
		}
		if (elapsed != notify_checkpoint) {
			notify_fmtr.BldToBfrMany(notify_bfr, exe_url.NameAndExt(), args_str, elapsed / 1000);
			if (prog_dlg != null) prog_dlg.Prog_none(GRP_KEY, "notify.prog", notify_bfr.ToStrAndClear());
		}
		return this;
	}
	...
	String Kill() {
		if (thread_kill_name == StringUtl.Empty) return "";
//        Runtime rt = Runtime.getRuntime();
	...
	protected Process Process_start() {
		try {process = pb.start();}
			...
			throw ErrUtl.NewArgs(e, "process start failed", "args", StringUtl.ConcatWith(" ", command_ary));
		}
		return process;
	}
	void Process_run_and_end() {
		...
		catch (InterruptedException e)     {throw ErrUtl.NewArgs(e, "thread interrupted at wait_for", "exe_url", exe_url.Xto_api(), "exeArgs", args_str);}
		catch (IOException e)             {throw ErrUtl.NewArgs(e, "io error", "exe_url", exe_url.Xto_api(), "exeArgs", args_str);}
		exit_code = process.exitValue();
		WhenEnd_run();
		process.destroy();
		rslt_out = sb.ToStrAndClear();
	}
	...
	public static String[] To_process_bldr_args_utl(Io_url exe_url, String args_str, boolean include_quotes) {
		...
		String_bldr sb = String_bldr_.new_();
		int len = StringUtl.Len(args_str);
		boolean in_quotes = false;
		for (int i = 0; i < len; i++) {
			char c = StringUtl.CharAt(args_str, i);
		...
		if (sb.HasSome()) list.Add(sb.ToStr());
	...
	public synchronized void run() {
		...
		try {
			...
			while (input_gobbler.isAlive()) {
				...
				catch (InterruptedException e) {throw ErrUtl.NewArgs(e, "thread interrupted at input gobbler");}
			}
			while (error_gobbler.isAlive()) {
				try {error_gobbler.join(50);}
				catch (InterruptedException e) {throw ErrUtl.NewArgs(e, "thread interrupted at error gobbler");}
			}
...
class StreamGobbler extends Thread {
	...
	public void run () {
		try {
			...
			stream.close();
			rslt = sb.ToStrAndClear();
		}
		catch (Exception e) {throw ErrUtl.NewArgs(e, "failed reading stream", "name", name);}
	}
}
