...
public class Io_stream_tid_ {
	...
	public static byte To_tid(String v) {
		if        (StringUtl.Eq(v, Key__raw))            return Io_stream_tid_.Tid__raw;
		else if    (StringUtl.Eq(v, Key__zip))            return Io_stream_tid_.Tid__zip;
		else if    (StringUtl.Eq(v, Key__gzip))            return Io_stream_tid_.Tid__gzip;
		else if    (StringUtl.Eq(v, Key__bzip2))            return Io_stream_tid_.Tid__bzip2;
		else if    (StringUtl.Eq(v, Key__xz))            return Io_stream_tid_.Tid__xz;
	...
	public static byte Obsolete_to_tid(String v) {
		if        (StringUtl.Eq(v, ".xdat"))            return Io_stream_tid_.Tid__raw;
		else if    (StringUtl.Eq(v, ".zip"))                return Io_stream_tid_.Tid__zip;
		else if    (StringUtl.Eq(v, ".gz"))                return Io_stream_tid_.Tid__gzip;
		else if    (StringUtl.Eq(v, ".bz2"))                return Io_stream_tid_.Tid__bzip2;
		else                                        throw ErrUtl.NewUnhandled(v);
	}
}
