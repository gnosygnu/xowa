...
public class Io_stream_rdr_ {
	...
	public static Io_stream_rdr    New__mem(String v) {return New__mem(BryUtl.NewU8(v));}
	...
	public static Io_stream_rdr New_by_url(Io_url url) {
		String ext = url.Ext();
		if        (StringUtl.Eq(ext, Io_stream_tid_.Ext__zip))         return Io_stream_rdr_.New__zip(url);
		else if    (StringUtl.Eq(ext, Io_stream_tid_.Ext__gz))         return Io_stream_rdr_.New__gzip(url);
		else if    (StringUtl.Eq(ext, Io_stream_tid_.Ext__bz2))         return Io_stream_rdr_.New__bzip2(url);
		else if    (StringUtl.Eq(ext, Io_stream_tid_.Ext__xz))         return new Io_stream_rdr__xz().Url_(url);
	...
	public static String Load_all_as_str(Io_stream_rdr rdr) {return StringUtl.NewU8(Load_all_as_bry(BryWtr.New(), rdr));}
	public static byte[] Load_all_as_bry(BryWtr rv, Io_stream_rdr rdr) {
		Load_all_to_bfr(rv, rdr);
		return rv.ToBryAndClear();
	}
	public static void Load_all_to_bfr(BryWtr rv, Io_stream_rdr rdr) {
		try {
			byte[] bry = new byte[4096];
			while (true) {
				int read = rdr.Read(bry, 0, 4096);
				if (read < gplx.core.ios.streams.Io_stream_rdr_.Read_done_compare) break; 
				rv.AddMid(bry, 0, read);
			}
		} finally {rdr.Rls();}
	}
	public static int Read_by_parts(java.io.InputStream stream, int part_len, byte[] bry, int bgn, int len) {
		...
		*/
		try {
			...
			throw ErrUtl.NewArgs(exc, "read failed", "bgn", bgn, "len", len);
		}
	}
