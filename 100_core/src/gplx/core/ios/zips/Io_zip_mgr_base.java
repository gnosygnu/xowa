...
public class Io_zip_mgr_base implements Io_zip_mgr {
	public void Zip_fil(Io_url src_fil, Io_url trg_fil) {
		...
		try {
			...
			zip_strm.flush();
			zip_strm.close();
		}    catch(IOException e) {ErrUtl.NewArgs(e, "error duing zip", "src", src_dir.Raw(), "trg", trg_fil.Raw());}
	}
	...
	private void Zip_dir__add_fil(ZipOutputStream zip_strm, byte[] bry, String zip_path, Io_url fil_url) {
		try {
			...
			fil_strm.close();
		}    catch(IOException e) {throw ErrUtl.NewArgs(e, "error duing zip", "src", zip_path);}
	}
	private Io_url[] Zip_dir__get_subs(Io_url url) {
		...
		try {
			...
			trgZip.close();
		}   catch(Exception e) {throw ErrUtl.NewArgs("failed to zip", "err", e.getMessage());}
	...
	public byte[] Unzip_bry(byte[] src, int bgn, int len)  {
		...
		}   catch(Exception e) {throw ErrUtl.NewArgs("failed to unzip", "err", e.getMessage());}
		return trg_stream.toByteArray();
			}
	public void Unzip_to_dir(Io_url src_fil, Io_url trg_dir) {
		...
		try{
			...
			while (zip_eny != null) {
				...
				if (Op_sys.Cur().Tid_is_wnt()) itm_name = StringUtl.Replace(itm_name, "/", "\\");
