...
public class Io_zip_compress_cmd__jre {
	...
	public byte Exec_hook(gplx.core.progs.Gfo_prog_ui prog_ui, Io_url[] src_urls, Io_url trg_url, String resume_name, long resume_file, long resume_item) {
		...
		catch (Exception e) {throw ErrUtl.NewArgs(e, "trg open failed", "url", trg_url.Raw());}
		...
		for (int i = 0; i < len; ++i) {
			...
			try {trg_stream.putNextEntry(trg_entry);}
				...
				throw ErrUtl.NewArgs(e, "zip entry failed", "url", src_url.Raw());
			}
			FileInputStream src_stream = null;
			try {src_stream = new FileInputStream(new File(src_url.Raw()));}
			catch (Exception e) {throw ErrUtl.NewArgs(e, "src open failed", "url", src_url.Raw());}
			while (true) {    // loop over bytes
				...
				try {read_in_raw = src_stream.read(buffer);}
					...
					throw ErrUtl.NewArgs(e, "src read failed", "url", src_url.Raw());
				}
				...
				try {trg_stream.write(buffer, 0, read_in_raw);}
					...
					throw ErrUtl.NewArgs(e, "trg write failed", "url", trg_url.Raw());
				}
			}
			try {
			...
			catch (Exception e) {throw ErrUtl.NewArgs(e, "trg close entry failed", "url", src_url.Raw());}
		}
		try {
			trg_stream.close();
			trg_stream.flush();            
		}
		catch (Exception e) {throw ErrUtl.NewArgs(e, "trg close failed", "url", trg_url.Raw());}
