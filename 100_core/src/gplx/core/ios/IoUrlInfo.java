...
class IoUrlInfo_nil implements IoUrlInfo {
	public String Key() {return KeyConst;} public static final String KeyConst = StringUtl.NullMark;
...
abstract class IoUrlInfo_base implements IoUrlInfo {
	public static final int DirSprLen = 1;
	public static final String NullString = "", ExtSeparator = ".";
	...
	public boolean IsDir(String raw) {return StringUtl.HasAtEnd(raw, DirSpr());}
	public abstract String XtoRootName(String raw, int rawLen);
	public String Xto_api(String raw) {
		return IsDir(raw)
			? StringUtl.DelEnd(raw, IoUrlInfo_base.DirSprLen)    // if Dir, remove trailing DirSpr, since most api will not expect it (ex: .Delete will malfunction)
			: raw;
	}
	public String OwnerDir(String raw) {
		...
		return StringUtl.MidByLen(raw, 0, ownerDirSprPos + 1);    // +1 to include backslash
	}
	public String OwnerRoot(String raw) {
		String temp = raw, rv = raw;
		while (true) {
			temp = OwnerDir(temp);
			if (StringUtl.Eq(temp, IoUrlInfo_base.NullString)) break;
			rv = temp;
		}
		return rv;
	}
	public String NameAndExt(String raw) {    // if Dir, will return \ as last char
		...
		return ownerDirSprPos == OwnerDirPos_hasNoOwner || ownerDirSprPos == OwnerDirPos_isRoot
			...
			: StringUtl.DelBgn(raw, ownerDirSprPos + 1);        // +1 to skip backslash
	}
	public String NameOnly(String raw) {
		String nameAndExt = NameAndExt(raw);
		if (IsDir(raw)) {
			String rootName = XtoRootName(raw, StringUtl.Len(raw));    // C:\ -> C; / -> root
			return rootName == null
				? StringUtl.DelEnd(nameAndExt, IoUrlInfo_base.DirSprLen)
				: rootName;
		}
		int pos = StringUtl.FindBwd(nameAndExt, IoUrlInfo_base.ExtSeparator);
		return pos == StringUtl.FindNone
			? nameAndExt                                    // Ext not found; return entire NameAndExt
			: StringUtl.MidByLen(nameAndExt, 0, pos);
	}
	public String Ext(String raw) {        // if Dir, return DirSpr; if Fil, return . as first char; ex: .txt; .png
		if (IsDir(raw)) return this.DirSpr();
		String nameAndExt = NameAndExt(raw);
		int pos = StringUtl.FindBwd(nameAndExt, IoUrlInfo_base.ExtSeparator);
		return pos == StringUtl.FindNone ? "" : StringUtl.DelBgn(nameAndExt, pos);
	}
	int OwnerDirPos(String raw, int rawLen) {
		if        (rawLen == 0)                        return OwnerDirPos_isNull;
			...
			return StringUtl.FindBwd(raw, this.DirSpr(), rawLen - 1 - posAdj); // -1 to adjust for LastIdx
		}
	}
	static final int
...
class IoUrlInfo_wnt extends IoUrlInfo_base {
	...
	@Override public boolean Match(String raw)    {return StringUtl.Len(raw) > 1 && StringUtl.CharAt(raw, 1) == ':';} // 2nd char is :; assumes 1 letter drives
	@Override public String XtoRootName(String raw, int rawLen) {
		return rawLen == 3 && StringUtl.CharAt(raw, 1) == ':' // only allow single letter drives; ex: C:\; note, CharAt(raw, 1) to match Match
			? CharUtl.ToStr(StringUtl.CharAt(raw, 0))
			: null;
	}
...
class IoUrlInfo_lnx extends IoUrlInfo_base {
	...
	@Override public boolean Match(String raw)    {return StringUtl.HasAtBgn(raw, DirSprStr);}    // anything that starts with /
	@Override public String XtoRootName(String raw, int rawLen) {
		return rawLen == 1 && StringUtl.Eq(raw, DirSprStr)
			? "root"
			: null;
	}
	@Override public String OwnerRoot(String raw) {return DirSprStr;}    // drive is always /
	@Override public String Xto_api(String raw) {
		return StringUtl.Eq(raw, DirSprStr)            // is root
			? DirSprStr
...
class IoUrlInfo_mem extends IoUrlInfo_base {
	...
	@Override public String XtoRootName(String raw, int rawLen) {
		return StringUtl.Eq(raw, key) ? StringUtl.DelEnd(key, 1) : null;
	}
	@Override public boolean Match(String raw) {return StringUtl.HasAtBgn(raw, key);}
...
class IoUrlInfo_alias extends IoUrlInfo_base {
	...
	@Override public boolean CaseSensitive() {return false;}
	@Override public String XtoRootName(String raw, int rawLen) {
		return StringUtl.Eq(raw, srcRootDir) ? srcRootName : null;
	}
	@Override public boolean Match(String raw) {return StringUtl.HasAtBgn(raw, srcDir);}
	@Override public String Xto_api(String raw) {            
		String rv = StringUtl.Replace(raw, srcDir, trgDir);                                            // replace src with trg
		if (!StringUtl.Eq(srcDirSpr, trgDirSpr)) rv = StringUtl.Replace(rv, srcDirSpr, trgDirSpr);        // replace dirSprs
		return IsDir(raw)
			? StringUtl.DelEnd(rv, IoUrlInfo_base.DirSprLen)    // remove trailingSeparator, else Directory.Delete will not work properly
			: rv;
	}
	void SrcDir_set(String v) {
		...
		if (srcRootName == null) srcRootName = lnx ? "root" : StringUtl.Mid(srcDir, 0, StringUtl.FindFwd(srcDir, ":"));
	...
	boolean DirSpr_lnx(String s) {return StringUtl.Has(s, Op_sys.Lnx.Fsys_dir_spr_str());}
