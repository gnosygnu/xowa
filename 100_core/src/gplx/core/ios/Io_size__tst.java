...
class Io_size__fxt {
	public void Test_XtoLong(String raw, long expd) {GfoTstr.EqObj(expd, Io_size_.parse_or(raw, LongUtl.MinValue));}
	public void Test_XtoLongFail(String raw) {
		long val = Io_size_.parse_or(raw, LongUtl.MinValue);
		if (val != LongUtl.MinValue) GfoTstr.Fail("expd parse failure; raw=" + raw);
	}
	public void Test_Equals(String lhs, String rhs) {GfoTstr.EqObj(Io_size_.parse_or(lhs, LongUtl.MinValue), Io_size_.parse_or(rhs, LongUtl.MinValue));}
	public void Test_XtoStr(long val, String expd) {GfoTstr.EqObj(expd, Io_size_.To_str(val));}
	public void Test_Xto_str(long val, int exp_1024, String val_fmt, String unit_pad, boolean round_0_to_1, String expd) {GfoTstr.EqObj(expd, Io_size_.To_str(val, exp_1024, val_fmt, unit_pad, round_0_to_1));}
	public void Test__to_str_new(long val, int decimal_places, String expd) {
		BryWtr bfr = BryWtr.New();
		GfoTstr.Eq(expd, Io_size_.To_str_new(bfr, val, decimal_places));
	}
}
