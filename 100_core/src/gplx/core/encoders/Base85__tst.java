...
public class Base85__tst {
	...
	@Test public void Log() {
		...
		fxt.Test_log(IntUtl.MaxValue, 5);
	}
	...
	@Test public void XtoStrAry() {
		...
		fxt.Exec_to_str(ary,      6, 14709); // #$%
			GfoTstr.EqObj("!!#!#$#$%", StringUtl.NewU8(ary));
	}
}
class Base85__fxt {
	public void Test_log(int val, int expd) {GfoTstr.EqObj(expd, Base85_.Bry_len(val));}
	public void Test_to_str(int val, String expd) {
		String actl = Base85_.To_str(val, 0);
		GfoTstr.EqObj(expd, actl);
		GfoTstr.EqObj(val, Base85_.To_int_by_str(expd));
	}
