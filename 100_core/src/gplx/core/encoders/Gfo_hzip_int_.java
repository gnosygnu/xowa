...
public class Gfo_hzip_int_ {
	...
	public static void Encode(int reqd, BryWtr bfr, int val) {
		int bfr_len = bfr.Len();
		...
		bfr.AddByteRepeat(AsciiByte.Bang, actl_len);            // fill with 0s; this asserts that there underlying array will be large enough for following write
	...
	public static int Decode(int reqd, byte[] src, int src_len, int src_bgn, IntRef pos_ref) {
		byte b0 = src[src_bgn];
