...
public class GfsCore_ {
	...
	public static Object Exec(GfsCtx ctx, Gfo_invk owner_invk, GfoMsg owner_msg, Object owner_primitive, int depth) {
		if (owner_msg.Args_count() == 0 && owner_msg.Subs_count() == 0 && StringUtl.Eq(owner_msg.Key(), "")) {UsrDlg_.Instance.Warn("empty msg"); return Gfo_invk_.Rv_unhandled;}
		...
		if        (rv == Gfo_invk_.Rv_cancel)        return rv;
			...
			if (ctx.Fail_if_unhandled())
				throw ErrUtl.NewArgs("Object does not support key", "key", owner_msg.Key(), "ownerType", ClassUtl.CanonicalNameByObj(owner_invk));
			else {
				Gfo_usr_dlg_.Instance.Warn_many(GRP_KEY, "unhandled_key", "Object does not support key: key=~{0} ownerType=~{1}", owner_msg.Key(), ClassUtl.CanonicalNameByObj(owner_invk));
		...
		if (owner_msg.Subs_count() == 0) {                    // msg is leaf
			...
			if (invk == null) {                                // rv is primitive; find appropriate mgr
				throw ErrUtl.NewArgs("unknown primitive", "type", ClassUtl.Name(rv.getClass()), "obj", ObjectUtl.ToStrOrNullMark(rv));
			}
			Object exec_rv = null;
