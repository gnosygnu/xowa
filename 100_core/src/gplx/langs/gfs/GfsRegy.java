...
class GfsRegy implements Gfo_invk {
	...
	public GfsRegyItm Get_at(int i) {return (GfsRegyItm)hash.GetAt(i);}
	public GfsRegyItm Get_by(String key) {return (GfsRegyItm)hash.GetByOrNull(key);}
	public GfsRegyItm FetchByType(Gfo_invk invk) {return (GfsRegyItm)typeHash.GetByOrNull(ClassUtl.CanonicalNameByObj(invk));}
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		Object rv = (GfsRegyItm)hash.GetByOrNull(k); if (rv == null) throw ErrUtl.NewMissingKey(k);
		return rv;
	}
	...
	public void Add(String key, Gfo_invk invk, boolean typeCmd) {
		...
		GfsRegyItm regyItm = new GfsRegyItm().Key_(key).InvkAble_(invk).IsCmd_(typeCmd).TypeKey_(ClassUtl.CanonicalNameByObj(invk));
