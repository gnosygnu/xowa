...
public class DsvDataWtr_csv_tst {
	...
	@Test public void Dat_Escape_FldSpr() {    // ,
		...
		expd = StringUtl.ConcatLinesCrlf("a,\",\",c");
		fx.tst_XtoStr(wtr, root, expd);
	}
	@Test public void Dat_Escape_RcdSpr() {    // NewLine
		root = fx_nde.csv_dat_(); this.AddCsvRow(root, "a", StringUtl.CrLf, "c");
		expd = StringUtl.ConcatLinesCrlf("a,\"" + StringUtl.CrLf + "\",c");
		fx.tst_XtoStr(wtr, root, expd);
	}
