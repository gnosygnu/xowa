...
class DsvParser {
	public DsvSymbols Symbols() {return sym;} DsvSymbols sym = DsvSymbols.default_();
	public void Init() {sb.Clear(); bldr.Init(); qteOn = false;}
	public DataRdr ParseAsRdr(String raw) {return GfoNdeRdr_.root_(ParseAsNde(raw), csvOn);} // NOTE: csvOn means parse; assume manual typed flds not passed in (ex: id,Int)
	public GfoNde ParseAsNde(String raw) {
		if (StringUtl.Eq(raw, "")) return bldr.BldRoot();
	...
	void ReadStreamInQte(CharStream strm) {
		if (strm.MatchAndMove(sym.QteDlm())) {                                // is quote
			if (strm.MatchAndMove(sym.QteDlm()))                            // double quote -> quote; "a""
				...
				throw ErrUtl.NewArgs("invalid quote in quoted field; quote must be followed by quote, fieldSpr, or recordSpr", "sym", strm.Cur(), "text", strm.To_str_by_pos(strm.Pos() - 10, strm.Pos() + 10));
		}
	...
	void ProcessFld(CharStream strm) {
		String val = sb.ToStrAndClear();
		if (cmdSeqOn) {
			cmdSeqOn = false;
			if (StringUtl.Eq(val, sym.CmdDlm()) && qteOn) {        // 2 cmdDlms in a row; cmdSeq encountered; next fld must be cmdName
		...
		if        (StringUtl.Eq(val, sym.CmdDlm()))                    // val is cmdDlm; do not add now; wait til next fld to decide
			cmdSeqOn = true;
		else if (nextValType == ValType_Data) {
			if (StringUtl.Len(val) == 0) val = qteOn ? "" : null; // differentiate between null and emptyString; ,, vs ,"",
			tkns.Add(val);
		}
		else if (nextValType == ValType_CmdName) {
			if        (StringUtl.Eq(val, sym.TblNameSym()))        lineMode = LineType_TblBgn;        // #
			else if (StringUtl.Eq(val, sym.FldNamesSym()))    lineMode = LineType_FldNames;    // @
			else if (StringUtl.Eq(val, sym.FldTypesSym()))    lineMode = LineType_FldTypes;    // $
			else if (StringUtl.Eq(val, sym.CommentSym()))        lineMode = LineType_Comment;    // '
			else                                            throw ErrUtl.NewArgs("unknown dsv cmd", "cmd", val);
		}
		else
			throw ErrUtl.NewArgs("unable to process field value", "value", val);
	}
	void ProcessLine(CharStream strm, boolean cleanup) {
		if (sb.Len() == 0 && tkns.Len() == 0)
	...
	public static DsvParser dsv_() {return new DsvParser();}
	public static DsvParser csv_(boolean hasHdr, GfoFldList flds) {
...
class DsvTblBldr {
	...
	public void MakeComment(List_adp tkns) {            
		if (stage == Stage_Row)                // comments in ROW; ignore; NOTE: tkns.Clear() could be merged, but this seems clearer
			...
			for (int i = 0; i < tkns.Len(); i++)
				sb.Add((String)tkns.GetAt(i));
			layout.HeaderList().Add_Comment(sb.ToStr());
			tkns.Clear();
		}
	}
	...
	Object[] MakeValsAry(List_adp tkns) {
		...
		if (tkns.Len() > subFldsCount) throw ErrUtl.NewArgs("values.Count cannot be greater than fields.Count", "values.Count", tkns.Len(), "fields.Count", subFldsCount);
		Object[] rv = new Object[subFldsCount];
		for (int i = 0; i < subFldsCount; i++) {
			ClassXtn typx = subFlds.Get_at(i).Type();
			String val = i < tkns.Len() ? (String)tkns.GetAt(i) : null;
	...
	void CreateFlds(int valCount) {
		...
		if (fldNamesCount == 0 && fldTypesCount == 0) {            // csv tbls where no names or types, just values
			...
			for (int i = 0; i < maxCount; i++) {
				String name = i < fldNamesCount ? (String)fldNames.GetAt(i) : "fld" + i;
				ClassXtn typx = i < fldTypesCount ? (ClassXtn)fldTypes.GetAt(i) : StringClassXtn.Instance;
	...
	public static final String NullTblName = "";
