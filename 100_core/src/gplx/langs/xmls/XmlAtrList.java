...
public class XmlAtrList {
	...
	public XmlAtr Get_by(String key) {
		Node xatr = list.getNamedItem(key);
		if (xatr == null) throw ErrUtl.NewMissingKey(key);
		return new XmlAtr(xatr);
	}
	public XmlAtr Fetch(String key) {
		Node xatr = list.getNamedItem(key); if (xatr == null) throw ErrUtl.NewMissingKey(key);
		return new XmlAtr(xatr);
	}
	...
	public XmlAtrList(NamedNodeMap list) {this.list = list;} NamedNodeMap list;
}
