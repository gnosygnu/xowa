...
public class Wmf_dump_list_parser {
	...
	private boolean Parse_href(Wmf_dump_itm itm, byte[] src, int a_pos) {	// EX: http://dumps.wikimedia.org/enwiki/20130807
		...
		int href_bgn_pos = Bry_find_.Find_fwd(src, AsciiByte.Quote, href_pos + Find_href.length);
		int href_end_pos = Bry_find_.Find_fwd(src, AsciiByte.Quote, href_bgn_pos + 1); if (href_end_pos == Bry_find_.Not_found) return false;
		byte[] href_bry = Bry_.Mid(src, href_bgn_pos + 1, href_end_pos);
		int date_end = href_bry.length;
		int date_bgn = Bry_find_.Find_bwd(href_bry, AsciiByte.Slash); if (date_bgn == Bry_find_.Not_found) return false;
		...
		int abrv_bgn = Bry_find_.Find_bwd(href_bry, AsciiByte.Slash, abrv_end); if (abrv_bgn == Bry_find_.Not_found) abrv_bgn = -1;	// "enwiki/20130708"
		byte[] abrv_bry = Bry_.Mid(href_bry, abrv_bgn + 1, abrv_end);
		itm.Wiki_abrv_(Bry_.Replace(abrv_bry, AsciiByte.Underline, AsciiByte.Dash));
		return true;
	}
	private DateAdp Parse_status_time(byte[] src, int a_pos) {
		...
		int bgn = Bry_find_.Find_fwd(src, AsciiByte.Gt, li_pos + Find_li.length); if (bgn == Bry_find_.Not_found) return null;
	...
	private byte[] Parse_status_msg(byte[] src, int a_pos) {
		...
		int bgn = Bry_find_.Find_fwd(src, AsciiByte.Gt, span_pos + Find_span_bgn.length); if (bgn == Bry_find_.Not_found) return null;
