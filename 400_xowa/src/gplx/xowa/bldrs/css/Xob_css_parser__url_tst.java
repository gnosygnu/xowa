...
class Xob_css_parser__url_fxt {
	protected Xob_css_parser__url url_parser; private final BryWtr bfr = BryWtr.NewWithSize(32);
	protected Xob_css_tkn__base cur_frag; protected byte[] src_bry;
	public void Clear() {
		url_parser = new Xob_css_parser__url(BryUtl.NewA7("site"));
	}
	protected void Exec_parse(String src_str, int expd_tid, String expd_str) {
		this.src_bry = BryUtl.NewU8(src_str);
		this.Exec_parse_hook();
		cur_frag.Write(bfr, src_bry);
		String actl_str = bfr.ToStrAndClear();
		GfoTstr.EqObj(expd_tid, cur_frag.Tid(), "wrong tid; expd={0}, actl={1}", expd_tid, cur_frag.Tid());
		GfoTstr.EqObj(expd_str, actl_str);
	}
	protected void Exec_parse_hook() {
	...
	public void Test_parse_warn(String src_str, String expd, String warn) {
		...
		GfoTstr.EqObj(true, StringUtl.Has(sub_frag.Fail_msg(), warn));
	}
}	
