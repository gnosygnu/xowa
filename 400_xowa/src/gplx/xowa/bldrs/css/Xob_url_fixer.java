...
class Xob_url_fixer {
	public static byte[] Fix(byte[] site, byte[] src, int src_len) {	// return "site/img.png" if "//site/img.png" or "http://site/img.png"; also, handle "img.png?key=val"
		...
		Object o = Xob_url_fixer_tkn.Bgn_trie().MatchBgn(src, bgn, src_len);
		if (o != null) {
			...
			if (o != null) {
				...
				switch (tkn.Tid()) {
					...
					case Xob_url_fixer_tkn.Tid_mid_rel_2:
						BryWtr tmp_bfr = BryWtr.NewWithSize(src_len);
						byte[] to_rel_root = BryLni.Mid(src, bgn, pos);
						byte[] to_rel_qry  = BryLni.Mid(src, pos, src_len);
						src = gplx.xowa.xtns.pfuncs.ttls.Pfunc_rel2abs.Rel2abs(tmp_bfr, seg_ary, to_rel_qry, to_rel_root, IntRef.NewNeg1());
						bgn = pos = 0;
...
class Xob_url_fixer_tkn {
	...
	public static Xob_url_fixer_tkn new_(int tid, String raw) {return new Xob_url_fixer_tkn(tid, BryUtl.NewU8(raw));}
	private static void trie_add(Btrie_slim_mgr trie, int tid, String s) {trie.AddObj(s, new_(tid, s));}
