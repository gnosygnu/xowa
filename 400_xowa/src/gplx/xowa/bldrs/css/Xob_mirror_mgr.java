...
public class Xob_mirror_mgr {
	...
	public void Code_add(byte[] src_url) {
		...
		code_hash.AddIfDupeUse1st(src_url, new Xobc_download_itm(Xobc_download_itm.Tid_css, StringUtl.NewU8(src_url), trg_url));
	}
	...
	public void Exec() {
		...
		css_parser.Parse(download_xrg.Exec_as_bry(StringUtl.NewU8(page_url)));
		while (true) {
			Xobc_download_itm[] code_ary = (Xobc_download_itm[])code_hash.ToAryAndClear(Xobc_download_itm.class);
			...
			for (int i = 0; i < code_ary_len; ++i) {
				...
				Io_mgr.Instance.SaveFilBry(fsys_root.Gen_sub_path_for_os(StringUtl.NewU8(code.Fsys_url())), code_src);
				css_parser.Parse(code_src);
			}
		}
		Xobc_download_itm[] file_ary = (Xobc_download_itm[])file_hash.ToAryAndClear(Xobc_download_itm.class);
		int file_ary_len = file_ary.length;
		for (int i = 0; i < file_ary_len; ++i) {
			Xobc_download_itm file = file_ary[i];
			download_xrg.Init(file.Http_str(), Io_url_.new_fil_(fsys_root.Gen_sub_path_for_os(StringUtl.NewU8(file.Fsys_url()))));
			download_xrg.Exec();
		}
	}
}
