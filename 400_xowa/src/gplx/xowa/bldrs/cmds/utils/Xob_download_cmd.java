...
public class Xob_download_cmd extends Xob_cmd__base implements Xob_cmd {
	...
	@Override public void Cmd_run() {
		...
		if (dump_type == null) throw ErrUtl.NewArgs("dump_type must be specified");
		...
		// download
		usr_dlg.Note_many("", "", "downloading file: now=~{0} src=~{1} trg=~{2}", GfoDateNow.Get().ToStrFmt_yyyyMMdd_HHmmss(), dump_src, dump_trg_zip.OwnerDir());
		...
		if (!download_wkr.Exec())
			usr_dlg.Warn_many("", "", "download failed: src=~{0} trg=~{1} err=~{2}", dump_src, dump_trg_zip.Raw(), ErrUtl.ToStrFull(download_wkr.Rslt_err()));
		if (unzip) {	// parsing unzipped file is faster, but takes up more storage space
			usr_dlg.Note_many("", "", "unzipping file: now=~{0} trg=~{1}", GfoDateNow.Get().ToStrFmt_yyyyMMdd_HHmmss(), dump_trg_bin.Raw());
	...
	@Override public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		if		(StringUtl.Eq(k, Invk_dump_date_))			dump_date = m.ReadStr("v");
		else if	(StringUtl.Eq(k, Invk_dump_type_))			dump_type = m.ReadStr("v");
		else if	(StringUtl.Eq(k, Invk_dump_src_))				dump_src = m.ReadStr("v");
		else if	(StringUtl.Eq(k, Invk_dump_trg_zip_))			dump_trg_zip = m.ReadIoUrl("v");
		else if	(StringUtl.Eq(k, Invk_dump_trg_bin_))			dump_trg_bin = m.ReadIoUrl("v");
		else if	(StringUtl.Eq(k, Invk_unzip_))				unzip = m.ReadYn("v");
	...
	private static boolean Found(IoItmHash hash, String dump_type) {
		String match = StringUtl.Format("{0}.sql", dump_type); // EX: "page_props.sql"
		int len = hash.Len();
		for (int i = 0; i < len; i++) {
			IoItm_base fil = (IoItm_base)hash.GetAt(i);
			if (StringUtl.Has(fil.Url().NameAndExt(), match))
				return true;
		}
		return false;
	}
