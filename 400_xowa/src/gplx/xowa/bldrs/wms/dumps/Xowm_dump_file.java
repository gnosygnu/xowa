...
public class Xowm_dump_file {
	public Xowm_dump_file(String domain_str, String dump_date, String dump_type_str) {
		...
		this.domain_itm = Xow_domain_itm_.parse(BryUtl.NewU8(domain_str));
	...
	public GfoDate File_modified()		{return file_modified;} private GfoDate file_modified;				// EX: 2015-08-10 20:12:34
	public void Dump_date_(String v) {dump_date = v;}
	public void Server_url_(String server_url) {
		this.server_url = server_url;
		String dump_dir_url = StringUtl.NewU8(Xowm_dump_file_.Bld_dump_dir_url(BryUtl.NewU8(server_url), dump_abrv, BryUtl.NewU8(dump_date)));
	...
	public boolean Connect() {
		...
		if (	!rv	// not found
			&&	StringUtl.In(server_url, Xowm_dump_file_.Server_wmf_http, Xowm_dump_file_.Server_wmf_https)	// server is dumps.wikimedia.org
			&&	StringUtl.Eq(dump_date, Xowm_dump_file_.Date_latest)	// request dump was latest
			) {
			Xoa_app_.Usr_dlg().Warn_many("", "", "wmf.dump:latest not found; url=~{0}", file_url);
			byte[] abrv_wm_bry = Xow_abrv_wm_.To_abrv(domain_itm);
			String new_dump_root = Xowm_dump_file_.Server_wmf_https + StringUtl.NewU8(abrv_wm_bry) + "/";	// EX: http://dumps.wikimedia.org/enwiki/
			...
			for (int i = dates_len - 1; i > -1; --i) {
				...
				if (StringUtl.Eq(new_dump_date, Xowm_dump_file_.Date_latest)) continue;	// skip latest; assume it is bad
				String new_dump_file = StringUtl.Replace(file_name, Xowm_dump_file_.Date_latest, new_dump_date);	// replace "-latest-" with "-20150602-";
	...
	private boolean Connect_exec(IoEngine_xrg_downloadFil args, String cur_file_url) {			
		...
		GfoDate tmp_file_modified = args.Src_last_modified();
		Xoa_app_.Usr_dlg().Note_many("", "", "wmf.dump:connect rslts; url=~{0} result=~{1} fil_len=~{2} file_modified=~{3} server_url=~{4} dump_date=~{5}", cur_file_url, rv, tmp_file_len, tmp_file_modified == null ? "<<NULL>>" : tmp_file_modified.ToStrFmt_yyyy_MM_dd_HH_mm_ss(), server_url, dump_date);
		if (rv) {
	...
	private static String Bld_file_name(byte[] dump_abrv, String dump_date, String dump_type_str) {
		byte[] dump_type_bry = BryUtl.NewU8(dump_type_str);
		...
		return StringUtl.NewU8(Xowm_dump_file_.Bld_dump_file_name(dump_abrv, BryUtl.NewU8(dump_date), dump_type_bry, dump_file_ext));
	}
}
