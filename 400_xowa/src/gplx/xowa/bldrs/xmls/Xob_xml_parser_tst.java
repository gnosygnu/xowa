...
public class Xob_xml_parser_tst {
	...
	@Test public void Xml() {
		...
		fil = page_bldr.Add(doc).XtoByteStreamRdr();
		tst_parse(fil, doc.Text_(BryUtl.NewA7("\"a & b <> a | b\"")), 0);
	}
	@Test public void Tab() {
		Xowd_page_itm doc = doc_(1, "a", "a \t b", Date_1);
		fil = page_bldr.Add(doc).XtoByteStreamRdr();
		tst_parse(fil, doc.Text_(BryUtl.NewA7("a &#09; b")), 0);
	}
	...
	@Test public void Cr_nl() {
		...
		fil = page_bldr.Add(doc).XtoByteStreamRdr();
		tst_parse(fil, doc.Text_(BryUtl.NewA7("a \n b")), 0);
	}
	@Test public void Cr() {
	...
	@Test public void Text_long() {
		String s = StringUtl.Repeat("a", 1024);
	...
	@Test public void Ns_file() {
		...
		GfoTstr.EqObj(Xow_ns_.Tid__file, doc.Ns_id());
		GfoTstr.EqObj("a", StringUtl.NewU8(doc.Ttl_page_db()));
	}
	@Test public void Ns_main() {
		Xowd_page_itm doc = doc_(1, "a", "a", Date_1);
		GfoTstr.EqObj(Xow_ns_.Tid__main, doc.Ns_id());
		GfoTstr.EqObj("a", StringUtl.NewU8(doc.Ttl_page_db()));
	}
	@Test public void Ns_main_book() {
		Xowd_page_itm doc = doc_(1, "Book", "a", Date_1);
		GfoTstr.EqObj(Xow_ns_.Tid__main, doc.Ns_id());
		GfoTstr.EqObj("Book", StringUtl.NewU8(doc.Ttl_page_db()));
	}
	@Test public void XmlEntities() {
		...
		GfoTstr.EqObj("A&b", StringUtl.NewU8(actl.Ttl_full_db()));
	}
	@Test public void Root() {
		Xowd_page_itm doc = doc_(1, "a", "a", Date_1);
		page_bldr.Bfr().AddStrA7("<root>\n");
		page_bldr.Add(doc);
		page_bldr.Bfr().AddStrA7("</root>");
		fil = page_bldr.XtoByteStreamRdr();
		tst_parse(fil, doc, 0);
	}
	private static final String Date_1 = "2012-01-01T01:01:01Z", Date_2 = "2012-02-02T02:02:02Z"; private GfoDateParser dateParser = new GfoDateParser();
	...
	int tst_parse(Io_buffer_rdr fil, Xowd_page_itm expd, int cur_pos) {
		...
		GfoTstr.EqObj(expd.Id(), actl.Id(), "id");
		GfoTstr.EqObj(StringUtl.NewU8(expd.Ttl_full_db()), StringUtl.NewU8(actl.Ttl_full_db()), "title");
		GfoTstr.EqObj(StringUtl.NewU8(expd.Text()), StringUtl.NewU8(actl.Text()), "text");
		GfoTstr.Eq(expd.Modified_on(), actl.Modified_on(), "timestamp");
		return rv;
	}
	Xowd_page_itm doc_(int id, String title, String text, String date) {
		Xowd_page_itm rv = new Xowd_page_itm().Id_(id).Ttl_(BryUtl.NewA7(title), ns_mgr).Text_(BryUtl.NewA7(text));
