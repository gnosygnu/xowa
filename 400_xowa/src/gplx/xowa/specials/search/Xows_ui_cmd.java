...
import gplx.threads.*; import gplx.html.*;
...
class Xows_ui_cmd implements GfoInvkAble, Cancelable, Xog_tab_close_lnr {
	...
	public boolean Canceled() {synchronized (thread_lock) {return canceled;}} private boolean canceled;
	public void Cancel() {
		Xoa_app_.Usr_dlg().Prog_many("", "", "search canceled: key=~{0}", key);
		synchronized (thread_lock) {canceled = true;}
	...
	public boolean Search() {
		...
		if (!cache.Done() && (qry.Itms_end() > cache.Itms_end())) {
			if (async) {
				...
				if (async_wkr == null) async_wkr = new Xows_ui_async(this, mgr.Html_wkr().Html_rows(), js_wkr, qry.Page_len(), wiki.Domain_bry());
				ThreadAdp_.invk_(gplx.xowa.apps.Xoa_thread_.Key_special_search_db, this, Invk_search_db).Start();
			}
			else
				Search_db();
			rv = true;
	...
	private void Hide_cancel_btn() {
		ThreadAdp_.invk_(gplx.xowa.apps.Xoa_thread_.Key_special_search_cancel, this, Invk_hide_cancel).Start();
	}
