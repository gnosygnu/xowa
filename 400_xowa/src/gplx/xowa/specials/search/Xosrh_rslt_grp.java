...
public class Xosrh_rslt_grp {
	public Xosrh_rslt_grp(int idx) {this.idx = idx;} private Xodb_page[] itms = Xodb_page.Ary_empty;
	...
	public Xodb_page Itms_get_at(int i) {return itms[i];}
	public void Itms_add(Xodb_page itm) {
		int new_itms_len = itms_len + 1;
		if (new_itms_len > itms_max) {
			itms_max = itms_max == 0 ? 2 : itms_max * 2;
			itms = (Xodb_page[])Array_.Resize(itms, itms_max);
		}
		itms[itms_len] = itm;
	...
	public void Itms_clear() {
		for (int i = 0; i < itms_len; i++)
			itms[i] = null;
		itms = Xodb_page.Ary_empty;
		itms_len = itms_max = 0;
	}
	public void Itms_sort(Xodb_page_sorter sorter) {Array_.Sort(itms, sorter);}
