...
public class Popup_history_page implements Xow_special_page {
	...
	public void Special__gen(Xow_wiki wikii, Xoa_page pagei, Xoa_url url, Xoa_ttl ttl) {
		...
		for (int i = len - 1; i > -1; --i) {
			Xow_popup_itm itm = (Xow_popup_itm)hash.GetAt(i);
			if (Ttl_chk(itm.Page_ttl())) continue;
			fmtr_main.BldToBfrMany(bfr, itm.Page_href(), itm.Page_ttl().Full_txt());
		}
		page.Db().Text().Text_bry_(bfr.TrimEnd(AsciiByte.Nl).ToBryAndRls());
		page.Html_data().Html_restricted_n_();
	}
	private BryFmtr fmtr_main = BryFmtr.New("<a href='~{href}'>~{ttl}</a>\n\n", "href", "ttl");	// NOTE: need to use anchor (as opposed to lnki or lnke) b/c xwiki will not work on all wikis
	public static boolean Ttl_chk(Xoa_ttl ttl) {
		return	ttl.Ns().Id_is_special()
			&&	BryLni.Eq(ttl.Page_db(), Xow_special_meta_.Itm__popup_history.Key_bry());
	}
