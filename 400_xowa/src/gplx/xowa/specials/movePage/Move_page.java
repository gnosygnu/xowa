...
public class Move_page implements Xow_special_page {
	...
	private void Exec_rename(Xowe_wiki wiki, Xoae_page page) {
		...
		Xow_ns trg_ns = wiki.Ns_mgr().Ids_get_or_null(trg_ns_id); if (trg_ns == null) throw ErrUtl.NewArgs("unknown ns", "ns", trg_ns_id);
	...
	private byte[] Bld_html(Xoae_page page) {
		...
		if (src_ttl == null) return BryUtl.Empty;
		...
		fmtr_all.BldToBfrMany(tmp_bfr
		, msg_mgr.Val_by_key_obj("move-page-legend")
		, BryUtl.Add(Xoh_href_.Bry__wiki, src_ttl.Full_db())
...
class Move_trg_ns_list_fmtr implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = 0; i < ns_len; i++) {
			...
			byte[] bry_selected = ttl.Ns().Id() == ns.Id() ? Bry_selected : BryUtl.Empty;
			fmtr.BldToBfrMany(bfr, ns.Id(), bry_selected, ns.Name_combo());
		}
	}
	private static final byte[] Bry_selected = BryUtl.NewA7(" selected=''");
		...
		for (int i = 0; i < args_len; i++) {
			...
			if (tid_obj != null) {
				switch (((ByteVal)tid_obj).Val()) {
					...
					case Key_trg_ns:				trg_ns = BryUtl.ToInt(val_bry); break;
					case Key_trg_ttl:				trg_ttl = val_bry; break;
					case Key_create_redirect:		create_redirect = BryUtl.ToBoolByInt(val_bry); break;
				}
			}
		}
	}
