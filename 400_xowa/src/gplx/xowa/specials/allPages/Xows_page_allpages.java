...
public class Xows_page_allpages implements BryBfrArg, Gfo_invk, Xow_special_page {
	...
	public BryFmtr Html_all() {return html_all;} BryFmtr html_all = BryFmtr.New(StringUtl.ConcatLinesNl
	...
	public BryFmtr Html_list_grp() {return html_list_grp;} BryFmtr html_list_grp = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
		(	""
		,	"  <tr>~{itms}"
		,	"  </tr>"
		),	"itms");
	public BryFmtr Html_list_itm_normal() {return html_list_itm_normal;} BryFmtr html_list_itm_normal = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
		(	""
	...
	public BryFmtr Html_list_itm_redirect() {return html_list_itm_redirect;} BryFmtr html_list_itm_redirect = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
		(	""
	...
	public BryFmtr Html_list_end() {return html_list_end;} BryFmtr html_list_end = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	...
	public void Special__gen(Xow_wiki wikii, Xoa_page pagei, Xoa_url url, Xoa_ttl ttl) {
		...
		url.Page_bry_(BryUtl.Add(BryUtl.NewA7("Special:"), ttl.Page_txt_wo_qargs()));	// HACK: need to re-set Page b/c href_wtr does not eliminate qargs; DATE:2013-02-08
	...
	public boolean Build_data(Xoa_url url, Xoa_ttl ttl) {
		...
		IntRef rslt_len = IntRef.New(rslt_list_len);
		...
		for (int i = 0; i < rslt_list_len; i++)
			rslt_list_ttls[i] = (Xowd_page_itm)rslt_list.GetAt(i);
		rslt_nxt = rslt_nxt2;
		rslt_prv = rslt_prv2;
		return true;
	}	private Gfo_qarg_mgr_old arg_hash = new Gfo_qarg_mgr_old();
	private static final byte[] Bry_arg_from = BryUtl.NewA7("from"), Bry_arg_ns = BryUtl.NewA7("namespace"), Bry_arg_hideredirects = BryUtl.NewA7("hideredirects");
	...
	public void Build_html(Xoae_page page) {
		...
		html_all.BldToBfrMany(tmp_bfr, this, anchor_prv, anchor_nxt);
		page.Db().Text().Text_bry_(tmp_bfr.ToBryAndClear());
		tmp_bfr.MkrRls();
		page.Html_data().Html_restricted_n_();
	}
	byte[] Build_html_end(BryWtr bfr, BryFmtr fmtr, Xowd_page_itm itm, boolean fwd) {
		Xoa_ttl ttl = Xows_page_allpages.ttl_(wiki, init_ns, itm); if (ttl == null) return BryUtl.Empty;	// occurs when range is empty; EX: Module:A in simplewikibooks
		...
		try {
			...
			return html_list_end.BldToBryMany(bfr, arg_from, args__rest, lbl_text);
		} finally {tmp_bfr.MkrRls();}
	}
	public static Xoa_ttl ttl_(Xowe_wiki wiki, Xow_ns ns, Xowd_page_itm itm) {
		byte[] ttl_bry = itm.Ttl_page_db();
		if (!ns.Id_is_main()) ttl_bry = BryUtl.Add(ns.Name_db_w_colon(), ttl_bry);
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = 0; i < len; i += itms_per_grp) {
			...
			html_list_grp.BldToBfrMany(bfr, html_itm_fmtr);
		}
		html_itm_fmtr.XferAry_end();
	}
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		if		(ctx.Match(k, Invk_html_all_)) 					html_all.FmtSet(m.ReadBry("v"));
		else if	(ctx.Match(k, Invk_html_list_grp_)) 			html_list_grp.FmtSet(m.ReadBry("v"));
		else if	(ctx.Match(k, Invk_html_list_itm_normal_)) 		html_list_itm_normal.FmtSet(m.ReadBry("v"));
		else if	(ctx.Match(k, Invk_html_list_itm_redirect_)) 	html_list_itm_redirect.FmtSet(m.ReadBry("v"));
...
class Xos_pagelist_html_itm_fmtr implements BryBfrArg {
	...
	}	private Xows_page_allpages mgr; Xowe_wiki wiki; Xoh_href_wtr href_wtr; BryFmtr itm_normal, itm_redirect; byte[] wiki_key; gplx.xowa.users.history.Xou_history_mgr history_mgr;
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = itm_idx; i < itm_end; i++) {
			...
			BryFmtr fmtr = ttl_itm.Redirected() ? itm_redirect : itm_normal;
			fmtr.BldToBfrMany(bfr, itm_pct, href, title, ttl.Full_txt(), cls);
		}
	}
}
