...
public class Xou_history_mgr_tst {
	...
	@Test public void Archive() {
		GfoDateNow.ManualSetY();	// NOTE: each DateTime_.Now() advances clock by 1 min; adding a new Datetime_now.Get() anywhere will throw off times on this test; DATE:2014-04-01
		fxt.Invk(Xou_history_mgr.Invk_current_itms_max_, 4).Invk(Xou_history_mgr.Invk_current_itms_reset_, 2);
		fxt.Add_many("A", "B", "C", "D", "E");
		fxt.Save();
		fxt.List_tst("E", "D");
		fxt.Fil_tst("mem/xowa/user/test_user/app/data/history/20010101_001000.000.csv", StringUtl.ConcatLinesNl
...
class Xou_history_mgr_fxt {
	...
	public Xou_history_mgr_fxt List_tst(String... expd) {
		...
		for (int i = 0; i < actl_len; i++) {
			...
			actl[i] = StringUtl.NewU8(itm.Page());
		}
