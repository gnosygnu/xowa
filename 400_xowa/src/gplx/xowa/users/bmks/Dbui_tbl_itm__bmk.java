...
public class Dbui_tbl_itm__bmk implements Dbui_tbl_itm {
	...
	public byte[] Key() {return key;} private static final byte[] key = BryUtl.NewA7("bmk");
	...
	public void Select(BryWtr bfr, int owner) {
		Xoud_bmk_itm_row[] db_rows = tbl.Select_grp(owner);
		Xow_wiki usr_wiki = app.User().Wikii();
		byte[] option_link = usr_wiki.Html__lnki_bldr().Href_(BryUtl.NewA7("home"), usr_wiki.Ttl_parse(BryUtl.NewA7("Options/Bookmarks"))).Img_16x16(gplx.xowa.htmls.core.htmls.utls.Xoh_img_path.Img_option).Bld_to_bry();// HOME
	...
	}	private static final byte[] Msg__delete_confirm = BryUtl.NewA7(" data-dbui-delete_confirm_msg='Are you sure you want to delete this bookmark?'");
	...
	public String Save(byte[] row_id, byte[] row_pkey, Dbui_val_hash vals) {
		...
		if (new_url.Page_bry() == null) return msg_bldr.Clear().Notify_fail_(Err.ToStr("Url is invalid", "url", new_url.Raw())).To_json_str();
	...
	public String Reorder(byte[][] pkeys, int owner) {
		...
		int len = db_rows.length; if (len != pkeys.length) return msg_bldr.Clear().Notify_fail_(Err.ToStr("Rows have changed")).Notify_hint_("Please reload the page").To_json_str();
		for (int i = 0; i < len; ++i) {
			int old_pkey = db_rows[i].Id();
			int new_pkey = BryUtl.ToIntOrNeg1(pkeys[i]);
	...
	private String Write_cells(Dbui_val_fmtr val_fmtr, Dbui_btn_itm[] btns, byte[] row_id, Dbui_row_itm row) {
		...
		cells_fmtr.AddToBfr(tmp_bfr);
		return app.Html__bridge_mgr().Msg_bldr().Clear().Data("html", tmp_bfr.ToBryAndClear()).To_json_str();
	}
	private Xoud_bmk_itm_row Get_db_row(byte[] pkey) {
		int bmk_id = BryUtl.ToInt(pkey);
		return tbl.Select_or_null(bmk_id);
	}
	private Dbui_row_itm Get_ui_row(Xoud_bmk_itm_row row) {return Get_ui_row(IntUtl.ToBry(row.Id()), row.Name(), row.Url(), row.Comment());}
	private Dbui_row_itm Get_ui_row(byte[] pkey, byte[] name, byte[] url, byte[] comment) {
		...
		vals[1] = new Dbui_val_itm(url, url_fmtr.BldToBryMany(tmp_bfr, Gfh_utl.Escape_for_atr_val_as_bry(tmp_bfr, AsciiByte.Apos, url)));
