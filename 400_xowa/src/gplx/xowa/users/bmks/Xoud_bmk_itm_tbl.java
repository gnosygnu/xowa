...
public class Xoud_bmk_itm_tbl implements Rls_able {
	...
	public Xoud_bmk_itm_tbl(Db_conn conn) {
		this.conn = conn;
		fld_id						= flds.AddIntPkeyAutonum("itm_id");
		fld_owner					= flds.AddInt("itm_owner");
		fld_sort					= flds.AddInt("itm_sort");
		fld_name					= flds.AddStr("itm_name"		, 255);
		fld_wiki					= flds.AddStr("itm_wiki"		, 255);
		fld_url						= flds.AddStr("itm_url"		, 1024);
		fld_comment					= flds.AddStr("itm_comment"	, 4096);
		conn.Rls_reg(this);
	}
	...
	public Xoud_bmk_itm_row[] Select_grp(int owner) {
		...
		return (Xoud_bmk_itm_row[])list.ToAryAndClear(Xoud_bmk_itm_row.class);
	}
