...
class Xowm_rev_sync {
	...
	public void Exec(byte[] domain_bry, Xoa_ttl[] ttls_ary) {
		synchronized (new_hash) {
			...
			for (int i = 0; i < len; i += batch_size)
				Exec_batch(domain_bry, i, len);
		}
	}
	private void Exec_batch(byte[] domain_bry, int itms_bgn, int itms_len) {
		int itms_end = itms_bgn + batch_size; if (itms_end > itms_len) itms_end = itms_len;
		wkr__cur_meta.Fetch_meta(domain_bry, cur_hash, itms_bgn, itms_end);
		wkr__new_meta.Fetch_meta(domain_bry, new_hash, itms_bgn, itms_end);
