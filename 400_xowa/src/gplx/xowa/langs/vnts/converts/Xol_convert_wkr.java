...
public class Xol_convert_wkr {
	...
	public void Add(byte[] src, byte[] trg) {trie.AddObj(src, trg);}	// called by -{H}-
	public void Del(byte[] src)				{trie.Del(src);}			// called by -{-}-
	public boolean Convert_text(BryWtr bfr, byte[] src) {return Convert_text(bfr, src, 0, src.length);}
	public boolean Convert_text(BryWtr bfr, byte[] src, int bgn, int end) {
		int pos = bgn;
		...
		while (pos < end) {
			...
			if (o == null) {										// no match; skip to next char
				int char_len = Utf8Utl.LenOfCharBy1stByte(b);	// NOTE: must increment by char_len, not +1
				if (matched) {
					if (char_len == 1)
						bfr.AddByte(b);
					else
						bfr.AddMid(src, pos, pos + char_len);
				}
				pos += char_len;
			}
			else {
				if (!matched) {
					bfr.AddMid(src, bgn, pos);	// add everything up to pos
					matched = true;
				}
				bfr.Add((byte[])o);
				pos = trv.Pos();
		...
		if (!matched) bfr.AddMid(src, bgn, end);	// no convert; make sure to add back src, else bfr will be blank
		return matched;
	}
	...
	private void Init_grp(Xol_convert_grp grp) {
		...
		for (int i = 0; i < len; ++i) {
			...
			trie.AddObj(itm.Src(), itm.Trg());	// NOTE: for dupes, latest value wins
		}
	}
}
