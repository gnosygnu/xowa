...
public class Xol_vnt_itm implements Gfo_invk {
	...
	public byte[][]					Fallback_ary() {return fallback_ary;} private byte[][] fallback_ary = BryUtl.AryEmpty;	// EX: zh-hans|zh
	public byte[][]					Convert_ary() {return convert_ary;} private byte[][] convert_ary = BryUtl.AryEmpty;		// EX: zh-hans|zh-cn
	...
	public void Mask__fallbacks_(Xol_vnt_regy regy, byte[][] ary) {
		this.mask_fallbacks = regy.Mask__calc(BryUtl.AryAdd(BryUtl.Ary(key), ary));// NOTE: must add lang.key which is not part of fallback; EX: "zh-cn" has fallback of "zh-hans", but chain should calc "zh-cn","zh-hans"
	}
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		if		(ctx.Match(k, Invk_fallbacks_))			fallback_ary = BrySplit.Split(m.ReadBry("v"), AsciiByte.Pipe);
		else if	(ctx.Match(k, Invk_converts_))			Convert_ary_(BrySplit.Split(m.ReadBry("v"), AsciiByte.Pipe));
