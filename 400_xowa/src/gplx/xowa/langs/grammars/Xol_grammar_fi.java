...
public class Xol_grammar_fi implements Xol_grammar {
	...
	public boolean Grammar_eval(BryWtr bfr, Xol_lang_itm lang, byte[] word, byte[] type) {
		...
		if (aou && BryUtl.HasAtEnd(lower, Bry_wiki))
			aou = false;			
		...
		switch (lower[word_len - 1]) {
			...
			case AsciiByte.Ltr_t: case AsciiByte.Ltr_v: case AsciiByte.Ltr_w: case AsciiByte.Ltr_x: case AsciiByte.Ltr_z:
				bfr.AddByte(AsciiByte.Ltr_i);
				break;
		}

		switch (tid) {
			case Xol_grammar_.Tid_genitive:		bfr.AddByte(AsciiByte.Ltr_n); break;						// case 'genitive': $word .= 'n';
			...
			case Xol_grammar_.Tid_illative:		bfr.AddByte(word[word_len - 1]).AddByte(AsciiByte.Ltr_n); break;// # Double the last letter and add 'n'
		}
		return true;
	}	private static Xol_grammar_manual_regy manual_regy;
	private static final byte[] Bry_sta_y = BryUtl.NewA7("sta"), Bry_sta_n = BryUtl.NewU8("stä"), Bry_a_y = BryUtl.NewA7("a"), Bry_a_n = BryUtl.NewU8("ä"), Bry_ssa_y = BryUtl.NewA7("ssa"), Bry_ssa_n = BryUtl.NewU8("ssä");
	...
	private static final byte[] Bry_wiki = BryUtl.NewA7("wiki");
}
