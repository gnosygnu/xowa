...
class Xol_lnki_trail_mgr_fxt {
	...
	public void Test_add_bulk(String raw, String... expd_ary) {
		lnki_trail_mgr.Add_bulk(BryUtl.NewU8(raw));
		int expd_len = expd_ary.length;
		GfoTstr.EqObj(expd_len, lnki_trail_mgr.Count());
		for (int i = 0; i < expd_len; i++) {
			byte[] expd_bry = BryUtl.NewU8(expd_ary[i]);
			byte[] actl_bry = (byte[])lnki_trail_mgr.Trie().MatchBgn(expd_bry, 0, expd_bry.length);
			GfoTstr.Eq(expd_bry, actl_bry);
		}
	}
}
