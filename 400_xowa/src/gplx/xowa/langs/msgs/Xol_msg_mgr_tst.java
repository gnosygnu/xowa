...
class Xol_msg_mgr_fxt {
	...
	public void Test_val_by_key(String val, String expd) {
		Xol_msg_itm itm = wiki.Lang().Msg_mgr().Itm_by_key_or_new(BryUtl.NewA7("test"));
		itm.Atrs_set(BryUtl.NewA7(val), false, true);
		GfoTstr.EqObj(expd, StringUtl.NewU8(wiki.Msg_mgr().Val_by_key_obj(BryUtl.NewA7("test"))), "has_tmpl_txt");
	}
	public void Test_mediaWiki_msg(String raw, String expd) {
		byte[] msg_ttl = BryUtl.NewA7("MediaWiki:msg_ttl");
		wiki.Db_mgr().Save_mgr().Data_create(wiki, Xoa_ttl.Parse(wiki, msg_ttl), BryUtl.NewA7(raw));
		GfoTstr.EqObj(expd, StringUtl.NewU8(wiki.Msg_mgr().Val_by_key_obj(BryUtl.NewA7("msg_ttl"))));
	}
	public void Test_val_html_accesskey_and_title(String init_title, String init_accesskey, String expd) {
		...
		GfoTstr.EqObj(expd, StringUtl.NewA7(wiki.Msg_mgr().Val_html_accesskey_and_title(BryUtl.NewA7("test"))));
	}
	public void Test__get_msg_val(String key, String expd) {
		GfoTstr.Eq(expd, Xol_msg_mgr_.Get_msg_val(wiki, wiki.Lang(), BryUtl.NewA7(key), BryUtl.AryEmpty));
	}
	private void new_msg_itm_(String key, String val) {
		Xol_msg_itm itm = wiki.Lang().Msg_mgr().Itm_by_key_or_new(BryUtl.NewA7(key));
		itm.Atrs_set(BryUtl.NewA7(val), false, true);
	}
}
