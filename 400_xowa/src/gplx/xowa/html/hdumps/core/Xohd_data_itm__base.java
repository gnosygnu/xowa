...
import gplx.xowa.html.hdumps.data.*; import gplx.xowa.files.repos.*;
public abstract class Xohd_data_itm__base implements Xohd_data_itm {
	public Xohd_data_itm__base Init_by_base(byte[] lnki_ttl, byte lnki_type, int lnki_w, int lnki_h, double lnki_upright, int html_uid, int html_w, int html_h, int file_repo_id, int lnki_ext, boolean file_is_orig, int file_w, double lnki_time, int lnki_page) {
		this.lnki_ttl = lnki_ttl; this.lnki_type = lnki_type; this.lnki_w = lnki_w; this.lnki_h = lnki_h; this.lnki_upright = lnki_upright;
		this.html_uid = html_uid; this.html_w = html_w; this.html_h = html_h;
		this.file_repo_id = file_repo_id; this.lnki_ext = lnki_ext;
		this.file_is_orig = file_is_orig; this.file_w = file_w; this.lnki_time = lnki_time; this.lnki_page = lnki_page;
		return this;
	}
	...
	public void Data_write(Bry_bfr bfr) {
		Data_write_static(bfr, this.Img_tid(), lnki_ttl, lnki_type, lnki_w, lnki_h, lnki_upright, html_uid, html_w, html_h, file_repo_id, lnki_ext, file_is_orig, file_w, lnki_time, lnki_page);
		Data_write_hook(bfr);
		bfr.Add_byte_nl();
	}
	public String Data_print() {
		...
		, Int_.Xto_str(file_repo_id), Int_.Xto_str(lnki_ext), Yn.Xto_str(file_is_orig), Int_.Xto_str(file_w), Double_.Xto_str(lnki_time), Int_.Xto_str(lnki_page)
		);
	}
	public static void Data_write_static(Bry_bfr bfr, int img_tid
		, byte[] lnki_ttl, byte lnki_type, int lnki_w, int lnki_h, double lnki_upright
		, int html_uid, int html_w, int html_h
		, int file_repo_id, int lnki_ext, boolean file_is_orig, int file_w, double lnki_time, int lnki_page) {
