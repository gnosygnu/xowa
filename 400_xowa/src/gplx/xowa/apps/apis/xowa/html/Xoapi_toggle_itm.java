...
public class Xoapi_toggle_itm {
	...
	public byte[] Icon_src() {return icon_src;} private byte[] icon_src = BryUtl.Empty;
	public byte[] Icon_title() {return icon_title;} private byte[] icon_title = BryUtl.Empty;
	public byte[] Elem_display() {return elem_display;} private byte[] elem_display = BryUtl.Empty;
	public byte[] Html_toggle_hdr_cls() {return html_toggle_hdr_cls;} public Xoapi_toggle_itm Html_toggle_hdr_cls_(byte[] v) {html_toggle_hdr_cls = v; return this;} private byte[] html_toggle_hdr_cls = BryUtl.Empty;
	...
	private void Html_toggle_gen() {			
		...
		BryFmtr fmtr = BryFmtr.New(); BryWtr bfr = BryWtr.NewWithSize(8);
		html_toggle_btn
			= fmtr.FmtSet("<a href='javascript:xowa_toggle_visible(\"~{key}\");' style='text-decoration: none !important;'>~{heading}<img id='~{key}-toggle-icon' src='~{src}' title='~{title}' /></a>")
			.KeysSet("key", "src", "title", "heading").BldToBryMany(bfr, key_bry, icon_src, icon_title, heading_bry)
			;
		html_toggle_hdr
			= fmtr.FmtSet(" id='~{key}-toggle-elem' style='~{display}~{toggle_hdr_cls}'")
			.KeysSet("key", "display", "toggle_hdr_cls").BldToBryMany(bfr, key_bry, elem_display, html_toggle_hdr_cls)
			;
	}
	private static byte[] Img_src_y, Img_src_n;	// assume these are the same for all itms
	private static final byte[]
	  Img_title_msg_y = BryUtl.NewA7("hide"), Img_title_msg_n = BryUtl.NewA7("show")
	, Img_display_y = BryUtl.NewA7("display:;"), Img_display_n = BryUtl.NewA7("display:none;")
	;
}
