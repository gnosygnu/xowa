...
public class Xowc_xtn_pages implements Gfo_invk {
	...
	public int Ns_page_id()			{return ns_page_id;}			private int ns_page_id = IntUtl.MinValue;
	public int Ns_page_talk_id()	{return ns_page_talk_id;}		private int ns_page_talk_id = IntUtl.MinValue;
	public int Ns_index_id()		{return ns_index_id;}			private int ns_index_id = IntUtl.MinValue;
	public int Ns_index_talk_id()	{return ns_index_talk_id;}		private int ns_index_talk_id = IntUtl.MinValue;
	...
	public void Reset() {
		ns_page_id = ns_page_talk_id = ns_index_id = ns_index_talk_id = IntUtl.MinValue;
		init_needed = true;
	}
	public void Init(Xow_ns_mgr ns_mgr) {
		...
		for (int i = 0; i < len; i++) {	// Page / Index ns_ids are variable per wiki; iterate over ns, and set ns_id
			...
			if		(BryLni.Eq(ns_name, page_name))		{ns_page_id = ns.Id(); ns_mgr.Ns_page_id_(ns_page_id);}
			else if (BryLni.Eq(ns_name, page_talk_name))	ns_page_talk_id = ns.Id();
			else if (BryLni.Eq(ns_name, index_name))		ns_index_id = ns.Id();
			else if (BryLni.Eq(ns_name, index_talk_name))	ns_index_talk_id = ns.Id();
		}
		int aliases_added = 0;
	...
	private int Set_canonical(Xow_ns_mgr ns_mgr, int aliases_added, int id, byte[] name) {
		...
		if (	ns == null							// ns doesn't exist; should throw error;
			||	!BryLni.Eq(ns.Name_db(), name)		// ns exists, but name doesn't match canonical
			) {
			ns_mgr.Aliases_add(id, StringUtl.NewA7(name));
			++aliases_added;
		}
	...
	public static final byte[] Xtn_key = BryUtl.NewA7("pages");
	public static final int Ns_index_id_default = 102, Ns_page_id_default = 104;

	private static final byte[]
	  Default_ns_page_name			= BryUtl.NewA7("Page")
	, Default_ns_page_talk_name		= BryUtl.NewA7("Page_talk")
	, Default_ns_index_name			= BryUtl.NewA7("Index")
	, Default_ns_index_talk_name	= BryUtl.NewA7("Index_talk")
	;
}
