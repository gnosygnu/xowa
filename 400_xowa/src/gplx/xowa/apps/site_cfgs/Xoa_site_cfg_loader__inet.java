...
public class Xoa_site_cfg_loader__inet implements Xoa_site_cfg_loader {
	...
	public byte[] Load_csv(Xoa_site_cfg_mgr mgr, Xow_wiki wiki, Xoa_site_cfg_itm__base itm) {
		if (IntUtl.In(wiki.Domain_tid(), Xow_domain_tid_.Tid__home, Xow_domain_tid_.Tid__other)) return null;	// do not call api if "home", "other"; for "wikia" and other non-wm wikis
		if (call_api) {
			call_api = false;
	...
	public static String Bld_url(BryWtr tmp_bfr, String domain_str, Hash_adp_bry db_hash, Xoa_site_cfg_itm__base[] itm_ary) {
		boolean first = true;
		...
		return first ? null : Xowm_api_mgr.Bld_api_url(domain_str, Qarg__bgn + tmp_bfr.ToStrAndClear());
	}
	private static final byte[] Bry__query = BryUtl.NewA7("query");
