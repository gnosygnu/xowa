...
class Http_server_wkr_ {
	public static String Assert_main_page(Xoae_app app, String req) {
		int mode = -1;
		String[] req_array = StringUtl.Split(req, "/");
		if		(StringUtl.HasAtEnd(req, "wiki/")) 	mode = 0;
		else if	(StringUtl.HasAtEnd(req, "wiki")) 	mode = 1;
		...
		byte[] wiki_domain = BryUtl.NewU8(req_array[1]);
		...
		wiki.Init_assert();
		String main_page = StringUtl.NewU8(wiki.Props().Main_page());
