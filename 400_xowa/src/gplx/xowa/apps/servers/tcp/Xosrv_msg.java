...
public class Xosrv_msg {
	...
	public void Print(BryWtr bfr) {
		...
		bfr.AddIntFixed(this.Version_tid()	,  1).AddBytePipe();		// 0|
		bfr.AddIntFixed(body_len				, 10).AddBytePipe();		// 0123456789|
		bfr.AddIntFixed(cksum					, 10).AddBytePipe();		// 0123456789|
		bfr.Add(cmd_name							).AddBytePipe();		// cmd|
		bfr.Add(msg_id								).AddBytePipe();		// id|
		bfr.Add(sender								).AddBytePipe();		// sender|
		bfr.Add(recipient							).AddBytePipe();		// recipient|
		bfr.Add(msg_date							).AddBytePipe();		// msg_date|
	...
	public static Xosrv_msg fail_(String fmt, Object... ary) {
		...
		rv.msg_text = BryUtl.NewU8(StringUtl.Format(fmt, ary));
		return rv;
	}
