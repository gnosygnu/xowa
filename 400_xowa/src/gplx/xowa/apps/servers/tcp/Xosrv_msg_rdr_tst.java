...
class Xosrv_msg_rdr_fxt {
	public Xosrv_msg_rdr_fxt Clear() {
		if (msg_rdr == null) {
			...
			msg_rdr = new Xosrv_msg_rdr(BryUtl.Empty, msg_rdr_stream);
		}
	...
	public Xosrv_msg Test_parse_msg(String raw, String expd_cmd, String expd_id, String expd_sender, String expd_recipient, String expd_date, String expd_text) {
		...
		GfoTstr.EqObj(StringUtl.NewA7(msg.Cmd_name())		, expd_cmd);
		GfoTstr.EqObj(StringUtl.NewA7(msg.Msg_id())		, expd_id);
		GfoTstr.EqObj(StringUtl.NewA7(msg.Sender())		, expd_sender);
		GfoTstr.EqObj(StringUtl.NewA7(msg.Recipient())		, expd_recipient);
		GfoTstr.EqObj(StringUtl.NewA7(msg.Msg_date())		, expd_date);
		GfoTstr.EqObj(StringUtl.NewA7(msg.Msg_text())		, expd_text);
		return msg;
	}
	public void Test_parse_err(String raw, String expd_err) {
		...
		msg_rdr_stream.Data_bry_(raw_bry).Read_limit_(raw_bry.length);
		Xosrv_msg msg = msg_rdr.Read();
		String msg_text = StringUtl.NewA7(msg.Msg_text());
		GfoTstr.EqBoolY(StringUtl.HasAtBgn(msg_text, expd_err), msg_text);
	}
