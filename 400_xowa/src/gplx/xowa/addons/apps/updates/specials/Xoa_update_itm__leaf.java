...
class Xoa_update_itm__leaf implements Mustache_doc_itm {
	...
	public boolean Mustache__write(String k, Mustache_bfr bfr) {
		if		(StringUtl.Eq(k, "version"))			bfr.Add_str_u8(version);
		else if	(StringUtl.Eq(k, "date"))				bfr.Add_str_u8(date);
		else if	(StringUtl.Eq(k, "priority"))			bfr.Add_str_u8(Xoa_app_version_itm.Priority__to_name(priority));
		else if	(StringUtl.Eq(k, "summary"))			bfr.Add_str_u8(summary);
		else if	(StringUtl.Eq(k, "details"))			bfr.Add_str_u8(details);
		else if	(StringUtl.Eq(k, "package_url"))		bfr.Add_str_u8(package_url);
		return true;
	}
	public Mustache_doc_itm[] Mustache__subs(String key) {
		if		(StringUtl.Eq(key, "priority_is_major"))		return Mustache_doc_itm_.Ary__bool(priority >= Xoa_app_version_itm.Priority__major);
