...
import org.junit.*;
import gplx.dbs.*;
import gplx.xowa.addons.apps.cfgs.specials.maints.services.*;
...
class Xocfg_cache_mgr__fxt {
	...
	public void Init__db_add(String ctx, String key, Object val) {
		...
		Xocfg_maint_svc.Create_itm(mgr.Db_app(), "test_grp", id++, key, "", "", "wiki", StringUtl.ClsValName, "dflt", "", "");
		mgr.Db_usr().Set_str(ctx, key, ObjectUtl.ToStrOrNull(val));
	}
	...
	public void Test__get(String ctx, String key, String expd) {
		GfoTstr.Eq(expd, mgr.Get(ctx, key), "ctx={0} key={1}", ctx, key);
	}
