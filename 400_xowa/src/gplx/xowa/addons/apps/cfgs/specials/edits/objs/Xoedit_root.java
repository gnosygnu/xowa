...
public class Xoedit_root implements Mustache_doc_itm {
	...
	public boolean Mustache__write(String k, Mustache_bfr bfr) {
		if		(StringUtl.Eq(k, "page_help"))	bfr.Add_str_u8(page_help);
		return true;
	}
	public Mustache_doc_itm[] Mustache__subs(String k) {
		if		(StringUtl.Eq(k, "grps"))			return grps;
		else if	(StringUtl.Eq(k, "nav_exists"))	return Mustache_doc_itm_.Ary__bool(nav_mgr.Itms().length > 1);	// NOTE: do not show combo if 0 or 1 item
		else if	(StringUtl.Eq(k, "app_is_drd"))	return Mustache_doc_itm_.Ary__bool(app_is_drd);
		else if	(StringUtl.Eq(k, "itms"))			return nav_mgr.Itms();
