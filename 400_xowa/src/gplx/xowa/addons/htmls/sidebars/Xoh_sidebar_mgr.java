...
public class Xoh_sidebar_mgr {
	...
	public void Init_by_wiki() {
		try {
			...
			wiki.Appe().Usr_dlg().Warn_many("", "", "sidebar failed: wiki=~{0} err=~{1}", wiki.Domain_str(), ErrUtl.ToStrLog(e));
		}
	}
	private byte[] Get_sidebar_or_null(BryWtr tmp_bfr, Xowe_wiki wiki) {
		...
		byte[] rv = rv_msg.Val();
		if (rv_msg.Defined_in() == Xol_msg_itm.Defined_in__wiki && BryUtl.IsNotNullOrEmpty(rv)) return rv;
	...
	public void Make(BryWtr tmp_bfr, byte[] src) {	// TEST:
		Xoh_sidebar_parser.Parse(tmp_bfr, wiki, grps, src);
		html_bry = Xoh_sidebar_htmlr.To_html(tmp_bfr, wiki, grps);
	}
	private static final byte[] Ttl__sidebar = BryUtl.NewA7("Sidebar");	// MediaWiki:Sidebar
}
