...
class Xoh_toc_wkr__txt__fxt {
	...
	public void Init__tidy(String html, String tidy)		{tidy_mgr.Add(BryUtl.NewU8(html), BryUtl.NewU8(tidy));}
	...
	public void Test__both(String html, String expd_anch, String expd_text) {
		wkr.Calc_anch_text(itm, BryUtl.NewU8(html));
		if (expd_anch != null)	GfoTstr.Eq(expd_anch, itm.Anch(), "anch");
		if (expd_text != null)	GfoTstr.Eq(expd_text, itm.Text(), "text");
	}
	...
	public void Test__remove_comment(String html, String expd) {
		...
		GfoTstr.Eq(expd, Gfh_utl.Del_comments(tmp, html_bry, 0, html_bry.length));
	}
}
class Xow_tidy_mgr_interface__test implements gplx.xowa.htmls.core.htmls.tidy.Xow_tidy_mgr_interface {
	...
	public void Exec_tidy(BryWtr bfr, boolean indent, byte[] page_url) {
		byte[] html = bfr.ToBryAndClear();
		byte[] actl = (byte[])hash.GetByOrFail(html);
		bfr.Add(actl);
	}
}
