...
*/
package gplx.xowa.addons.htmls.tocs; import gplx.*;
...
class Xoh_toc_htmlr implements gplx.core.brys.Bfr_arg {
	...
	public void Bfr_arg__add(Bry_bfr bfr) {
		...
		for (int i = prv_lvl; i > 0; --i) {
			int indent = i * 2;
			bfr.Add_byte_repeat(AsciiByte.Space, indent + 2).Add(Gfh_tag_.Li_rhs).Add_byte_nl();			// EX: "    </li>\n"
			bfr.Add_byte_repeat(AsciiByte.Space, indent    ).Add(Gfh_tag_.Ul_rhs).Add_byte_nl();			// EX: "  </ul>\n"
		}
	}
	private void Write(Bry_bfr bfr, Xoh_toc_itm itm) {
		...
		switch (CompareAbleUtl.Compare(cur_lvl, prv_lvl)) {
			case CompareAbleUtl.More:		// start new "<ul>"
				bfr.Add_byte_repeat(AsciiByte.Space, indent).Add(Gfh_tag_.Ul_lhs).Add_byte_nl();			// EX: "  <ul>\n"
				break;
			...
			case CompareAbleUtl.Less:		// close old "</ul>" and "</li>"
				for (int j = prv_lvl; j > cur_lvl; --j) {
					bfr.Add_byte_repeat(AsciiByte.Space, (j * 2) + 2).Add(Gfh_tag_.Li_rhs).Add_byte_nl();	// EX: "    </li>\n"
					bfr.Add_byte_repeat(AsciiByte.Space, (j * 2)    ).Add(Gfh_tag_.Ul_rhs).Add_byte_nl();	// EX: "    </ul>\n"
				}
				bfr.Add_byte_repeat(AsciiByte.Space, indent + 2).Add(Gfh_tag_.Li_rhs).Add_byte_nl();		// EX: "    </li>\n"
				break;
			default: throw Err_.new_unhandled_default(CompareAbleUtl.Compare(cur_lvl, prv_lvl));
		}

		// write "<li ..."
		bfr.Add_byte_repeat(AsciiByte.Space, indent);	// indent
