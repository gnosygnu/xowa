...
public class Xob_pageprop_cmd extends Xob_sql_dump_base implements Xosql_dump_cbk {
	...
	public void On_fld_done(int fld_idx, byte[] src, int val_bgn, int val_end) {
		switch (fld_idx) {
			case Fld__pp_page:					this.tmp_id					= BryUtl.ToIntOr(src, val_bgn, val_end, -1); break;
			case Fld__pp_propname:				this.tmp_key_is_hiddencat	= BryLni.Eq(src, val_bgn, val_end, Key_hiddencat); break;
		}
	}
	public void On_row_done() {
		if (tmp_key_is_hiddencat)
			tbl.Insert_cmd_by_batch(tmp_id);
		if (++rows % 10000 == 0) usr_dlg.Prog_many("", "", "parsing pageprops sql: row=~{0}", IntUtl.ToStrFmt(rows, "#,##0"));
	}
	...
	private static final byte[] Key_hiddencat = BryUtl.NewA7("hiddencat");
}
