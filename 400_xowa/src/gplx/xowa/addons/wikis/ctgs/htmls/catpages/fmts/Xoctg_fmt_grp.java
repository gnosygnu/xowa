...
public class Xoctg_fmt_grp {	// subc|page|file
	...
	public void Write_catpage_grp(BryWtr bfr, Xow_wiki wiki, Xol_lang_itm lang, Uca_ltr_extractor ltr_extractor, Xoctg_catpage_ctg dom_ctg, int grp_max) {	// TEST:
		...
		byte[] startdiv = BryUtl.Empty, enddiv = BryUtl.Empty;
//			if (count > 6) {
...
//			}
	...
	public byte[] Bld_bwd_fwd(Xow_wiki wiki, Xoa_ttl ttl, Xoctg_catpage_grp view_grp, int grp_max) {	// TEST:
		if (view_grp.Count_all() < grp_max) return BryUtl.Empty;	// NOTE: must be "<", not "<="; FOOTNOTE:LT_NOT_LTE; DATE:2019-12-14
	...
	private void Html_nav_bry(BryWtr bfr, Xow_wiki wiki, Xoa_ttl ttl, Xoctg_catpage_grp grp, int grp_max, boolean url_is_from) {
		BryWtr href_bfr = wiki.Utl__bfr_mkr().GetB512();
		...
		href_bfr.AddByte(AsciiByte.Question).Add(arg_idx_lbl).AddByte(AsciiByte.Eq);		// filefrom=
		Gfo_url_encoder_.Http_url.Encode(href_bfr, arg_sortkey);								// Abc
		href_bfr.AddByte(AsciiByte.Hash).Add(div_id);											// #mw-subcategories
		byte[] nav_href = href_bfr.ToBryAndRls();
		...
		if (	( url_is_from && BryUtl.IsNullOrEmpty(grp.Next_sortkey()))
	...
	private static final BryFmt
	  Fmt__nav__href = BryFmt.New("\n(<a href=\"~{nav_href}\" class=\"xowa_nav\" title=\"~{nav_title}\">~{nav_text}</a>)")
	, Fmt__nav__text = BryFmt.New("\n(~{nav_text})")
	, Fmt__ctg = BryFmt.Auto_nl_skip_last
	( ""
	, "<div id=\"~{div_id}\">"
	, "  <h2>~{msg_label_bry}</h2>"
	...
	private static byte[]
	  Ctg_subc_header = BryUtl.NewA7("subcategories")
	, Ctg_subc_count = BryUtl.NewA7("category-subcat-count")
	, Mw_subcategories = BryUtl.NewA7("mw-subcategories")
	, Ctg_page_header = BryUtl.NewA7("category_header")
	, Ctg_page_count = BryUtl.NewA7("category-article-count")
	, Mw_pages = BryUtl.NewA7("mw-pages")
	, Ctg_file_header = BryUtl.NewA7("category-media-header")
	, Ctg_file_count = BryUtl.NewA7("category-file-count")
	, Mw_category_media = BryUtl.NewA7("mw-category-media")
	;
}
/*
== LT_NOT_LTE ==
