...
class Xoctg_pagebox_hash {
	...
	public Xoctg_pagebox_itm Get_at(int i) {return (Xoctg_pagebox_itm)hash_by_ttl.GetAt(i);}
	public Xoctg_pagebox_itm Get_by_ttl(byte[] full_db) {return (Xoctg_pagebox_itm)hash_by_ttl.GetByOrNull(full_db);}
	...
	public Xoctg_pagebox_itm[] To_ary_and_clear() {
		hash_by_id.Clear();
		return (Xoctg_pagebox_itm[])hash_by_ttl.ToAryAndClear(Xoctg_pagebox_itm.class);
	}
	...
	public void Sort_and_fill_ids() {
		// sort
		hash_by_ttl.SortBy(Xoctg_pagebox_hash_sorter.Sorter);

		// fill ids_hash
		hash_by_id.Clear();
		int len = hash_by_ttl.Len();
		for (int i = 0; i < len; ++i) {
			Xoctg_pagebox_itm itm = (Xoctg_pagebox_itm)hash_by_ttl.GetAt(i);
...
class Xoctg_pagebox_hash_sorter implements ComparerAble {
	public int compare(Object lhsObj, Object rhsObj) {
		...
		return -IntUtl.Compare(lhs.Count__all(), rhs.Count__all());
	}
