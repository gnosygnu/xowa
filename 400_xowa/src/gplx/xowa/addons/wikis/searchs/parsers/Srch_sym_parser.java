...
class Srch_sym_parser__terminal implements Srch_sym_parser {
	public Srch_sym_parser__terminal(byte[] hook_bry) {this.hooks_ary = BryUtl.Ary(hook_bry);}
...
class Srch_sym_parser__split implements Srch_sym_parser {
	...
	public Srch_sym_parser__split(boolean handle_eos, String... hooks_ary_as_str) {
		...
		this.hooks_ary = BryUtl.Ary(hooks_ary_as_str);
		int hooks_len = hooks_ary.length;
		for (int i = 0; i < hooks_len; ++i) {
			byte[] hook = hooks_ary[i];
			trie.AddObj(hook, new IntVal(hook.length));
		}
	}
...
class Srch_sym_parser__paren_bgn implements Srch_sym_parser {
	...
	public Srch_sym_parser__paren_bgn(byte bgn_byte, byte end_byte) {
		...
		this.hooks_ary = BryUtl.Ary(BryUtl.NewByByte(bgn_byte));
	}
	...
	public int Parse(Srch_text_parser mgr, byte[] src, int src_end, int paren_lhs_bgn, int paren_lhs_end) {
		int word_bgn = mgr.Cur__bgn();
		paren_lhs_end = BryFind.FindFwdWhile(src, paren_lhs_end, src_end, bgn_byte);
		int paren_rhs_bgn = BryFind.FindFwd(src, end_byte, paren_lhs_end, src_end);
		if (paren_rhs_bgn == BryFind.NotFound) return paren_lhs_end;	// paren_rhs missing; noop; NOTE: handles both "a(b" -> "a(b" and "a (b" -> "a", "b"
		int paren_rhs_end = BryFind.FindFwdWhile(src, paren_rhs_bgn, src_end, end_byte);
...
class Srch_sym_parser__dot implements Srch_sym_parser {				// handle periods which will add two entries; EX: "H. G. Wells" -> "H.", "G.", "H", "G", "Wells"
	...
	public Srch_sym_parser__dot(String sym_str) {this.sym = BryUtl.NewU8(sym_str); this.sym_len = sym.length;}
	public int Tid() {return Srch_sym_parser_.Tid__dot;}
	public byte[][]	Hooks_ary() {return Hooks_const;} private static final byte[][] Hooks_const = BryUtl.Ary(".");
	public int Parse(Srch_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		int word_bgn = mgr.Cur__bgn();
		if		(word_bgn == -1) {									// dot at start of word; EX: ".NET", "Colt .45"
			...
			if (BryFind.FindFwd(src, sym, hook_bgn + 1, word_end) == -1)	// only add "root" word, if sym is not in middle of String; EX: ".int" -> "int"; ".A.B" x> "A.B"
			...
			if (mgr.Cur__end__chk(hook_end)) {							// at end of word; EX: "U.S.A. b" vs. "H. G. b"
				...
				if (BryFind.FindFwd(src, sym, word_bgn, hook_bgn) == -1)	// no dots in word; add "root" word; "vs." -> "vs.", "vs"; "U.S.A." -> "U.S.A." x> "U.S.A"
					...
					for (int i = word_bgn; i < hook_end; ++i) {			// dots in middle of String; extract word; EX: "U.S.A." -> USA
						if (BryLni.Eq(src, i, i + sym_len, sym)) continue;// skip syms
						mgr.Tmp_bfr.AddByte(src[i]);
					}
					mgr.Words__add_direct(mgr.Tmp_bfr.ToBryAndClear());
				}
				mgr.Cur__bgn__reset();
			}
		}
...
class Srch_sym_parser__ellipsis implements Srch_sym_parser {	// ellipsis which have variable length; EX: "..", "...", ".... "
	...
	public Srch_sym_parser__ellipsis(byte ellipsis_byte, String hook) {
		...
		this.hooks_ary = BryUtl.Ary(hook);
	}
	...
	public int Parse(Srch_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		...
		int rv = BryFind.FindFwdWhile(src, hook_end, src_end, ellipsis_byte);	// skip multiple ellipsis; EX: ......
...
class Srch_sym_parser__apos implements Srch_sym_parser {	// apos which can be contraction ("I'm"), possessive ("today's") and plural ("plans'")
	...
	public Srch_sym_parser__apos(String hook) {
		this.hook_bry = BryUtl.NewU8(hook);
		this.hooks_ary = BryUtl.Ary(hook_bry);
	}
	...
	public int Parse(Srch_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		...
		if		(word_bgn == -1) {							// sym at word_bgn; EX: "'n'"
			...
			int alt_word_end = BryFind.FindBwdSkip(src, word_end, alt_word_bgn, hook_bry);
		...
		if (hook_end == word_end) {							// sym at word_end; EX: "A' "
			...
			if		(hook_bgn  - word_bgn == 1)
				root_word = mgr.Tmp_bfr.AddByte(src[word_bgn]).AddMid(src, hook_bgn + 1, word_end).ToBryAndClear();
			else if	(word_end - hook_end  == 1)
				root_word = mgr.Tmp_bfr.AddMid(src, word_bgn, hook_bgn).AddByte(src[hook_end]).ToBryAndClear();
...
class Srch_sym_parser__dash implements Srch_sym_parser {
	public Srch_sym_parser__dash(String hook) {this.hooks_ary = BryUtl.Ary(hook);}
