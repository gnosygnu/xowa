...
class Srch_text_parser_fxt {
	...
	public Srch_word_itm Make_word(String raw, int count) {return new Srch_word_itm(BryUtl.NewU8(raw)).Count_(count);}
	...
	public void Test__split(String src, Srch_word_itm... expd_words) {
		...
		GfoTstr.EqLines(To_str(expd_words), To_str(word_parser.word_hash));
	}
	private String To_str(Srch_word_itm[] word_ary) {
		...
		for (int i = 0; i < len; ++i) {
			...
			tmp_bfr.Add(word.Word).AddBytePipe();
			tmp_bfr.AddIntVariable(word.Count());
		}
