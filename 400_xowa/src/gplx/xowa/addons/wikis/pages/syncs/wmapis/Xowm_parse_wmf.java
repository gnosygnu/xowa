...
public class Xowm_parse_wmf {
	...
	private static byte[] Core_download(BryWtr bfr, Xof_download_wkr download_wkr, byte[] wiki_domain, byte[] page_full_db) {
		...
		bfr.AddStrA7("action=parse&format=json&page=");	// NOTE:do not add redirects=1; want to get "actual" html, not "redirected" html; DATE:2017-05-06
	...
	private static byte[] Redirect_download(BryWtr bfr, Xof_download_wkr download_wkr, byte[] wiki_domain, byte[] page_full_db) {
		// build url; EX: "https://en.wikipedia.org/w/api.php?action=parse&format=json&redirects=1&page=EARTH&prop="
		Xowm_api_bldr.Bld_bgn(bfr, wiki_domain);
		bfr.AddStrA7("action=parse&format=json&redirects=1&prop=&page=");	// NOTE:"prop=" will ignore all data except for redirect data
		bfr.Add(page_full_db);
