...
public class Xofulltext_cache_mgr {
	...
	public int Ids__get_or_neg1(byte[] qry_key) {
		IntVal id = (IntVal)id_hash.GetByOrNull(qry_key);
		return id == null ? -1 : id.Val();
	}
	public int Ids__next() {return next_qry_id++;}
	public void Add(int id, byte[] key) {
		id_hash.Add(key, new IntVal(id));
		Xofulltext_cache_qry qry = (Xofulltext_cache_qry)qry_hash.GetByOrNull(id);
		if (qry == null) {
	...
	public void Add_page(int query_id, byte[] wiki_bry, int page_id, byte[] page_ttl) {
		// get qry
		Xofulltext_cache_qry qry = (Xofulltext_cache_qry)qry_hash.GetByOrFail(query_id);

		// get page
		Xofulltext_cache_page page = (Xofulltext_cache_page)qry.Pages().GetByOrNull(page_id);
		if (page == null) {
			page = new Xofulltext_cache_page(page_id, qry.Pages().Len(), page_ttl);
	...
	public void Add_line(int query_id, byte[] wiki_bry, int page_id, int line_seq, byte[] line_html) {
		// get qry
		Xofulltext_cache_qry qry = (Xofulltext_cache_qry)qry_hash.GetByOrFail(query_id);
		Xofulltext_cache_page page = (Xofulltext_cache_page)qry.Pages().GetByOrFail(page_id);
	...
	public Xofulltext_cache_qry Get_or_null(int qry_id) {
		return (Xofulltext_cache_qry)qry_hash.GetByOrNull(qry_id);
	}
	public Xofulltext_cache_page[] Get_pages_rng(int qry_id, int page_seq_bgn, int page_seq_end) {
		Xofulltext_cache_qry qry = (Xofulltext_cache_qry)qry_hash.GetByOrNull(qry_id);
		...
		return (Xofulltext_cache_page[])list.ToAryAndClear(Xofulltext_cache_page.class);
	}
	public Xofulltext_cache_line[] Get_lines_rest(int qry_id, byte[] wiki_bry, int page_id) {
		...
		for (int i = 1; i < lines_len; i++) {
			Xofulltext_cache_line line = (Xofulltext_cache_line)page.Lines().GetAt(i);
			list.Add(line);
		}
		return (Xofulltext_cache_line[])list.ToAryAndClear(Xofulltext_cache_line.class);
	}
}
