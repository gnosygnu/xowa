...
class Xow_info_doc implements Mustache_doc_itm {
	...
	public Mustache_doc_itm[] Mustache__subs(String key) {
		if		(StringUtl.Eq(key, "wiki_info"))		return ary;
...
class Xow_info_doc_wiki implements Mustache_doc_itm {
	...
	public boolean Mustache__write(String key, Mustache_bfr bfr) {
		if		(StringUtl.Eq(key, "wiki_domain"))	bfr.Add_bry(domain);
		else if	(StringUtl.Eq(key, "wiki_date"))		bfr.Add_str_u8(date);
		else if	(StringUtl.Eq(key, "wiki_dir"))		bfr.Add_str_u8(dir);
		else if	(StringUtl.Eq(key, "wiki_size"))		bfr.Add_str_u8(size);
		else										return false;
		return true;
	}
