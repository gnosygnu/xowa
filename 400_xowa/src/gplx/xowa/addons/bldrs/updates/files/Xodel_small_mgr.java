...
class Xodel_small_mgr {
	...
	private static void Find_small_files(Db_conn conn, int ext_id, int max) {
		String ext_name = StringUtl.NewU8(Xof_ext_.Get_ext_by_id_(ext_id));
		String reason = "small:" + ext_name;
		conn.Exec_sql_concat_w_msg
		( StringUtl.Format("finding small files; ext={0} max={1}", ext_name, max)
		...
		, "WHERE   f.fil_ext_id = " + IntUtl.ToStr(ext_id)
		, "AND     t.thm_size <= " + IntUtl.ToStr(max)
		);
	}
} 
