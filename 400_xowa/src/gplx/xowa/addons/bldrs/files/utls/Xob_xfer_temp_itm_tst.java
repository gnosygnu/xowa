...
public class Xob_xfer_temp_itm_tst {
	...
	@Test public void Missing_orig()		{fxt.Test_fail(Xob_xfer_temp_itm.Chk_tid_orig_page_id_is_null		, KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_page_id, null));}
	@Test public void File_is_audio()		{fxt.Test_fail(Xob_xfer_temp_itm.Chk_tid_orig_media_type_is_audio	, KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_media_type, Xof_media_type.Name_audio));}
	@Test public void File_is_mid()		{
		fxt.Test_fail(Xob_xfer_temp_itm.Chk_tid_orig_media_type_is_audio	, KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext, Xof_ext_.Id_mid));
	}
	@Test public void Redirect_src_is_empty() {	// orig_cmd sets all direct files to have "orig_join" == "lnki_ttl"
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ttl	, "A.png")
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_lnki_ttl			, "A.png")
		);
	...
	@Test public void Redirect_src_has_val() {	// orig_cmd sets all redirect files to have "orig_join" = redirect and "lnki_ttl" as orig; EX: A.png redirects to B.png; orig_join will be B.png (the actual image) and redirect_src will be A.png (the original lnki)
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ttl	, "B.png")
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_lnki_ttl			, "A.png")
		);
	...
	@Test public void Redirect_should_take_trg_ext() {// if "A.png" redirects to "B.jpg", ext_id should be ".jpg" (the actual file) not ".png (lnki_ext_id)
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ttl	, "B.jpg")
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_lnki_ttl			, "A.png")
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext	, Xof_ext_.Id_jpg)	// .png b/c B.jpg
		);
	...
	@Test public void Thumbtime_check() {// PURPOSE: one image actually had a thumbtime defined; EX: General_Dynamics_F-16_Fighting_Falcon; [[File:Crash.arp.600pix.jpg|thumb|thumbtime=2]]
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext	, Xof_ext_.Id_jpg)
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_time		, (double)3)
		);
		fxt.Test_lnki_thumbtime(Xof_lnki_time.Null);
		fxt.Reset().Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_media_type	, Xof_media_type.Name_video)
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_time		, (double)3)
		);
	...
	@Test public void Page_check() {
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext	, Xof_ext_.Id_jpg)
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_page		, 3)
		);
		fxt.Test_lnki_page(Xof_lnki_page.Null);
		fxt.Reset().Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext	, Xof_ext_.Id_pdf)
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_page		, 3)
		);
		fxt.Test_lnki_page(3);
		fxt.Reset().Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_file_ext	, Xof_ext_.Id_djvu)
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_page		, 3)
		);
		fxt.Test_lnki_page(3);
	}
	@Test public void Media_should_be_ignored() {// ignore [[Media:]] for xfer_thumb (needed for xfer_orig)
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_lnki_ttl			, "A.png")
		,	KeyVal.NewStr(Xob_lnki_regy_tbl.Fld_lnki_src_tid		, Xop_file_logger_.Tid__media)
		);
	...
	@Test public void Orig_width_is_0() {// PURPOSE: ignore files with an orig width of 0; note that ogg files that are sometimes flagged as VIDEO; EX:2009_10_08_Marc_Randazza_interview.ogg; DATE:2014-08-20
		fxt.Test_bgn
		(	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_lnki_ttl			, "A.ogg")
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_media_type	, Xof_media_type.Name_video)	// VIDEO
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_w			, 0)							// no width defined in image table
		,	KeyVal.NewStr(Xob_orig_regy_tbl.Fld_orig_h			, 0)
		);
...
class Xob_xfer_temp_itm_fxt {
	...
	public Xob_xfer_temp_itm_fxt Init_rdr_image() {
		...
		nde = GfoNde_.vals_(flds, ObjectUtl.Ary
	...
	public Xob_xfer_temp_itm_fxt Test_atr(String key, Object val) {
		GfoTstr.EqObj(rdr.Read(key), val);
		return this;
	}
	...
	public Xob_xfer_temp_itm_fxt Test_lnki_ext_id(int expd) {GfoTstr.EqObj(expd, itm.Lnki_ext()); return this;}
	public Xob_xfer_temp_itm_fxt Test_lnki_thumbtime(double expd) {GfoTstr.EqObj(expd, itm.Lnki_thumbtime()); return this;}
	public Xob_xfer_temp_itm_fxt Test_lnki_page(int expd) {GfoTstr.EqObj(expd, itm.Lnki_page()); return this;}
	public Xob_xfer_temp_itm_fxt Test_lnki_redirect_src(String expd) {GfoTstr.EqObj(expd, itm.Redirect_src()); return this;}
	...
	public Xob_xfer_temp_itm_fxt Test_itm_chk_fail_id(byte expd) {
		GfoTstr.EqObj(expd, itm.Chk_tid());
		return this;
	}
}
