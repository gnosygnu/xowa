...
public class Xob_bin_db_mgr {
	...
	private final Ordered_hash nth_hash = Ordered_hash_.New(); private final IntRef tier_key = IntRef.NewNeg1();
	...
	public void Init_by_mnt_mgr(Fsm_mnt_mgr trg_mnt_mgr) {
		...
		for (int i = 0; i < len; ++i) {	// iterate ns_ids and add default nth
			...
			nth_hash.Add(IntRef.New(ns_ids[i]), nth);
		}
		...
		for (int i = 0; i < len; ++i) {	// iterate bin_dbs to find max pt_id for each ns
			...
			int ns_id = itm.Ns_id();
			Xob_bin_db_itm nth = (Xob_bin_db_itm)nth_hash.GetByOrNull(tier_key.ValSet(ns_id));
		...
		for (int i = 0; i < len; ++i) {	// iterated tiers to calculate max_size
			Xob_bin_db_itm nth = (Xob_bin_db_itm)nth_hash.GetAt(i);
	...
	public Xob_bin_db_itm Get_nth_by_tier(int tier_id) {
		if (schema_is_1) return (Xob_bin_db_itm)nth_hash.GetByOrNull(tier_key.ValSet(0)); // v1 is always in ns_0
		if (tier_id >= ns_ids_len) throw ErrUtl.NewArgs("tier out of range", "tier_id", tier_id, "len", ns_ids_len);
		int ns_id = ns_ids[tier_id];
		return (Xob_bin_db_itm)nth_hash.GetByOrNull(tier_key.ValSet(ns_id));
	}
}
