...
class Pack_hash {
	...
	public Pack_list	Get_at(int i)		{return (Pack_list)hash.GetAt(i);}
	public Pack_list	Get_by(int tid)		{return (Pack_list)hash.GetByOrNull(tid);}
	...
	public Pack_itm Add(int list_tid, Io_url pack_url, Io_url... raw_urls) {
		Pack_list list = (Pack_list)hash.GetByOrNull(list_tid);
		if (list == null) {
	...
	public void Consolidate(int... tids) {	// merge n itms into 1 itm; needed for search-core + search-link -> search
		...
		for (int i = 0; i < tids_len; ++i) {
			Pack_list list = (Pack_list)hash.GetByOrNull(tids[i]);
