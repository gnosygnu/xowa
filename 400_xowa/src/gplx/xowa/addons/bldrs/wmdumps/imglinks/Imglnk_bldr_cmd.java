...
public class Imglnk_bldr_cmd extends Xob_sql_dump_base implements Xosql_dump_cbk {
	...
	public Imglnk_bldr_cmd(Xob_bldr bldr, Xowe_wiki wiki) {this.Cmd_ctor(bldr, wiki); this.make_fil_len = IoConsts.LenMB;}
	...
	public void On_fld_done(int fld_idx, byte[] src, int val_bgn, int val_end) {
		switch (fld_idx) {
			case Fld__il_from:			this.tmp_page_id = BryUtl.ToIntOr(src, val_bgn, val_end, -1); break;
			case Fld__il_to:			this.tmp_il_to = BryLni.Mid(src, val_bgn, val_end); break;
		}
