...
public class Ref_itm_grp {
	...
	public Ref_itm_lst Lsts_get_at(int i) {return i < lsts.Count() ? (Ref_itm_lst)lsts.FetchAt(i) : null;}	// NOTE: null can be returned; see Infobox planet; w:Mars
	public void Lsts_add(byte[] itm_name, byte[] follow, Ref_nde itm) {
		...
		if (lst_idx >= lsts_len) {
			...
			lst = (Ref_itm_lst)lsts.FetchAt(lsts_len - 1);
		lst.Itms_add(itm_name, follow, itm);
	}
	public void Lsts_clear() {
		int lsts_len = lsts.Count();
		for (int i = 0; i < lsts_len; i++) {
			Ref_itm_lst lst = (Ref_itm_lst)lsts.FetchAt(i);
			lst.Itms_clear();
		}
		lsts.Clear();
		lst_idx = 0;
