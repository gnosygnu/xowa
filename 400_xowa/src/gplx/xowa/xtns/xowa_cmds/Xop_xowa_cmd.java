...
public class Xop_xowa_cmd implements Xox_xnde {
	public Xop_root_tkn Xtn_root() {throw ErrUtl.NewUnimplemented("xowa_cmd.xtn_root should not be called");}
	...
	public void Xtn_parse(Xowe_wiki wiki, Xop_ctx ctx, Xop_root_tkn root, byte[] src, Xop_xnde_tkn xnde) {
		...
		if (src[itm_end - 1] 	== AsciiByte.Nl) --itm_end;	// ignore last \n;
		byte[] raw = BryLni.Mid(src, itm_bgn, itm_end);
		byte[] xtn_src = raw;
		if (wiki.Sys_cfg().Xowa_cmd_enabled()) {	// only exec if enabled for wiki
			Object rslt = wiki.Appe().Gfs_mgr().Run_str(StringUtl.NewU8(raw));
			xtn_src = BryUtl.NewU8(ObjectUtl.ToStrOrNullMark(rslt));
		}
