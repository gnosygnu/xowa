...
public class Wdata_wiki_mgr implements Gfo_evt_itm, Gfo_invk {
	...
	public byte[] Domain() {return domain;} public void Domain_(byte[] v) {domain = v;} private byte[] domain = BryUtl.NewA7("www.wikidata.org");
	...
	public Wdata_doc_parser Wdoc_parser(Json_doc jdoc) {
		...
		return BryLni.Eq(itm_0.Key().Data_bry(), Wdata_doc_parser_v2.Bry_type)
			|| BryLni.Eq(itm_0.Key().Data_bry(), Wdata_doc_parser_v2.Bry_id)
	...
	public void Resolve_claim(BryWtr rv, Xow_domain_itm domain, Wbase_claim_base claim_itm) {
	...
	public void Resolve_to_bfr(BryWtr bfr, Xowe_wiki wiki, Wbase_claim_grp prop_grp, byte[] lang_key, boolean mode_is_statements) {
		synchronized (thread_lock) {	// LOCK:must synchronized b/c prop_val_visitor has member bfr which can get overwritten; DATE:2016-07-06
			if (hwtr_mgr == null) Hwtr_mgr_assert();
	...
	public byte[] Popup_text(Xoae_page page) {
		...
		if (wdoc == null) return BryUtl.Empty;
		return hwtr_mgr.Popup(wdoc);
	}
	public void Write_json_as_html(BryWtr bfr, Xoa_ttl page_ttl, byte[] data_raw) {
		Hwtr_mgr_assert();
	...
	public static void Write_json_as_html(Json_parser jdoc_parser, BryWtr bfr, byte[] data_raw) {
	...
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		...
		else if	(ctx.Match(k, Invk_domain))						return StringUtl.NewU8(domain);
	...
	public static final byte[] Ns_property_name_bry = BryUtl.NewA7(Ns_property_name);
	public static final int Ns_lexeme = 146;
	public static final String Ns_lexeme_name = "Lexeme";
	public static final byte[] Ns_lexeme_name_bry = BryUtl.NewA7(Ns_lexeme_name);

	public static final byte[] Html_json_id = BryUtl.NewA7("xowa-wikidata-json");
