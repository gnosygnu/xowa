...
public class Wdata_xwiki_link_wtr implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		if (!BryLni.Eq(qid, Qid_null) && !page_ttl.Ns().Id_is_special())	// don't write "In other languages" if no qid; also skip Special ns; needed for pages with wbase page, but no sitelinks; PAGE:en.w:Tintinan; DATE:2015-08-03
	...
	public static byte[] Write_wdata_links(List_adp slink_list, Xowe_wiki wiki, Xoa_ttl ttl, Wdata_external_lang_links_data external_links_mgr) {
		try {
			...
			int len = links.Len();
			for (int i = 0; i < len; i++) {
				Wdata_sitelink_itm slink = (Wdata_sitelink_itm)links.GetAt(i);
				byte[] xwiki_key = slink.Site();
				Xow_abrv_wm abrv_itm = Xow_abrv_wm_.Parse_to_abrv_or_null(xwiki_key);
				if (abrv_itm == null) {
					wiki.Appe().Usr_dlg().Warn_many("", "", "unknown wiki in wikidata: ttl=~{0} wiki=~{1}", ttl.Page_db_as_str(), StringUtl.NewU8(xwiki_key));
					continue;
				}
				...
				tmp_bfr.AddByte(AsciiByte.Colon);
				tmp_bfr.Add(slink.Name());
				Xoa_ttl slink_ttl = Xoa_ttl.Parse(wiki, tmp_bfr.ToBryAndClear());
				...
				if (	xwiki_itm == null									// not a known xwiki; EX: [[zzz:abc]]
					||	BryLni.Eq(xwiki_itm.Domain_bry(), wiki.Domain_bry())	// skip if same as self; i.e.: do not include links to enwiki if already in enwiki
					) continue;
			...
			return doc.Qid();
		} catch (Exception e) {return Qid_null;}
	}
	public static final byte[] Qid_null = BryUtl.Empty;	// NOTE: return Empty, not null else Bry_fmtr will fail
}
class Xoa_ttl_sorter implements ComparerAble {
	public int compare(Object lhsObj, Object rhsObj) {
		Xoa_ttl lhs = (Xoa_ttl)lhsObj, rhs = (Xoa_ttl)rhsObj;
		return BryUtl.Compare(lhs.Raw(), rhs.Raw());
	}
