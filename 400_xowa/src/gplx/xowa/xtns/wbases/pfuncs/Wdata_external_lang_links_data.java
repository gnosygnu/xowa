...
public class Wdata_external_lang_links_data {
	...
	public byte[] Langs_get_at(int i) {return (byte[])langs_list.GetAt(i);}
	...
	public void Parse(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, Pf_func_base pfunc) {
		...
		if (BryLni.Eq(argx, Key_sort)) {sort = true; return;}	// {{noexternallanglinks:*}}; assume it cannot be combined with other langs_hash: EX: {{noexternallanglinks:*|en|fr}}
		...
		for (int i = 0; i < args_len; i++) {
			...
			byte[] lang = tmp_bfr.ToBryAndClear();
			Langs_add(lang);
		}
		tmp_bfr.MkrRls();
	}
