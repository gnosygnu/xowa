...
class Wdata_itemByTitle_page_fxt {
	...
	public void Init_wdata_page(String qid_ttl, String text) {
		...
		app.Wiki_mgr().Wdata_mgr().Doc_mgr.Add(BryUtl.NewA7(qid_ttl), doc);
	...
	public void Test_open(String link, String expd) {
		...
		GfoTstr.EqLines(expd, StringUtl.NewA7(page.Db().Text().Text_bry()));
	}
	public String Expd_html(String wmf_key, String ttl_str) {
		BryWtr tmp_bfr = wiki.Utl__bfr_mkr().GetK004();
		special_page.Html_fmtr().BldToBfrMany(tmp_bfr, "Search for items by site and title", "Site", wmf_key, "Page", ttl_str, "Search");
		return tmp_bfr.ToStrAndRls();
	}
}
