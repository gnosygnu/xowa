...
public class Wdata_hwtr_mgr {
	private final BryWtr bfr = BryWtr.NewAndReset(IoConsts.LenMB);
	public Wdata_fmtr__toc_div	Fmtr_toc() {return fmtr_toc;} private final Wdata_fmtr__toc_div fmtr_toc = new Wdata_fmtr__toc_div();
	public Wdata_fmtr__json		Fmtr_json() {return fmtr_json;} private final Wdata_fmtr__json fmtr_json = new Wdata_fmtr__json();
	public Wdata_fmtr__claim_grp	Fmtr_claim() {return fmtr_claim;} private final Wdata_fmtr__claim_grp fmtr_claim = new Wdata_fmtr__claim_grp();
	...
	public BryFmtr Fmtr_main() {return fmtr_main;} private final BryFmtr fmtr_main = BryFmtr.New("~{oview}~{toc}~{claims}~{links}~{labels}~{descriptions}~{aliases}~{json}", "oview", "toc", "claims", "links", "labels", "descriptions", "aliases", "json");
	...
	public byte[] Popup(Wdata_doc wdoc) {
		...
		fmtr_descr.Init_by_wdoc(wdoc.Descr_list());
		fmtr_alias.Init_by_wdoc(wdoc.Alias_list());
		bfr.AddStrA7("<div id='wb-item' class='wikibase-entityview wb-item' lang='en' dir='ltr'>");
		bfr.AddStrA7("<div class='wikibase-entityview-main'>");
		fmtr_main.BldToBfrMany(bfr, fmtr_oview, "", "", "", "", "", "", "");
		bfr.AddStrA7("</div>");
		bfr.AddStrA7("</div>");
	...
	public byte[] Write(Wdata_doc wdoc) {
		bfr.AddStrA7("<div id='wb-item' class='wikibase-entityview wb-item' lang='en' dir='ltr'>");
		bfr.AddStrA7("<div class='wikibase-entityview-main'>");
		fmtr_main.BldToBfrMany(bfr, fmtr_oview, fmtr_toc, fmtr_claim, fmtr_slink, fmtr_label, fmtr_descr, fmtr_alias, fmtr_json);
		bfr.AddStrA7("</div>");
		bfr.AddStrA7("</div>");
		return bfr.ToBryAndClear();
	}
	public static void Write_link_wikidata(BryWtr bfr, byte[] href, byte[] text) {
		text = gplx.langs.htmls.Gfh_utl.Escape_html_as_bry(text);
		fmtr_link_wikidata.BldToBfrMany(bfr, href, text);
	}	private static final BryFmtr fmtr_link_wikidata = BryFmtr.New("<a href='/wiki/~{href}'>~{text}</a>", "href", "text");
}
