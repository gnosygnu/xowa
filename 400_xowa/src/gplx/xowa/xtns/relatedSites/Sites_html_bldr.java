...
class Sites_xtn_skin_itm implements Xopg_xtn_skin_itm {
	...
	public byte[] Key() {return KEY;} public static final byte[] KEY = BryUtl.NewA7("RelatedSites");
...
public class Sites_html_bldr implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = 0; i < list_len; ++i) {
			...
			fmtr_itm.BldToBfr(bfr, itm.Cls(), href, xwiki_itm_name);
		}
	}
	private static byte[] Xto_href(BryWtr tmp_bfr, BryFmtr url_fmtr, Xoh_href_parser href_parser, Xowe_wiki wiki, Xow_xwiki_itm xwiki_itm, byte[] ttl_page_db) {
		gplx.langs.htmls.encoders.Gfo_url_encoder_.Href.Encode(tmp_bfr, ttl_page_db);
		byte[] rv = url_fmtr.FmtSet(xwiki_itm.Url_fmt()).BldToBryMany(tmp_bfr, tmp_bfr.ToBryAndClear());
		if (xwiki_itm.Domain_tid() != Xow_domain_tid_.Tid__other)
			rv = BryUtl.Add(Xoh_href_.Bry__site, rv);
		return rv;
	}
