...
public class Process_stream_rdr {
	...
	public byte[] Read() {
		...
		if (bytes_read < 16) {
			...
			else throw Err_.new_("failed to read header");
		}
		int body_len = HexDecUtl.parse_or_(bry_header, 0,8, -1); 	if (body_len == -1) throw Err_.new_("failed to read body_len");
		int chk_len= HexDecUtl.parse_or_(bry_header, 9, 16, -1);	if (chk_len == -1 || chk_len != (body_len * 2) - 1)throw Err_.new_("failed to read chk_len");
