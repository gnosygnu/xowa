...
public class Scrib_lua_mod {
	...
	public int Fncs_len() {return hash.Len();}
	public Scrib_lua_proc Fncs_get_at(int i) {return (Scrib_lua_proc)hash.GetAt(i);}
	public Scrib_lua_proc Fncs_get_by_key(String key) {return (Scrib_lua_proc)hash.GetByOrNull(key);}
	public void Fncs_add(Scrib_lua_proc prc) {hash.Add(prc.Key(), prc);}
	public int Fncs_get_id(String key) {
		Scrib_lua_proc fnc = Fncs_get_by_key(key); if (fnc == null) throw ErrUtl.NewArgs("Scrb_fnc does not exist", "module", name, "func", key);
		return fnc.Id();
	}
	public Scrib_lua_proc LoadString(String text) {
		if (lua_id != -1) return init_chunk_func;
		text = StringUtl.Replace(text, "&#09;", "\t");	// NOTE: this should only get called once per module
		text_bry = BryUtl.NewU8(text);
