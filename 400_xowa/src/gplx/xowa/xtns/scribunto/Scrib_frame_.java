...
public class Scrib_frame_ {
	...
	public static int Get_frame_tid(String frame_id) {
		if		(StringUtl.Eq(frame_id, "current"))	return Tid_current;
		else if (StringUtl.Eq(frame_id, "parent"))	return Tid_parent;
		else										return Tid_dynamic;
	}
	public static Xot_invk Get_frame(Scrib_core core, String frame_id) {
		if		(StringUtl.Eq(frame_id, "current"))	return core.Frame_current();
		else if (StringUtl.Eq(frame_id, "parent"))	return core.Frame_parent();
		else if (StringUtl.Eq(frame_id, "empty"))		return Xot_invk_mock.new_(core.Frame_current().Defn_tid(), 0, null, KeyValUtl.AryEmpty);	// not sure if it should return null title; DATE:2014-07-12
		else {
