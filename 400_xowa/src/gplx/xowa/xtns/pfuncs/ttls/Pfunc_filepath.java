...
public class Pfunc_filepath extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		// init
		byte[] val_ary = Eval_argx(ctx, src, caller, self); if (val_ary == BryUtl.Empty) return;
		...
		if (tmp_rslt.Repo_idx() == ByteUtl.MaxValue127) return;
		...
		xfer_itm.Orig_ttl_and_redirect_(ttl_bry, BryUtl.Empty);	// redirect is empty b/c Get_page does all redirect lookups
