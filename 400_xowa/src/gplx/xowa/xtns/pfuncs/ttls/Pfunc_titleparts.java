...
public class Pfunc_titleparts extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {// REF.MW:ParserFunctions_body.php
		...
		int parts_len = parts_len_ary == BryUtl.Empty ? IntUtl.MinValue : BryUtl.ToIntOr(parts_len_ary, IntUtl.MaxValue);
		if (parts_len == IntUtl.MaxValue) {// len is not an int; EX: "a";
		...
		int parts_bgn = parts_bgn_arg == BryUtl.Empty ? 0 : BryUtl.ToIntOr(parts_bgn_arg, IntUtl.MinValue);
		if (parts_bgn == IntUtl.MinValue) {// parts_bgn is not an int; EX: "a"
	...
	private byte[] TitleParts(byte[] src, int parts_len, int parts_bgn) {
		...
		synchronized (dlms_ary) {// LOCK:static-obj; DATE:2016-07-06
			...
			if (	bgn_idx < 0																// bgn_idx can be negative when parts_len is negative and greater than array; EX: {{#titleparts:a/b|-1|-2}} results in dlms_ary_len of 1 and parts_bgn of -2 which will be parts_bgn of -1
				||	bgn_idx > dlms_ary_len) return BryUtl.Empty;							// if bgn > len, return ""; EX: {{#titleparts:a/b|1|3}} should return ""
			// adjust parts_len for negative/null
			if		(parts_len == IntUtl.MinValue) parts_len = dlms_ary_len;					// no parts_len; default to dlms_ary_len
				...
				if (parts_len < 1) return BryUtl.Empty;									// NOTE: if zerod'd b/c of neg length, return empty; contrast with line below; EX: a/b/c|-4
			}		
			...
			if (end_pos < bgn_pos) return BryUtl.Empty;
			return BryLni.Mid(src, bgn_pos, end_pos);
		}
