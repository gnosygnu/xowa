...
public class Pfunc_displaytitle extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		...
		byte[] val_html = tmp_bfr.ToBryAndClear();
		if (restrict) {	// restrict only allows displayTitles which have text similar to the pageTitle; PAGE:de.b:Kochbuch/_Druckversion; DATE:2014-08-18
			...
			byte[] val_html_lc = tmp_bfr.ToBryAndClear();
			...
			if (!BryLni.Eq(val_html_lc, page_ttl_lc))
				val_html = null;
		}
	...
	private static byte[] Standardize_displaytitle_text(Xol_case_mgr case_mgr, byte[] val) {
		...
		return BryUtl.Replace(rv, AsciiByte.Space, AsciiByte.Underline);	// force underline; PAGE:de.w:Mod_qos DATE:2014-11-06
	}
