...
public class Pfunc_rev_props extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		...
		switch (id) {
			...
			case Xol_kwd_grp_.Id_rev_id:				bfr.AddIntVariable(ctx.Page().Db().Page().Id()); break;	// NOTE: making rev_id and page_id interchangeable; XOWA does not store rev_id
			...
			case Xol_kwd_grp_.Id_rev_revisionsize:	// default revsize to pagesize; MW has additional logic for subst which should not apply to XO; https://gerrit.wikimedia.org/r/#/c/82650/
				bfr.AddIntVariable(ctx.Page().Db().Text().Text_bry().length);
				break;
