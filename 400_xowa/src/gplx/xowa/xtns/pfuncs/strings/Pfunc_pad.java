...
public class Pfunc_pad extends Pf_func_base {
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {// REF.MW: CoreParserFunctions.php|pad
		int self_args_len = self.Args_len();
		byte[] val = Eval_argx(ctx, src, caller, self);
		int val_len = Utf8Utl.LenOfBry(val);			// NOTE: length must be in chars, not bytes, else won't work for non-ASCII chars; EX:niǎo has length of 4, not 5; PAGE:zh.d:不 DATE:2014-08-27
		
		byte[] pad_len = Pf_func_.Eval_arg_or_empty(ctx, src, caller, self, self_args_len, 0);
		int pad_len_int = BryUtl.ToIntOr(pad_len, 0, pad_len.length, -1);
		...
		for (int val_idx = val_len; val_idx < pad_len_int; val_idx++) {
			...
			if (b_len == 1)
				...
				bfr.AddMid(pad_str, pad_idx, pad_idx + b_len);
			pad_idx += b_len;
			if (pad_idx >= pad_str_len) pad_idx = 0;
		}
		if (!pad_dir_right) bfr.Add(val);
	}
	boolean pad_dir_right; static final byte[] Ary_pad_dflt = BryUtl.NewA7("0");
