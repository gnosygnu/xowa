...
public class Pfunc_pagesincategory extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {	// REF.MW: /includes/parser/CoreParserFunctions.php|pagesincategory
		byte[] ctg_ttl_bry = Eval_argx(ctx, src, caller, self);
		if (BryUtl.IsNullOrEmpty(ctg_ttl_bry)) {bfr.AddIntDigits(1, 0); return;}			// no title; EX: "{{PAGESINCATEGORY:}}"
		...
		if (ctg_id == Xowd_page_itm.Id_null) {bfr.AddIntDigits(1, 0); return;}	// category doesn't exist; EX: "{{PAGESINCATEGORY:Unknown_category}}"
		Xowd_category_itm ctg_itm = Xodb_cat_db_.Get_cat_core_or_fail(core_data_mgr).Select(ctg_id);
		if (ctg_itm == null) {bfr.AddIntDigits(1, 0); return;}					// category counts don't exist; shouldn't happen
