...
public class Pft_func_time__basic__tst {
	@Before	public void init()					{fxt.Reset(); GfoDateNow.ManualSet(GfoDateUtl.New(2012, 1, 2, 3, 4, 5, 6));} private final Xop_fxt fxt = new Xop_fxt();
	...
	@Test public void Timezone_offset()		{
		GfoDate.TimezoneOffsetTest = -18000;
		fxt.Test_parse_tmpl_str("{{#time:Z|}}"											, "-18000");
		GfoDate.TimezoneOffsetTest = IntUtl.MinValue;
	...
	@Test public void Lang() {
		Xol_lang_itm fr_lang = fxt.App().Lang_mgr().Get_by_or_new(BryUtl.NewA7("fr"));
		Xol_msg_itm msg_itm = fr_lang.Msg_mgr().Itm_by_key_or_new(BryUtl.NewA7("January"));
		msg_itm.Atrs_set(BryUtl.NewA7("Janvier"), false, false);
...
class Pft_func_time__fxt {
	...
	public void Test__parse_w_offset_mins(int offset, String raw, String expd) {
		try {
			GfoDate.TimezoneOffsetTest = offset * 60;
