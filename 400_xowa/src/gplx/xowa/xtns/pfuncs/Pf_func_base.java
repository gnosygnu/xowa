...
public abstract class Pf_func_base implements Pf_func {
	...
	public byte[] Name() {return name;} public Pf_func_base Name_(byte[] v) {name = v; name_len = v.length; return this;} private byte[] name = BryUtl.Empty; int name_len = 0;
	...
	public abstract void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src);
	public byte[] Argx_dat() {return argx_dat;} public void Argx_dat_(byte[] v) {argx_dat = v;} private byte[] argx_dat = BryUtl.Empty;
	public byte[] Eval_argx(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self) {
		if (argx_dat == BryUtl.Empty) {
			...
			if (subs_len > 0) {
				...
				argx_dat = tmp.ToBryAndClearAndTrim();
			}
		}
		return argx_dat;
	}		
	public byte[] Eval_argx_or_null(Xop_ctx ctx, byte[] src, Xot_invk caller, Xot_invk self, byte[] func_name) {
		if (argx_dat == BryUtl.Empty) {
			...
			if (subs_len == 0) {	// no subs; either {{#func}} or {{#func:}}
				...
				int colon_pos = BryFind.FindBwd(src, AsciiByte.Colon, self.Src_end(), src_bgn);	// look for ":"; NOTE: used to be src_bgn - 1, but this would always search one character too many; DATE:2014-02-11
				if (colon_pos == BryFind.NotFound)		// no colon; EX: {{#func}}
					return Eval_arg_or_null_is_null;
				else {									// colon found; EX: {{#func:}}
					if (BryUtl.MatchBwdAny(src, colon_pos - 1, src_bgn - 1, func_name))  // #func == func_name; EX: {{NAMESPACE:}}
	...
	public static final byte[] Eval_arg_or_null_is_null = null, Eval_arg_or_null_is_empty = BryUtl.Empty;
}
