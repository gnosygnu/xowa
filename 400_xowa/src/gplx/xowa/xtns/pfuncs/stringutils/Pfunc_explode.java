...
public class Pfunc_explode extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		...
		byte[] dlm = AsciiByte.SpaceBry; int idx = 0, limit = -1;
		if (args_len > 0) {
			dlm = Pf_func_.Eval_arg_or(ctx, src, caller, self, args_len, 0, null);
			if (BryUtl.IsNullOrEmpty(dlm)) dlm = AsciiByte.SpaceBry;	// handle empty String; EX: {{#explode:a b||1}}
			if (args_len > 1) {
				byte[] pos_bry = Pf_func_.Eval_arg_or(ctx, src, caller, self, args_len, 1, null);
				if (pos_bry != null) idx = BryUtl.ToIntOr(pos_bry, 0);
				if (args_len > 2) {
					byte[] limit_bry = Pf_func_.Eval_arg_or(ctx, src, caller, self, args_len, 2, null);
					if (limit_bry != null) limit = BryUtl.ToIntOr(pos_bry, -1);
				}
			}
		}
		if (idx < 0) {
	...
	private static byte[] Split_and_get_by_idx(byte[] src, byte[] dlm, int idx, int limit) {
		...
		while (true) {
			int find_pos = BryFind.FindFwd(src, dlm, pos);
			if (find_pos == BryFind.NotFound) break;
			if (found == idx) return BryLni.Mid(src, pos, find_pos);
			pos = find_pos + dlm_len;
			++found;
		}
		return found == idx ? BryLni.Mid(src, pos, src_len) : BryUtl.Empty;
	}
}	
