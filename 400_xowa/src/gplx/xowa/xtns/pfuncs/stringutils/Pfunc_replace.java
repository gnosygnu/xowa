...
public class Pfunc_replace extends Pf_func_base {
	...
	@Override public void Func_evaluate(BryWtr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		...
		if (BryUtl.IsNullOrEmpty(find)) find = AsciiByte.SpaceBry;	// NOTE: MW defaults empty finds to space (" "); note that leaving it as "" would cause Replace to loop infinitely
		byte[] repl = Pf_func_.Eval_arg_or(ctx, src, caller, self, self_args_len, 1, BryUtl.Empty);
		byte[] limit_bry = Pf_func_.Eval_arg_or(ctx, src, caller, self, self_args_len, 2, null);
		int limit = limit_bry == null ? IntUtl.MaxValue : BryUtl.ToIntOrNeg1(limit_bry);
		BryWtr tmp_bfr = ctx.Wiki().Utl__bfr_mkr().GetB128();
		try {
			byte[] rv = BryUtlByWtr.Replace(tmp_bfr, argx, find, repl, 0, argx.length, limit);
			bfr.Add(rv);
		}
