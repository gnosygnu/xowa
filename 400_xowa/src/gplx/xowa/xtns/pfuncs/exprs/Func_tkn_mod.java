...
class Func_tkn_mod extends Func_tkn_base {
	...
	@Override public boolean Calc_hook(Xop_ctx ctx, Pfunc_expr_shunter shunter, Val_stack val_stack) {
		...
		// must convert to long else issues with (39052000900/1) mod 100 which should be 0, not 47; JAVA does not fail int conversion, and instead converts to IntUtl.Max_value; EX: de.w:Quijano_(Piélagos)
		long rhs = ((GfoDecimal)val_stack.Pop()).ToLong();
		long lhs = ((GfoDecimal)val_stack.Pop()).ToLong();
		if (rhs == 0) {
			shunter.Err_set(ctx, Xol_msg_itm_.Id_pfunc_expr_division_by_zero);
			return false;
		}
		val_stack.Push(GfoDecimalUtl.NewByLong(lhs % rhs));
		return true;
	}
}
