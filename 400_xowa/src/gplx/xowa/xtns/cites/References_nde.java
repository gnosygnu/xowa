...
public class References_nde implements Xox_xnde, Mwh_atr_itm_owner1 {
	public byte[] Group() {return group;} public References_nde Group_(byte[] v) {group = v; return this;} private byte[] group = BryUtl.Empty;
	...
	public void Xtn_parse(Xowe_wiki wiki, Xop_ctx ctx, Xop_root_tkn root, byte[] src, Xop_xnde_tkn xnde) {
		...
		if (xnde.CloseMode() == Xop_xnde_tkn.CloseMode_pair) {	// "<references>", "</references>"; parse anything in between but only to pick up <ref> tags; discard everything else; DATE:2014-06-27
			...
			byte[] references_src = BryLni.Mid(src, itm_bgn, itm_end);
	...
	.Add_str_obj("group", ByteVal.New(References_nde.Xatr_id_group));
	public static final byte[] ESCAPED_TAG_BGN = BryUtl.NewA7("&lt;references>");
}
