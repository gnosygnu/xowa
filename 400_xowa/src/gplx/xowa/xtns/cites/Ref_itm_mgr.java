...
public class Ref_itm_mgr {
	...
	private final Ref_itm_grp grp_default = new Ref_itm_grp(BryUtl.Empty);
	...
	public Ref_itm_lst Lst_get(byte[] grp_name, int lst_idx) {
		return BryUtl.IsNullOrEmpty(grp_name) ? grp_default.Lsts_get_at(lst_idx) : ((Ref_itm_grp)grps.GetByOrNull(grp_name)).Lsts_get_at(lst_idx);	// NOTE: must be Bry_.Len_eq_0 else <references group=""/> not same as <references/>; DATE:2013-02-06
	}	
	...
	public Ref_itm_grp Grps_get(byte[] grp_name) {
		if (BryUtl.IsNullOrEmpty(grp_name)) return grp_default;
