...
class Gallery_parser_fxt {
	...
	public String[] Expd(String ttl, String caption, String alt, String link, int page)	{return new String[] {ttl, caption, alt, link, IntUtl.ToStr(page)};}
	public void Init_kwd_set(int kwd_id, String kwd_val) {
		wiki.Lang().Kwd_mgr().Get_or_new(kwd_id).Itms()[0].Val_(BryUtl.NewA7(kwd_val));
		parser.Init_by_wiki(wiki);
	}
	public void Test_parse(String raw, String[]... expd) {
		List_adp actl = List_adp_.New();
		byte[] src = BryUtl.NewA7(raw);
		parser.Parse_all(actl, Gallery_mgr_base_.New(Gallery_mgr_base_.Tid__traditional), new Gallery_xnde(), src, 0, src.length);
		GfoTstr.EqLines(StringUtl.AryFlatten(expd), StringUtl.AryFlatten(Xto_str_ary(src, actl)));
	}
	private String[][] Xto_str_ary(byte[] src, List_adp list) {
		int len = list.Len();
		String[][] rv = new String[len][];
		for (int i = 0; i < len; i++) {
			Gallery_itm itm = (Gallery_itm)list.GetAt(i);
			String[] ary = new String[5];
			rv[i] = ary;
			ary[0] = StringUtl.NewU8(itm.Ttl().Full_txt());
			...
			ary[1] =  caption == null ? null : StringUtl.NewU8(caption);
		}
		return rv;
	}
	private String Xto_str_ary_itm(byte[] src, int bgn, int end) {
		return bgn == BryFind.NotFound && end == BryFind.NotFound ? null : StringUtl.NewU8(src, bgn, end);
	}
}
