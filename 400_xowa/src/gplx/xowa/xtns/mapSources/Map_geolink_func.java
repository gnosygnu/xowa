...
public class Map_geolink_func extends Pf_func_base {
			...
			for (int i = 0; i < args_len; i++) {
				...
				if (key_tid_obj != null) {
					...
					switch (((ByteVal)key_tid_obj).Val()) {
						...
						case Key_tid_prec:		prec = BryUtl.ToIntOr(val, prec); break;
					}
				}
			}
	...
	private static final Xol_msg_itm tmp_msg_itm = new Xol_msg_itm(-1, BryUtl.Empty);
	private static byte[] Xto_coord(BryWtr bfr, Map_math math, boolean pass, byte[] dir, byte[] or) {
		return pass
			? bfr.AddDouble(MathUtl.AbsAsDouble(math.Dec())).AddByte(AsciiByte.Underline).Add(dir).ToBryAndClear()
			: or
			;
	}
	...
	private static byte[] Xto_dec(BryWtr bfr, Map_math math, boolean pass) {
		return pass
			? bfr.AddDouble(math.Dec()).ToBryAndClear()
			: Bry_arg_5_fail
			;
	}
	private static final byte[]
	  Bry_arg_0_fail = BryUtl.NewA7("0_N")
	, Bry_arg_1_fail = BryUtl.NewA7("0_E")
	, Bry_arg_5_fail = BryUtl.NewA7("0")
	;
/*
