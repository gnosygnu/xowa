...
import gplx.xowa.*;
import org.junit.*;
import gplx.xowa.apps.cfgs.*; import gplx.xowa.wikis.nss.*; import gplx.langs.htmls.*;
...
public class Dpl_xnde_tst {
	...
	@Test public void Err__bad_key_causes_out_of_bound() {	// PURPOSE: bad key causes out of bounds error; PAGE:de.n:Portal:Brandenburg DATE:2016-04-21
		...
		fxt.Test__html(StringUtl.ConcatLinesNlSkipLast
		( "&lt;DynamicPageList&gt;"
...
class Dpl_xnde_fxt {	
	...
	public void Clear() {
		next_id = 100;
		fxt.Reset();
		this.exec__raw = StringUtl.Empty;
		this.expd_warns = StringUtl.AryEmpty;
	...
	public void Init__create_ctg_pages(String ctg_ttl, Dpl_page_mok... pages) {
		Xoctg_catpage_ctg ctg = new Xoctg_catpage_ctg(1, BryUtl.NewU8(ctg_ttl));
		...
		for (int i = 0; i < pages_len; i++) {
			...
			Xoa_ttl ttl = fxt.Wiki().Ttl_parse(BryUtl.NewU8(ttl_str));
		...
		catpage_mgr.Cache__add(BryUtl.NewU8("Category:" + ctg_ttl), ctg);
	}
	...
	public String Make__html(String itm_html, String... pages) {
		bfr.AddStrA7("<ul>").AddByteNl();
		int pages_len = pages.length;
		for (int i = 0; i < pages_len; i++) {
			String page = pages[i];
			bfr.AddStrA7("  <li><a href=\"/wiki/").AddStrA7(page).AddStrA7("\" title=\"").AddStrA7(page).AddStrA7("\"");
			if (itm_html != null) bfr.AddStrA7(itm_html);
			bfr.AddStrA7(">").AddStrA7(page).AddStrA7("</a></li>").AddByteNl();
		}
		bfr.AddStrA7("</ul>").AddByteNl();
		return bfr.ToStrAndClear();
	}
	public void Exec__parse(String... raw_lines) {this.exec__raw = StringUtl.ConcatLinesNlSkipLast(raw_lines);}
