...
class Wdata_claims_parser_v2 {
	...
	private Wdata_claim_itm_core Parse_claim_itm(byte[] qid, Json_itm_nde nde, int pid) {
		...
		for (int i = 0; i < len; ++i) {
			...
			switch (tid) {
				case Wdata_dict_claim.Tid_mainsnak:			claim_itm = Parse_mainsnak(qid, Json_itm_nde.cast_(sub.Val()), pid); break;
				...
				case Wdata_dict_claim.Tid_qualifiers:		qualifiers = Parse_qualifiers(qid, Json_itm_nde.cast_(sub.Val())); break;
	...
	public Wdata_references_grp[] Parse_references(byte[] qid, Json_itm_ary owner) {
		int len = owner.Subs_len();
		Wdata_references_grp[] rv = new Wdata_references_grp[len];
		for (int i = 0; i < len; ++i) {
			Json_itm_nde grp_nde = Json_itm_nde.cast_(owner.Subs_get_at(i));
			rv[i] = Parse_references_grp(qid, grp_nde);
		}
		return rv;
	}
	private Wdata_references_grp Parse_references_grp(byte[] qid, Json_itm_nde owner) {
		int len = owner.Subs_len();
		Hash_adp_bry dict = Wdata_dict_reference.Dict;
		Wdata_claim_grp_list snaks = null; int[] snaks_order = null;
		for (int i = 0; i < len; ++i) {
			Json_itm_kv sub = Json_itm_kv.cast_(owner.Subs_get_at(i));
			byte tid = Wdata_dict_utl.Get_tid_or_invalid(qid, dict, sub.Key().Data_bry()); if (tid == Wdata_dict_utl.Tid_invalid) continue;
			switch (tid) {
				case Wdata_dict_reference.Tid_hash:	break;	// ignore: "b923b0d68beb300866b87ead39f61e63ec30d8af"
				case Wdata_dict_reference.Tid_snaks:			snaks = Parse_qualifiers(qid, Json_itm_nde.cast_(sub.Val())); break;
	...
	public int[] Parse_pid_order(Json_itm_ary ary) {
		...
		int[] rv = new int[len];
		for (int i = 0; i < len; ++i) {
			Json_itm pid_itm = ary.Subs_get_at(i);
	...
	public Wdata_claim_itm_core Parse_datavalue(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		int len = nde.Subs_len();
		...
		switch (value_tid) {
			...
			case Wdata_dict_val_tid.Tid_entity:				return Parse_datavalue_entity				(qid, pid, snak_tid, Json_itm_nde.cast_(value_itm));
			case Wdata_dict_val_tid.Tid_time:				return Parse_datavalue_time					(qid, pid, snak_tid, Json_itm_nde.cast_(value_itm));
			case Wdata_dict_val_tid.Tid_quantity:			return Parse_datavalue_quantity				(qid, pid, snak_tid, Json_itm_nde.cast_(value_itm));
			case Wdata_dict_val_tid.Tid_globecoordinate:	return Parse_datavalue_globecoordinate		(qid, pid, snak_tid, Json_itm_nde.cast_(value_itm));
			case Wdata_dict_val_tid.Tid_monolingualtext:	return Parse_datavalue_monolingualtext		(qid, pid, snak_tid, Json_itm_nde.cast_(value_itm));
		}
		throw Exc_.new_("unknown val_tid", "val_tid", value_tid);
	}
	private Wdata_claim_itm_entity Parse_datavalue_entity(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		...
		if (entity_id_bry == null) throw Exc_.new_("pid is invalid entity", "pid", pid);
	...
	private Wdata_claim_itm_monolingualtext Parse_datavalue_monolingualtext(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		...
		if (lang == null || text == null) throw Exc_.new_("pid is invalid monolingualtext", "pid", pid);
		return new Wdata_claim_itm_monolingualtext(pid, snak_tid, lang, text);
	}
	private Wdata_claim_itm_globecoordinate Parse_datavalue_globecoordinate(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		...
		if (lat == null || lng == null) throw Exc_.new_("pid is invalid globecoordinate", "pid", pid);
		return new Wdata_claim_itm_globecoordinate(pid, snak_tid, lat, lng, alt, prc, glb);
	}
	private Wdata_claim_itm_quantity Parse_datavalue_quantity(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		...
		if (amount == null) throw Exc_.new_("pid is invalid quantity", "pid", pid);
		return new Wdata_claim_itm_quantity(pid, snak_tid, amount, unit, ubound, lbound);
	}
	private Wdata_claim_itm_time Parse_datavalue_time(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {
		...
		if (time == null) throw Exc_.new_("pid is invalid time", "pid", pid);
	...
	private static int Parse_pid(byte[] pid_bry) {
		int rv = Bry_.Xto_int_or(pid_bry, 1, pid_bry.length, -1); if (rv == -1) throw Exc_.new_("invalid pid", "pid", String_.new_u8(pid_bry));
		return rv;
	}
}
