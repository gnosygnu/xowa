...
import gplx.core.primitives.*; import gplx.json.*; import gplx.xowa.xtns.wdatas.core.*;
public class Wdata_doc_parser_v1 implements Wdata_doc_parser {
	...
	public byte[] Parse_qid(Json_doc doc) {
		try {
			...
			switch (kv_val.Tid()) {
				...
				default:
					throw Exc_.new_unhandled(kv_val.Tid());
			}
			...
			for (int i = 0; i < list_len; ++i) {
				...
				switch (data_val.Tid()) {
					...
					case Json_itm_.Tid_nde:			// EX: 'en':{'0:en_val_1','1:en_val_2'}; PAGE:wd.q:621080 DATE:2014-09-21
						...
						break;
					default: throw Exc_.new_unhandled(data_val.Tid());
				}
				Wdata_alias_itm itm		= new Wdata_alias_itm(lang_bry, vals_bry_ary);
				rv.Add(lang_bry, itm);
			}
			return rv;
		} catch (Exception e) {throw Exc_.new_exc(e, "xo", "failed to parse aliases", "qid", String_.new_u8(qid));}
	}
	...
	public Wdata_claim_itm_base Parse_claims_data(byte[] qid, int pid, byte snak_tid, Json_itm_nde nde) {throw Exc_.new_unimplemented();}
	...
	private Wdata_claim_itm_core Make_claim_itm(byte[] src, Json_itm_nde prop_nde) {
		...
		for (int i = 0; i < len; i++) {
			...
			switch (bv.Val()) {
				...
				case Prop_tid_refs:
					break;
				default:				throw Exc_.new_unhandled(bv.Val());
			}
		}
		return rv;
	}
	...
	private Wdata_claim_itm_core Make_itm(int pid, byte snak_tid, byte val_tid, Json_itm_ary ary) {
		switch (val_tid) {
			...
			default: {throw Exc_.new_unhandled(val_tid);}
		}		
	}
	...
	public Wdata_claim_grp_list Parse_qualifiers(byte[] qid, Json_itm_nde nde)		{throw Exc_.new_unimplemented();}
	public Wdata_references_grp[] Parse_references(byte[] qid, Json_itm_ary owner)	{throw Exc_.new_unimplemented();}
	public int[] Parse_pid_order(byte[] qid, Json_itm_ary ary) {throw Exc_.new_unimplemented();}
