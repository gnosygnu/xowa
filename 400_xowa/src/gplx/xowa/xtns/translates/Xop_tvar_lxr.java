...
public class Xop_tvar_lxr implements Xop_lxr {
	...
	private static final byte[] Hook_bgn = BryUtl.NewA7("<tvar|"), Close_nde = BryUtl.NewA7("</>");
	public int Make_tkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {
		int rhs_end = BryFind.FindFwd(src, AsciiByte.Gt, cur_pos); if (rhs_end == BryFind.NotFound) return ctx.Lxr_make_txt_(cur_pos);
		int lhs_bgn = BryFind.FindFwd(src, Close_nde    , rhs_end); if (lhs_bgn == BryFind.NotFound) return ctx.Lxr_make_txt_(cur_pos);
		byte[] body = BryLni.Mid(src, rhs_end + IntUtl.Offset1, lhs_bgn);
		...
		ctx.Subs_add(root, tkn_mkr.Tvar(bgn_pos, end_pos, cur_pos, rhs_end, rhs_end + IntUtl.Offset1, lhs_bgn, body));
		return end_pos;
	}
