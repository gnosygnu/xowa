...
public class Xop_languages_xnde implements Xox_xnde {
	...
	private Xoa_ttl Root_ttl_of(Xowe_wiki wiki, Xoa_ttl ttl) {
		...
		int slash_pos = BryFind.FindBwd(page_bry, Xoa_ttl.Subpage_spr);
		if (slash_pos == BryFind.NotFound) return ttl;
		byte[] root_bry = BryLni.Mid(page_bry, 0, slash_pos);
	...
	private List_adp Find_lang_pages(Xop_ctx ctx, Xowe_wiki wiki) {
		...
		IntRef rslt_count = IntRef.New(0);
		Xow_ns page_ns = root_ttl.Ns();
		wiki.Db_mgr().Load_mgr().Load_ttls_for_all_pages(Cancelable_.Never, rslts, null, null, rslt_count, page_ns, root_ttl.Page_db(), IntUtl.MaxValue, 0, IntUtl.MaxValue, true, false);
		...
		for (int i = 0; i < len; i++) {
			Xowd_page_itm page = (Xowd_page_itm)rslts.GetAt(i);
			byte[] page_ttl_bry = page.Ttl_page_db();
			int page_ttl_bry_len = page_ttl_bry.length;
			if 		(BryLni.Eq(root_ttl_bry, page_ttl_bry)) continue; 	// ignore self; EX: "page"
			if 		(lang_bgn < page_ttl_bry_len 							// guard against out of bounds
				...
				byte[] lang_key = BryLni.Mid(page_ttl_bry, lang_bgn, page_ttl_bry_len);
				if (BryLni.Eq(lang_key, Xol_lang_itm_.Key_en))			// lang is english; mark english found;
	...
	public void Xtn_write(BryWtr bfr, Xoae_app app, Xop_ctx ctx, Xoh_html_wtr html_wtr, Xoh_wtr_ctx hctx, Xoae_page wpg, Xop_xnde_tkn xnde, byte[] src) {
		...
		fmtr_all.BldToBfrMany(bfr, "Other languages", fmtr_mgr_itms);
	}
	private static final Xop_languages_fmtr fmtr_mgr_itms = new Xop_languages_fmtr();
	public static final BryFmtr fmtr_all = BryFmtr.New(StringUtl.ConcatLinesNl
	(	"<table>"
	,	"  <tbody>"
	,	"    <tr valign=\"top\">"
	...
	,	fmtr_itm_basic = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	(	""
	,	"         <a href=\"~{anchor_href}\" title=\"~{anchor_title}\">~{anchor_text}</a>&#160;•"		
	), "anchor_href", "anchor_title", "anchor_text")
	,	fmtr_itm_english = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	(	""
	,	"         <a href=\"~{anchor_href}\" title=\"~{anchor_title}\"><span class=\"mw-pt-languages-ui\">~{anchor_text}</span></a>&#160;•"
	), "anchor_href", "anchor_title", "anchor_text")
	,	fmtr_itm_selected = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	(	""
...
class Xop_languages_fmtr implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = 0; i < len; i++) {
			Xol_lang_stub lang = (Xol_lang_stub)langs.GetAt(i);
			byte[] lang_key = lang.Key();
			boolean lang_is_en = BryLni.Eq(lang_key, Xol_lang_itm_.Key_en);
			byte[] lang_ttl_bry = lang_is_en ? root_ttl_bry : BryUtl.AddWithDlm(Xoa_ttl.Subpage_spr, root_ttl_bry, lang_key);
			...
			BryFmtr fmtr = null;
			if		(BryLni.Eq(lang_key, Xol_lang_itm_.Key_en)) 	fmtr = Xop_languages_xnde.fmtr_itm_english;
			else if	(BryLni.Eq(lang_key, cur_lang))			fmtr = Xop_languages_xnde.fmtr_itm_selected;
			else 												fmtr = Xop_languages_xnde.fmtr_itm_basic;
			fmtr.BldToBfrMany(bfr, lang_href, lang_title, lang.Canonical_name());
		}
	}
}
