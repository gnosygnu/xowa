			...
			while (line_bgn < src_end) {																			// iterate over each \n
				...
				switch (src[line_bgn]) {
					case AsciiByte.Space:																			// "\n\s" -> "\n&#160;"
						int space_end = Bry_find_.Find_fwd_while(src, line_bgn, src_end, AsciiByte.Space);
					...
					case AsciiByte.Colon: {																		// "\n:" -> <span class='mw-poem-indented' style='display: inline-block; margin-left: #em;'>
						int colon_end = Bry_find_.Find_fwd_while(src, line_bgn, src_end, AsciiByte.Colon);
				...
				int line_end = Bry_find_.Find_fwd(src, AsciiByte.Nl, line_bgn, src_end);						// find end "\n"
