...
class Lst_pfunc_lstx_ {
	public static void Sect_exclude(BryWtr bfr, Lst_section_nde_mgr sec_mgr, byte[] src, byte[] sect_exclude, byte[] sect_replace) {
		...
		for (int i = 0; i < sections_len; i++) {
			...
			if		(section_tid == Lst_section_nde.Xatr_bgn && BryLni.Eq(section_key, sect_exclude)) {	// exclude section found
				bfr.AddMid(src, bgn_pos, section.Xnde().Tag_open_bgn());									// write everything from bgn_pos up to exclude
			}
			else if (section_tid == Lst_section_nde.Xatr_end && BryLni.Eq(section_key, sect_exclude)) {	// exclude end found
