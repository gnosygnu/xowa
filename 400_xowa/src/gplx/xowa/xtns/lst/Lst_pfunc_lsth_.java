...
class Lst_pfunc_lsth_ {
	public static void Hdr_include(BryWtr bfr, byte[] src, Xopg_toc_mgr toc_mgr, byte[] lhs_hdr, byte[] rhs_hdr) {// REF.MW:LabeledSectionTransclusion.class.php|pfuncIncludeHeading; MW does regex on text; XO uses section_itms
		// get <hdr> idxs
		int len = toc_mgr.Len();
		...
		bfr.AddMid(src, snip_bgn, snip_end);
	}
	private static int Match_or_neg1(Xopg_toc_mgr toc_mgr, int hdrs_len, byte[] src, byte[] match, int hdrs_bgn) {
		for (int i = hdrs_bgn; i < hdrs_len; ++i) {
			...
			int txt_end = BryFind.FindFwd(src, Bry__hdr_end, txt_bgn);				// find "=\n"
			txt_end = BryFind.FindBwdSkip(src, txt_end, txt_bgn, AsciiByte.Eq);	// skip bwd to get to pos before 1st "="; EX: "===\n" -> find "=="

			// remove ws
			txt_bgn = BryFind.FindFwdWhileWs(src, txt_bgn, txt_end);
			txt_end = BryFind.FindBwdSkipWs(src, txt_end, txt_bgn);
			if (BryLni.Eq(src, txt_bgn, txt_end, match)) return i;
		}
		return -1;
	}
	private static final byte[] Bry__hdr_end = BryUtl.NewA7("=\n");
}
