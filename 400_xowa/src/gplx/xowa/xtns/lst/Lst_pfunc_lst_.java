...
public class Lst_pfunc_lst_ {
	...
	public static void Sect_include(BryWtr bfr, Lst_section_nde_mgr sec_mgr, byte[] src, byte[] lst_bgn, byte[] lst_end) {
		...
		if		(BryUtl.IsNullOrEmpty(lst_end))
			include_mode = Include_to_eos;
		else if (BryUtl.IsNullOrEmpty(lst_bgn))
		...
		for (int i = 0; i < sections_len; i++) {
			...
			if		(section_tid == Lst_section_nde.Xatr_bgn && BryLni.Eq(section_key, lst_bgn)) {
			...
			else if (section_tid == Lst_section_nde.Xatr_end && BryLni.Eq(section_key, lst_end)) {
