...
public class Indicator_html_bldr implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		bldr_itm.Init(list);
		fmtr_grp.BldToBfrMany(bfr, bldr_itm);
	}
	...
	public void Deserialise(Xow_wiki wiki, Xoh_page hpg, byte[] data) {
		// exit if empty
		if (BryUtl.IsNullOrEmpty(data)) return;
...
class Indicator_html_bldr_itm implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = list_len - 1; i > -1; --i) {	// reverse order
			Indicator_xnde xnde = (Indicator_xnde)list.GetAt(i);
			fmtr_itm.BldToBfrMany(bfr, xnde.Name(), xnde.Html());
		}
	}
	private static final BryFmtr fmtr_itm = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	( ""
