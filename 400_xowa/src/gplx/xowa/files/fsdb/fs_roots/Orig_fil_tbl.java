...
class Orig_fil_tbl_sql implements Orig_fil_tbl {
	...
	public void Ctor(Db_provider provider, boolean created) {
	...
	private Db_stmt Insert_stmt() {return Db_stmt_.new_insert_(provider, Tbl_name, Fld_fil_uid, Fld_fil_name, Fld_fil_ext_id, Fld_fil_w, Fld_fil_h, Fld_fil_dir_url);}
	public void Insert(Orig_fil_itm fil_itm) {
		if (stmt_insert == null) stmt_insert = Insert_stmt();
		stmt_insert.Clear()
		.Val_int_(fil_itm.Fil_uid())
		.Val_str_by_bry_(fil_itm.Fil_name())
		.Val_int_(fil_itm.Fil_ext_id())
		.Val_int_(fil_itm.Fil_w())
		.Val_int_(fil_itm.Fil_h())
		.Val_str_by_bry_(fil_itm.Fil_dir_url())
		.Exec_insert();
	}	
	public void Rls() {
		stmt_select.Rls();
	...
	private void Create_table(Db_provider p) {
