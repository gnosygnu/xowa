...
public class Xof_orig_wkr__mock implements Xof_orig_wkr {
	...
	public Xof_orig_itm		Find_as_itm(byte[] ttl, int list_idx, int list_len) {
		return (Xof_orig_itm)hash.GetByOrNull(ttl); // new Xof_orig_itm((byte)meta_itm.Vrtl_repo(), ttl, Xof_ext_.new_by_ttl_(ttl).Id(), meta_itm.Orig_w(), meta_itm.Orig_h(), meta_itm.Ptr_ttl());
	}
