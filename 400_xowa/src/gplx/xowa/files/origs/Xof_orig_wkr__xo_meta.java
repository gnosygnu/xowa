...
public class Xof_orig_wkr__xo_meta implements Xof_orig_wkr {
	private final Io_url wiki_meta_dir; private final byte dir_spr_byte;  private final BryWtr url_bfr = BryWtr.NewWithSize(255);
	...
	public Xof_orig_itm		Find_as_itm(byte[] ttl, int list_idx, int list_len) {
		...
		url_bfr.Add(wiki_meta_dir.RawBry())					// /xowa/file/#meta/simple.wikipedia.org/
			.AddByte(md5[0]).AddByte(dir_spr_byte)		// 0/
			.AddByte(md5[1]).AddByte(dir_spr_byte)		// 6/
			.AddMid(md5, 0, 3).AddStrA7(".csv")			// 061.csv
			;
		Io_url meta_url = Io_url_.new_fil_(url_bfr.ToStrAndClear());
