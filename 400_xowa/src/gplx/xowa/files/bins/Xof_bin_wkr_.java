...
public class Xof_bin_wkr_ {
	...
	public static final byte Tid_null = ByteUtl.MaxValue127, Tid_noop = 1, Tid_not_found = 2
	...
	public static byte X_key_to_tid(String key) {
		if		(StringUtl.Eq(key, Key_fsdb_wiki))	return Tid_fsdb_xowa;
		else if	(StringUtl.Eq(key, Key_http_wmf))		return Tid_http_wmf;
		else if	(StringUtl.Eq(key, Key_fsys_wmf))		return Tid_fsys_wmf;
		else if	(StringUtl.Eq(key, Key_fsys_xowa))	return Tid_fsys_xowa;
