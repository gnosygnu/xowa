...
public class Xof_img_wkr_resize_img_imageMagick implements Xof_img_wkr_resize_img {
	...
	public boolean Resize_exec(Io_url src, Io_url trg, int trg_w, int trg_h, int ext_id, StringRef rslt_val) {
		...
		cmd.Prog_fmt_(StringUtl.Replace(wmf_mgr.Download_wkr().Download_xrg().Prog_fmt_hdr(), "~", "~~") + " converting: ~{process_seconds} second(s); ~{process_exe_name} ~{process_exe_args}");
		cmd.Run(src.Raw(), trg.Raw(), trg_w, trg_h);
		rslt_val.ValSet(cmd.Rslt_out());
