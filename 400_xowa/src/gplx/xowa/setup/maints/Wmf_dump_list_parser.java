...
public class Wmf_dump_list_parser {
	...
	private boolean Parse_href(Wmf_dump_itm itm, byte[] src, int a_pos) {	// EX: http://dumps.wikimedia.org/enwiki/20130807
		...
		int href_bgn_pos = Bry_finder.Find_fwd(src, Byte_ascii.Quote, href_pos + Find_href.length);
		int href_end_pos = Bry_finder.Find_fwd(src, Byte_ascii.Quote, href_bgn_pos + 1); if (href_end_pos == Bry_.NotFound) return false;
	...
	private byte[] Parse_status_msg(byte[] src, int a_pos) {
		...
		int bgn = Bry_finder.Find_fwd(src, Byte_ascii.Gt, span_pos + Find_span_bgn.length); if (bgn == Bry_.NotFound) return null;
		int end = Bry_finder.Find_fwd(src, Find_span_end, bgn); if (end == Bry_.NotFound) return null;
