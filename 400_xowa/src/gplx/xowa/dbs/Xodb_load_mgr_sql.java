...
import gplx.xowa.apps.gfss.*; import gplx.xowa.bldrs.cmds.ctgs.*; import gplx.xowa.ctgs.*; import gplx.xowa.specials.search.*; import gplx.xowa.wikis.data.tbls.*;
...
public class Xodb_load_mgr_sql implements Xodb_load_mgr {
	...
	private void Load_init_cfg(Xowe_wiki wiki) {
		...
		try {
			...
			for (int i = 0; i < len; ++i) {
				...
				gfs_mgr.Build_prop_set(bfr, Bry_.new_u8(cfg_itm.Key()), Bry_.new_u8(cfg_itm.To_str_or("")));
			}
	...
	public Xowd_page_itm[] Load_ctg_list(byte[][] ctg_ttls) {
		...
		Ordered_hash hash2 = Ordered_hash_.new_();
		for (int i = 0; i < len; i++) {
