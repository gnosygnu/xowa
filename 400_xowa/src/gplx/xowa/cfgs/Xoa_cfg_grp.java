...
public class Xoa_cfg_grp {		
	public Xoa_cfg_grp(Xoa_cfg_mgr mgr, Xoa_cfg_grp_tid tid, byte[] key_bry) {
		this.mgr = mgr; this.tid = tid; this.key_bry = key_bry; this.key_str = String_.new_utf8_(key_bry);
	} 	Xoa_cfg_mgr mgr; OrderedHash hash = OrderedHash_.new_bry_();
	...
	public Xoa_cfg_itm Get_itm_or_null(byte[] itm_key) {
		return (Xoa_cfg_itm)hash.Fetch(itm_key);
	}
	public Xoa_cfg_itm Get_itm_or_make(byte[] itm_key) {
		Xoa_cfg_itm rv = (Xoa_cfg_itm)hash.Fetch(itm_key);
		if (rv == null) {
		...
		if (rv == null) { 		// match by domain failed; try type
			...
			if (rv == null)		// match by type failed; try all
				...
				if (rv == null)		// match by type failed; try app
					rv = (Xoa_cfg_itm)hash.Fetch(Xoa_cfg_grp_tid.Key_app_bry);	
		}
		return rv;
	}
	public void Db_customized_n_() {
		int len = hash.Count();
		for (int i = 0; i < len; i++) {
			Xoa_cfg_itm itm = (Xoa_cfg_itm)hash.FetchAt(i);
