...
public class Xop_lnki_wkr_ {
	...
	public static boolean Parse_ttl(Xop_ctx ctx, byte[] src, Xop_lnki_tkn lnki, int pipe_pos) {
		...
		ttl_bgn = BryFind.FindFwdWhile(src, ttl_bgn, pipe_pos, AsciiByte.Space);		// remove \s from bgn
		int ttl_end = BryFind.FindBwdWhile(src, pipe_pos, ttl_bgn, AsciiByte.Space);	// remove \s from end
	...
	public static boolean Parse_ttl(Xop_ctx ctx, byte[] src, Xop_lnki_tkn lnki, int ttl_bgn, int ttl_end) {
		byte[] ttl_bry = BryLni.Mid(src, ttl_bgn, ttl_end);
		...
		if (page_ttl.Ns().Subpages_enabled()
			...
			IntRef rel2abs_tid = ctx.Tmp_mgr().Pfunc_rel2abs().ValSetZero();
			...
			lnki.Subpage_slash_at_end_(BryUtl.GetAtEnd(ttl_bry) == AsciiByte.Slash);
			ttl_bry = new_bry;
			tmp_bfr.MkrRls();
		}
	...
	private static Xoa_ttl Adj_ttl_for_file(Xowe_wiki wiki, Xop_ctx ctx, Xoa_ttl ttl, byte[] ttl_bry) {	// NOTE: remove the xwiki part; EX: [[en:File:A.png]] -> [[File:A.png]]
		...
		byte[] ttl_in_xwiki_bry = BryLni.Mid(ttl_bry, xwiki_bry_len + 1, ttl_bry_len); // +1 to position after xwiki :; EX: [[en:File:A.png]]; +1 to put after ":" at "F"
	...
	public static void Page_parse(Xop_ctx ctx, byte[] src, Gfo_number_parser number_parser, Xop_lnki_tkn lnki, Arg_nde_tkn arg) {
		...
		byte[] val_bry = BryUtl.Trim(src, val_tkn_bgn, val_tkn_end);	// some tkns have trailing space; EX.WWI: [[File:Bombers of WW1.ogg|thumb |thumbtime=3]]
	...
	public static byte[] Val_extract(byte[] src, Arg_nde_tkn arg) {
		int val_tkn_bgn = arg.Val_tkn().Src_bgn(), val_tkn_end = arg.Val_tkn().Src_end();
		return BryUtl.Trim(src, val_tkn_bgn, val_tkn_end);	// trim trailing space
	}
	public static void Thumbtime_parse(Xop_ctx ctx, byte[] src, Gfo_number_parser number_parser, Xop_lnki_tkn lnki, Arg_nde_tkn arg) {
		int val_tkn_bgn = arg.Val_tkn().Src_bgn(), val_tkn_end = arg.Val_tkn().Src_end();
		long fracs = GfoTimeSpanUtl.ParseToFracs(src, val_tkn_bgn, val_tkn_end, false);
		if (fracs == GfoTimeSpanUtl.ParseNull) {
			ctx.Msg_log().Add_itm_none(Xop_lnki_log.Upright_val_is_invalid, src, val_tkn_bgn, val_tkn_end);
		}
		else
			lnki.Time_(fracs / GfoTimeSpanUtl.RatioFracsToSecs);
	}
	...
	public static void Write_lnki(BryWtr bfr, Xoa_ttl ttl, boolean literal) {
		bfr.Add(Xop_tkn_.Lnki_bgn);
		if (literal) bfr.AddByte(AsciiByte.Colon);
		bfr.Add(ttl.Full_db());
