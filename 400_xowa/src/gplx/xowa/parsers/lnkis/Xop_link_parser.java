...
public class Xop_link_parser {
	...
	public byte[] Parse(BryWtr tmp_bfr, Xoa_url tmp_url, Xowe_wiki wiki, byte[] raw, byte[] or) {
		...
		switch (tmp_url.Protocol_tid()) {
			case Gfo_protocol_itm.Tid_http: case Gfo_protocol_itm.Tid_https:	// "http:" or "https:"; check if to offline wiki and redirect 
				...
				if (	!tmp_url.Wiki_is_missing()							// https://www.a.org and others will be marked "missing" by Xow_url_parser
						&&(	BryLni.Eq(wiki_bry, wiki.Domain_bry())			// link is to this wiki; check if alias
			...
			case Gfo_protocol_itm.Tid_file:		// "file:///" or "File:A.png"
				...
				if (proto_len + 1 < raw_len && raw[proto_len + 1] == AsciiByte.Slash) {	// next char is slash, assume xfer_itm refers to protocol; EX: file:///C/A.png
					int slash_pos = BryFind.FindBwd(raw, AsciiByte.Slash);
					if (slash_pos != BryFind.NotFound)	// set xowa_title to file_name; TODO_OLD: call Xoa_url.build; note that this will fail sometimes when (a) xfer_itm is very long (File:ReallyLongName will be shortened to 128 chars) or (b) xfer_itm has invalid windows characters (EX:File:a"b"c.jpg)
						html_xowa_ttl = BryLni.Mid(raw, slash_pos + AsciiByte.Len1, raw.length);
				}
				else // next char is not slash; assume xfer_itm refers to ns; EX:File:A.png
					raw = tmp_bfr.Add(Xoh_href_.Bry__wiki).Add(raw).ToBryAndClear();
				break;
			default:	// is page only; EX: Abc
				if (BryUtl.IsNullOrEmpty(raw))		// empty link should not create anchor; EX:[[File:A.png|link=|abc]]; [[File:Loudspeaker.svg|11px|link=|alt=play]]; PAGE:en.w:List_of_counties_in_New_York; DATE:2016-01-10;
					raw = BryUtl.Empty;
				else {
					if (raw[0] == AsciiByte.Colon) raw = BryLni.Mid(raw, 1, raw.length);	// ignore initial colon; EX: [[:commons:A.png]]
	...
	private static boolean Parse__ttl(BryWtr tmp_bfr, Xowe_wiki wiki, byte[] wiki_bry, byte[] page_bry) {
		...
		if (	xwiki_itm != null					// ttl is xwiki; EX:[[File:A.png|link=wikt:A]]
			...
			page_bry = BryUtl.Add(page_bry, xwiki_wiki.Props().Main_page());	// append Main_Page to ttl; EX:"wikt:" + "Wikipedia:Main_Page" -> "wikt:Wikipedia:Main_Page"
		...
		if (page_ttl_is_valid) {						// xwiki; need to define wiki / page
			if (xwiki_itm != null) {					// is alias; set wiki, page
				...
				page_bry = BryLni.Mid(page_bry, xwiki_itm.Key_bry().length + 1, page_bry.length);	// +1 to skip ":"
			}
		...
		if (BryLni.Eq(wiki_bry, wiki.Domain_bry())) {		// NOTE: check against wiki.Key_bry() again; EX: in en_wiki, and http://commons.wikimedia.org/wiki/w:A
