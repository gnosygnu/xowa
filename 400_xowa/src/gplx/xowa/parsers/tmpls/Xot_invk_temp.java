...
public class Xot_invk_temp implements Xot_invk {
	...
	private Hash_adp arg_idx_hash; private IntRef arg_idx_ref;
	...
	public byte[] Frame_ttl() {return frame_ttl;} public void Frame_ttl_(byte[] v) {frame_ttl = v;} private byte[] frame_ttl = BryUtl.Empty;	// NOTE: set frame_ttl to non-null value; PAGE:en.w:Constantine_the_Great {{Christianity}}; DATE:2014-06-26
	...
	public Arg_nde_tkn Args_eval_by_idx(byte[] src, int idx) {			// NOTE: idx is base0
		return arg_idx_hash == null										// only true if no args, or all args are keys; EX: {{A|b=1|c=2}}
			? null
			: (Arg_nde_tkn)arg_idx_hash.GetByOrNull(arg_idx_ref.ValSet(idx));	// lookup int in hash; needed b/c multiple identical keys should retrieve last, not first; EX: {{A|1=a|1=b}}; PAGE:el.d:ἔχω DATE:2014-07-23
	}
	public Arg_nde_tkn Args_get_by_idx(int i) {return list == null ? null : (Arg_nde_tkn)list.GetAt(i);}
	...
	public void Args_add_by_key(byte[] key, Arg_nde_tkn arg) {
		...
		int key_as_int = BryUtl.ToIntOr(key, IntUtl.MinValue);
		if (key_as_int != IntUtl.MinValue)						// key is int; add it to arg_idx_hash; EX:{{A|1=a}} vs {{A|a}}; DATE:2014-07-23
	...
	private void Arg_idx_hash_add(int int_key, Arg_nde_tkn arg) {
		if (arg_idx_hash == null) {
			...
			arg_idx_ref = IntRef.NewNeg1();
		}
		arg_idx_hash.AddIfDupeUseNth(IntRef.New(int_key), arg);	// Add_if_dupe_use_nth to keep latest version; needed for {{A|1=a|1=b}} DATE:2014-07-23
	}
