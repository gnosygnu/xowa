...
public class Xot_defn_tmpl implements Xot_defn {
	...
	byte[] Extract_onlyinclude(byte[] src, BryBfrMkr bfr_mkr) {
		BryWtr bfr = bfr_mkr.GetM001();
		int pos = 0;
		int src_len = src.length;
		while (true) {
			int find_bgn = BryFind.FindFwd(src, Bry_onlyinclude_bgn, pos, src_len);
			if (find_bgn == BryFind.NotFound) {
				break;
			}
			int find_bgn_lhs = find_bgn + Bry_onlyinclude_bgn_len;
			int find_end = BryFind.FindFwd(src, Bry_onlyinclude_end, find_bgn_lhs, src_len);
			if (find_end == BryFind.NotFound) {
				break;
			}
			bfr.AddMid(src, find_bgn_lhs, find_end);
			pos = find_end + Bry_onlyinclude_end_len;
		}
		return bfr.ToBryAndRls();
	}
	private static final byte[] Bry_onlyinclude_bgn = BryUtl.NewA7("<onlyinclude>"), Bry_onlyinclude_end = BryUtl.NewA7("</onlyinclude>");
	...
	public boolean Tmpl_evaluate(Xop_ctx ctx, Xot_invk caller, BryWtr bfr) {
		if (root == null) Parse_tmpl(ctx);
		Xoae_page page = ctx.Page();
		if (!ctx.Wiki().Parser_mgr().Tmpl_stack_add(full_name)) {
			bfr.AddStrA7("<!-- template loop detected:" + gplx.langs.htmls.Gfh_utl.Escape_html_as_str(StringUtl.NewU8(name)) + " -->");
		...
		if (onlyInclude_exists) {
			...
			if (!onlyinclude_parsed) {
				...
				Xot_defn_tmpl tmpl = wiki.Parser_mgr().Main().Parse_text_to_defn_obj(new_ctx, new_ctx.Tkn_mkr(), wiki.Ns_mgr().Ns_template(), BryUtl.Empty, new_data);
	...
	public Xot_defn Clone(int id, byte[] name) {throw ErrUtl.NewUnimplemented();}
