		...
		if (find_tkn != null) {	// NOTE: find_tkn defaults to null
			...
			if (find_tkn_static) {	// subs_are_static, so extract idx/key; EX: {{{a b}}} will have 3 subs which are all static; {{{a{{{1}}}b}}} will be dynamic
				...
				if (find_tkn_end - find_tkn_bgn > 0) {	// NOTE: handles empty find_tkns; EX: {{{|safesubst:}}}
					prm_idx = BryUtl.ToIntOr(src, find_tkn_bgn, find_tkn_end, -1);			// parse as number first; note that bgn,end should not include ws; EX: " 1 " will fail
					if (prm_idx == -1) prm_key = BryLni.Mid(src, find_tkn_bgn, find_tkn_end);// not a number; parse as key
				}
			}
		}
		if (dflt_tkn != null) dflt_tkn.Tmpl_compile(ctx, src, prep_data);
	}
	@Override public boolean Tmpl_evaluate(Xop_ctx ctx, byte[] src, Xot_invk caller, BryWtr bfr) {
		if (!find_tkn_static) {
			int subs_len = find_tkn.Subs_len();
			BryWtr find_bfr = BryWtr.New();
			for (int i = 0; i < subs_len; i++)
				find_tkn.Subs_get(i).Tmpl_evaluate(ctx, src, caller, find_bfr);
			prm_idx = BryUtl.ToIntOrTrimWs(find_bfr.Bry(), 0, find_bfr.Len(), -1);	// parse as number first; NOTE: trim needed to transform "{{{ 1 }}}" to "1"; it.w:Portale:Giochi_da_tavolo; DATE:2014-02-09
			if (prm_idx == -1)
				prm_key = find_bfr.ToBryAndClearAndTrim();									// not a number; parse as key; NOTE: must trim; PAGE:en.w:William Shakespeare; {{Relatebardtree}}
		}
		Arg_nde_tkn arg_nde = null;
		if (prm_idx == -1) {	// prm is key; EX: "{{{key1}}}"
			if (prm_key != BryUtl.Empty)	// NOTE: handles empty find_tkns; EX: {{{|safesubst:}}}
...
//				if (prm_idx > invk_args_len) {Tmpl_write_missing(ctx, src, caller, bfr); return true;}
		...
		if (arg_val.Itm_static() == BoolUtl.YByte)
			bfr.AddMid(src, arg_val.Dat_bgn(), arg_val.Dat_end());
			...
			BryWtr arg_val_bfr = BryWtr.New();
			arg_val.Tmpl_evaluate(ctx, src, caller, arg_val_bfr);
			bfr.AddBfrAndClear(arg_val_bfr);
		}
		return true;
	}
	private void Tmpl_write_missing(Xop_ctx ctx, byte[] src, Xot_invk caller, BryWtr bfr) {
	...
	int prm_idx = -1; byte[] prm_key = BryUtl.Empty; boolean find_tkn_static = true;
