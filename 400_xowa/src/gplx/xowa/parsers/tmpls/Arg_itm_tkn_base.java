...
public class Arg_itm_tkn_base extends Xop_tkn_itm_base implements Arg_itm_tkn {
	...
	public byte[] Dat_to_bry(byte[] src) {return BryUtl.IsNullOrEmpty(dat_ary) ? BryLni.Mid(src, dat_bgn, dat_end) : dat_ary;}
	public byte[] Dat_ary() {return dat_ary;} private byte[] dat_ary = BryUtl.Empty;
	...
	public Arg_itm_tkn Dat_rng_ary_(byte[] src, int bgn, int end) {
		...
		dat_ary = BryLni.Mid(src, bgn, end);
		return this;
	}
