...
class Vnt_html_doc_wkr implements Mwh_doc_wkr {
	...
	public void Init(BryWtr bfr, Xol_vnt_itm vnt_itm) {this.bfr = bfr; this.vnt_itm = vnt_itm; this.convert_vnt_idx = vnt_itm.Idx();}
	public void On_atr_each			(Mwh_atr_parser mgr, byte[] src, int nde_tid, boolean valid, boolean repeated, boolean key_exists, byte[] key_bry, byte[] val_bry_manual, int[] itm_ary, int itm_idx) {
		...
		if (atr_hash.Get_by_mid(key_bry, 0, key_bry.length) != null) {		// title, alt
			...
			if (BryFind.FindFwd(src, Bry__url_frag, val_bgn, val_end) == BryFind.NotFound) {	// do not convert if urls are present
				literal = false;
				byte[] val_bry = val_bry_manual == null ? BryLni.Mid(src, val_bgn, val_end) : val_bry_manual;
		...
		if (literal) {
			...
			bfr.AddMid(src, atr_bgn, atr_end);
		}
	}
	...
	public void On_nde_head_bgn(Mwh_doc_parser mgr, byte[] src, int nde_tid, int key_bgn, int key_end) {
		bfr.AddByte(AsciiByte.AngleBgn).AddMid(src, key_bgn, key_end);	// EX: "<span"
	}
	...
	public void On_nde_tail_end	(Mwh_doc_parser mgr, byte[] src, int nde_tid, int itm_bgn, int itm_end) {bfr.AddMid(src, itm_bgn, itm_end);}
	public void On_comment_end  (Mwh_doc_parser mgr, byte[] src, int nde_tid, int itm_bgn, int itm_end) {bfr.AddMid(src, itm_bgn, itm_end);}
	public void On_entity_end  (Mwh_doc_parser mgr, byte[] src, int nde_tid, int itm_bgn, int itm_end)	{bfr.AddMid(src, itm_bgn, itm_end);}
	private static final byte[] Bry__url_frag = BryUtl.NewA7("://");	// REF.MW: if ( !strpos( $attr, '://' ) ) {
}
