...
public class Vnt_convert_lang {
	...
	private final BryWtr bfr = BryWtr.NewWithSize(255), tmp_frame_bfr = BryWtr.NewWithSize(255), tmp_convert_bfr = BryWtr.NewWithSize(255);
	...
	public byte[] Parse_bry(Xol_vnt_itm vnt_itm, byte[] src) {
		...
		while (pos < src_len) {
			int curly_bgn = BryFind.FindFwd(src, Bry__curly_bgn, pos, src_len);
			if (curly_bgn == BryFind.NotFound) {						// No more markup, append final segment
	...
	private byte[] Parse_recursive(BryWtr frame_bfr, Xol_vnt_itm vnt_itm, int depth) {
		pos += 2;	// skip "-{"
		...
		while (pos < src_len) {
			...
			switch (((ByteVal)o).Val()) {
				case Tid__curly_bgn:
					frame_bfr.AddMid(src, bgn_pos, pos);	// add everything from bgn of frame to cur pos; EX: "a" in "-{a-{b}-c}-"
					frame_bfr_dirty = true;
					if (depth >= max_depth) {
						pos += 2;	// skip "-{"
						frame_bfr.Add(Bry__curly_bgn);
						if (!warning_done) {
							frame_bfr.AddStrA7("<span class=\"error\">max depth");
							// wfMessage('language-converter-depth-warning')->numParams($this->mMaxDepth)->inContentLanguage()->text()
							frame_bfr.AddStrA7("</span>");
							warning_done = true;
						}
						continue;
					}
					frame_bfr.Add(Parse_recursive(BryWtr.NewWithSize(16), vnt_itm, depth + 1)); // Recursively parse another rule
					bgn_pos = pos;
					break;
				case Tid__curly_end:
					if (frame_bfr_dirty) {	// recursive; use frame_bfr
						frame_bfr.AddMid(src, bgn_pos, pos);	// add everything from bgn of frame to cur pos; EX: "a" in "-{a-{b}-c}-"
						byte[] frame_bry = frame_bfr.ToBryAndClear();
				...
				default: throw ErrUtl.NewUnhandled(-1);		// never happens
			}
		}
		Auto_convert(frame_bfr, vnt_itm, src, bgn_pos, src_len);	// Unclosed rule
		pos = src_len;
		return BryUtl.Add(Bry__curly_bgn, frame_bfr.ToBryAndClear());
	}
	...
	public byte[] Auto_convert(Xol_vnt_itm vnt_itm, byte[] src) {
		...
		return tmp_convert_bfr.ToBryAndClear();
	}
	private void Auto_convert(BryWtr bfr, Xol_vnt_itm vnt_itm, byte[] src, int bgn, int end) {
	...
	private static final byte[] Bry__curly_bgn = BryUtl.NewA7("-{"), Bry__curly_end = BryUtl.NewA7("}-");
