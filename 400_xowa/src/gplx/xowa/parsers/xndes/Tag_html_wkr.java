...
class Tag_html_wkr_noop implements Tag_html_wkr {
	...
	public byte[] Tag__build(Xowe_wiki wiki, Xop_ctx ctx) {return BryUtl.Empty;}
...
class Tag_html_wkr_basic implements Tag_html_wkr {
	...
	private final BryWtr tmp_bfr;
	private byte[] tag_name;
	public Tag_html_wkr_basic(BryWtr tmp_bfr, boolean atrs_encode) {
		this.tmp_bfr = tmp_bfr;
		this.atrs_encode = atrs_encode;
	}
	public void Tag__process_name(byte[] tag_name) {
		this.tag_name = tag_name;
		tmp_bfr.AddByte(AsciiByte.Lt).Add(tag_name);	// EX: "<ref"
	}		
	public void Tag__process_attr(byte[] key, byte[] val) {
		int val_len = BryUtl.Len(val);
		...
		// write key
		tmp_bfr.AddByte(AsciiByte.Space);	// write space between html_args
		int key_len = BryUtl.Len(key);
		if (key_len > 0) {
			if (atrs_encode)
		...
		// write val
		tmp_bfr.AddByte(AsciiByte.Quote);
		if (atrs_encode)
	...
	public void Tag__process_body(byte[] body) {
		tmp_bfr.AddByte(AsciiByte.Gt);
		tmp_bfr.Add(body);
		tmp_bfr.AddByte(AsciiByte.Lt).AddByte(AsciiByte.Slash).Add(tag_name).AddByte(AsciiByte.Gt);	// EX: "</ref>"
	}
