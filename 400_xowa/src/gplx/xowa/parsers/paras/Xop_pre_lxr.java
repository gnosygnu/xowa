...
public class Xop_pre_lxr implements Xop_lxr {
	...
	public int Make_tkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {
		...
		if (bgn_pos == Xop_parser_.Doc_bgn_bos) {		// BOS; gobble up all \s\t; EX: "BOS\s\s\sa" -> "BOSa"
			if		(b == AsciiByte.Nl) {			// next char is nl
			...
			if (b == AsciiByte.Lt)						// next char is <; possible xnde; flag so that xnde can escape; DATE:2013-11-28; moved outside Doc_bgn_bos block above; PAGE:en.w:Comment_(computer_programming); DATE:2014-06-23
	...
	private static final byte[] Hook_space = new byte[] {AsciiByte.Nl, AsciiByte.Space};
}
/*
