...
public class Xop_list_tkn_chkr extends Xop_tkn_chkr_base {
	...
	public int[] List_path() {return list_path;} public Xop_list_tkn_chkr List_path_(int... v) {list_path = v; return this;} private int[] list_path = IntAryUtl.Empty;
	...
	@Override public int Chk_hook(Tst_mgr mgr, String path, Object actl_obj, int err) {
		...
		err += mgr.Tst_val(list_path == IntAryUtl.Empty, path, "list_path", ArrayUtl_ToStr(list_path), ArrayUtl_ToStr(actl.List_path()));
		return err;
	}
	private static String ArrayUtl_ToStr(Object ary) {
		String_bldr sb = String_bldr_.new_();
		int ary_len = ArrayUtl.Len(ary);
		for (int i = 0; i < ary_len; i++)
			sb.AddObj(ArrayUtl.GetAt(ary, i)).AddCharNl();
		return sb.ToStr();
	}
}
