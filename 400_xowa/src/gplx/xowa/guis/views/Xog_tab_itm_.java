...
public class Xog_tab_itm_ {
	public static String Tab_name_min(String name, int min) {
		int name_len = StringUtl.Len(name);
		return min == Tab_name_min_disabled || name_len > min ? name : name + StringUtl.Repeat(" ", min - name_len);
	}
	public static String Tab_name_max(String name, int max) {
		int name_len = StringUtl.Len(name);
		return max == Tab_name_max_disabled || name_len <= max ? name : StringUtl.Mid(name, 0, max) + "...";
	}
