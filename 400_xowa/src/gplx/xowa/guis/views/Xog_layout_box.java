...
import gplx.gfui.draws.*; import gplx.gfui.controls.elems.*;
...
public class Xog_layout_box implements Gfo_invk {
	...
	public int X_abs() {return x_abs;} private int x_abs = IntUtl.MinValue;
	public int Y_abs() {return y_abs;} private int y_abs = IntUtl.MinValue;
	public int W_abs() {return w_abs;} private int w_abs = IntUtl.MinValue;
	public int H_abs() {return h_abs;} private int h_abs = IntUtl.MinValue;
	private int x_rel = IntUtl.MinValue;
	private int y_rel = IntUtl.MinValue;
	public Xog_layout_box W_rel_(int v) {w_rel = v; return this;} private int w_rel = IntUtl.MinValue;
	public Xog_layout_box H_rel_(int v) {h_rel = v; return this;} private int h_rel = IntUtl.MinValue;
	private String text;
	private String font_name;
	private float font_size = FloatUtl.NaN;
	private FontStyleAdp font_style;
	public byte Mode() {return mode;} private byte mode = Mode_rel;
	public void Adj_size(Rect_ref rect) {
		if (w_abs > -1) rect.W_(w_abs);	if (w_rel != IntUtl.MinValue) rect.W_(w_rel + rect.W());
		if (h_abs > -1) rect.H_(h_abs);	if (h_rel != IntUtl.MinValue) rect.H_(h_rel + rect.H());
	}
	public void Adj_pos(Rect_ref rect) {
		if (x_abs > -1) rect.X_(x_abs);	if (x_rel != IntUtl.MinValue) rect.X_(x_rel + rect.X());
		if (y_abs > -1) rect.Y_(y_abs);	if (y_rel != IntUtl.MinValue) rect.Y_(y_rel + rect.Y());
	}
	public void Adj_text(GfuiElem elem) {
		if (text != null) elem.Text_(text);
		if (font_name != null || !FloatUtl.IsNaN(font_size) || font_style != null)
	...
	public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		...
		else if	(ctx.Match(k, Invk_size_abs_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 2, null); if (ary != null) {w_abs = ary[0]; h_abs = ary[1];}}
		else if	(ctx.Match(k, Invk_size_rel_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 2, null); if (ary != null) {w_rel = ary[0]; h_rel = ary[1];}}
		else if	(ctx.Match(k, Invk_pos_abs_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 2, null); if (ary != null) {x_abs = ary[0]; y_abs = ary[1];}}
		else if	(ctx.Match(k, Invk_pos_rel_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 2, null); if (ary != null) {x_rel = ary[0]; y_rel = ary[1];}}
		else if	(ctx.Match(k, Invk_rect_abs_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 4, null); if (ary != null) {w_abs = ary[0]; h_abs = ary[1]; x_abs = ary[2]; y_abs = ary[3];}}
		else if	(ctx.Match(k, Invk_rect_rel_))				{int[] ary = IntAryUtl.Parse(m.ReadStr("v"), 4, null); if (ary != null) {w_rel = ary[0]; h_rel = ary[1]; x_rel = ary[2]; y_rel = ary[3];}}
		...
		else if	(ctx.Match(k, Invk_mode_))					mode = StringUtl.Eq(m.ReadStr("v"), "abs") ? Mode_abs : Mode_rel;
	...
	private static FontAdp Font_make(String font_name, float font_size, FontStyleAdp font_style) {
		...
		float new_font_size = FloatUtl.IsNaN(font_size) ? 8 : font_size;
