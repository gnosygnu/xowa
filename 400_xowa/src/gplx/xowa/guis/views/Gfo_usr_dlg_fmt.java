...
public class Gfo_usr_dlg_fmt {
	public boolean Write_prog_cur(int cur, Gfo_usr_dlg usr_dlg) {
		...
		prog_prv = cur;
		String pct = GfoDecimalUtl.CalcPctStr(cur + List_adp_.Base1, end, "00.00");
		usr_dlg.Prog_many(grp_key, msg_key, fmt, IntUtl.ToStrPadBgnZero(cur + List_adp_.Base1, endLen), end, pct);
	...
	public static Gfo_usr_dlg_fmt fmt_(String grp_key, String msg_key, String fmt, int end, float pct) {
		...
		rv.fmt = fmt; rv.end = end; rv.endLen = IntUtl.CountDigits(end); rv.prog_interval = (int)((float)end * (float)(pct / (float)100));;
		return rv;
	}	String grp_key, msg_key;
	int prog_interval; int prog_prv = IntUtl.MinValue;
}
