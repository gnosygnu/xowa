...
import gplx.xowa.wikis.xwikis.*; import gplx.xowa.xtns.wdatas.core.*; import gplx.xowa.htmls.hzips.*;
...
public class Xoh_lnki_wtr {
	...
	private void Write_plain(Bry_bfr bfr, Xoh_wtr_ctx hctx, byte[] src, Xop_lnki_tkn lnki, Xoa_ttl lnki_ttl, Xop_lnki_caption_wtr caption_wkr) {
		...
		if (hctx.Mode_is_alt())
			...
			if (hctx.Mode_is_hdump()) {
				bfr.Add(gplx.xowa.htmls.hdumps.abrvs.Xohd_abrv_.Html_redlink_bgn);
				bfr.Add_int_variable(lnki.Html_uid());
				bfr.Add(gplx.xowa.htmls.hdumps.abrvs.Xohd_abrv_.Html_redlink_end);
			}
			else {
