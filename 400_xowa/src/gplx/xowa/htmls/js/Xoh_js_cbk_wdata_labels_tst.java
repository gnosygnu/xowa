...
public class Xoh_js_cbk_wdata_labels_tst {
	...
	@Test public void Basic() {
		...
		Tst_wikidata_label_get(StringUtl.Ary("en", "q1", "q2", "Property:P1"), StringUtl.Ary("en_q1", "en_q2", "en_property_p1"));
	}
	@Test public void Outliers() {
		fxt.Init__docs__add(fxt.Wdoc_bldr("q1").Add_label("en", "en_q1").Add_label("de", "de_q1").Xto_wdoc());
		Tst_wikidata_label_get(StringUtl.Ary("fr", "q1"), StringUtl.Ary((String)null));
		Tst_wikidata_label_get(StringUtl.Ary("de", "q1"), StringUtl.Ary("de_q1"));
		Tst_wikidata_label_get(StringUtl.Ary("xowa_title", "q1"), StringUtl.Ary("q1"));
		Tst_wikidata_label_get(StringUtl.Ary("xowa_ui_lang", "q1"), StringUtl.Ary("en_q1"));
		Tst_wikidata_label_get(StringUtl.Ary("fr;de", "q1"), StringUtl.Ary("de_q1"));
	}
	@Test public void Escaped() {	// PURPOSE: \t should be escaped; EX:wd.q:2; DATE:2014-04-23
		Wdata_doc d = doc_("q1", StringUtl.ConcatLinesNl
		(	"{ 'entity':['item',1]"
		,	", 'label':"
		...
		Tst_wikidata_label_get(StringUtl.Ary("en", "q1"), StringUtl.Ary("\ta"));
	}
	private Wdata_doc doc_(String qid, String src) {
		Json_doc jdoc = fxt.Make_json(src);
		Xoae_app app = Xoa_app_fxt.Make__app__edit();
		Wdata_doc rv = new Wdata_doc(app.Wiki_mgr().Wdata_mgr(), jdoc, BryUtl.NewA7(qid));
		return rv;
	}
