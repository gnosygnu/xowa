...
public class Xoh_ns_file_page_mgr_tst {
	...
	@Test public void Image() {
		...
		.tst(StringUtl.ConcatLinesNlSkipLast
...
class Xoh_ns_file_page_mgr_fxt {
	...
	private final Xof_file_itm file = new Xof_fsdb_itm(); private final BryWtr bfr = BryWtr.New();
...
//			file.Orig_ttl_and_redirect_(ttl_bry, Bry_.Empty);
		file.Init_at_orig(ByteUtl.Zero, wiki.Domain_bry(), ttl_bry, Xof_ext_.new_by_ttl_(ttl_bry), 0, 0, BryUtl.Empty);
		file.Init_at_hdoc(0, Xof_html_elem.Tid_img);
		wkr.Bld_html(wiki, bfr, file, ttl, opt, BryUtl.ToA7Bry(html_file_size, 0));	// TEST: must pass in elem_val b/c test only uses 2nd Bld_html while app uses 1st
		GfoTstr.EqLines(expd, bfr.ToStrAndClear());
	}
	public static final String Hdr = StringUtl.ConcatLinesNlSkipLast("<br/>");
//		(	"<ul id=\"filetoc\">"
//		,	"  <li>"
//		,	"    <a href=\"#file\">"
//		,	"      File"
