...
class Xoh_file_page__other_resolutions implements BryBfrArg {
	...
	public void AddToBfr(BryWtr bfr) {
		...
		int len = ary.length;
		Xof_repo_itm repo = repo_mgr.Get_trg_by_id_or_null(orig_itm.Orig_repo_id(), orig_itm.Lnki_ttl(), BryUtl.Empty);
		if (repo == null) return;
		for (int i = 0; i < len; ++i) {
			Int_2_ref itm = ary[i];
			xfer_itm.Init_at_lnki(Xof_exec_tid.Tid_wiki_page, BryUtl.Empty, orig_itm.Lnki_ttl(), Xop_lnki_type.Id_none, Xop_lnki_tkn.Upright_null, itm.Val_0(), itm.Val_1(), Xof_lnki_time.Null, Xof_lnki_page.Null, Xof_patch_upright_tid_.Tid_all);
			xfer_itm.Init_at_orig(orig_itm.Orig_repo_id(), orig_itm.Orig_repo_name(), orig_itm.Orig_ttl(), orig_itm.Orig_ext(), orig_itm.Orig_w(), orig_itm.Orig_h(), BryUtl.Empty);
			...
			file_page.Html_alts().BldToBfrMany(bfr, xfer_itm.Html_w(), xfer_itm.Html_h(), xfer_itm.Html_view_url().To_http_file_bry(), itm_separator, orig_itm.Lnki_ttl());
		}
	}	
}
