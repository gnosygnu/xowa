...
//		if (expd_len != actl_len) throw new RuntimeException(String.format("expd != actl; expd:%s actl:%s", IntUtl.To_str(expd_len), IntUtl.To_str(actl_len)));
//		for (int i = 0; i < expd_len; ++i) {
//			byte expd_byte = expd_bry[i];
//			byte actl_byte = actl_bry[i];
//			if (expd_byte != actl_byte) throw new RuntimeException(String.format("expd != actl; %s expd:%s actl:%s", IntUtl.To_str(i), ByteUtl.To_str(expd_byte), ByteUtl.To_str(actl_byte)));
//		}
