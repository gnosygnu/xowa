...
public class Xoh_hzip_mgr implements Xoh_hzip_wkr {
	...
	private final BryRdr rdr = new BryRdr().DfltDlmSet(Xoh_hzip_dict_.Escape);
	...
	public byte[] Encode_as_bry(Xoh_hzip_bfr bfr, Xow_wiki wiki, Xoh_page hpg, byte[] src) {Encode(bfr, wiki, hpg, src); return bfr.ToBryAndClear();}
	public Gfo_poolable_itm	Encode1(Xoh_hzip_bfr bfr, Xoh_hdoc_wkr hdoc_wkr, Xoh_hdoc_ctx hctx, Xoh_page hpg, boolean wkr_is_root, byte[] src, Object data_obj) {throw ErrUtl.NewUnimplemented();}
	...
	public void Decode(BryWtr bfr, Xow_wiki wiki, Xoh_page hpg, byte[] src) {
		hctx.Init_by_page(wiki, hpg);
		rdr.InitByPage(hpg.Url_bry_safe(), src, src.length);
		Decode1(bfr, hdoc_wkr, hctx, hpg, rdr, src, 0, src.length, null);
	}
	public void Decode1(BryWtr html_bfr, Xoh_hdoc_wkr hdoc_wkr, Xoh_hdoc_ctx hctx, Xoh_page hpg, BryRdr rdr, byte[] src, int src_bgn, int src_end, Xoh_data_itm data_itm) {
		...
		while (true) {
			...
			if (o == null) {
				...
				if (txt_bgn != -1) {bfr.AddMid(src, txt_bgn, pos); txt_bgn = -1;}	// handle pending txt
				Xoh_hzip_wkr wkr = (Xoh_hzip_wkr)o;
				int hook_len = wkr.Hook().length;
				try {
					rdr.InitBySect(wkr.Key(), pos, pos + hook_len);
		...
		if (txt_bgn != -1) bfr.AddMid(src, txt_bgn, src_end);
