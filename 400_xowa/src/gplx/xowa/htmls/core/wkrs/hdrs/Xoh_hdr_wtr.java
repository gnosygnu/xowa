...
public class Xoh_hdr_wtr implements BryBfrArg, Xoh_wtr_itm {
	private int hdr_num; private byte[] hdr_id, hdr_content, hdr_capt_rhs;
	private Xoh_page hpg;
	public boolean Init_by_parse(BryWtr bfr, Xoh_page hpg, Xoh_hdoc_ctx hctx, byte[] src, Xoh_hdr_data data) {
	...
	public boolean Init_by_decode(Xoh_page hpg, Xoh_hdoc_ctx hctx, byte[] src, Xoh_data_itm data_itm) {
		...
		this.hdr_content = BryLni.Mid(src, data.Capt_bgn(), data.Capt_end());
		if (data.Anch_is_diff())
			hdr_id = BryLni.Mid(src, data.Anch_bgn(), data.Anch_end());
		else
			hdr_id = BryUtl.Replace(hdr_content, AsciiByte.Space, AsciiByte.Underline);
		hdr_capt_rhs  = data.Capt_rhs_exists() ? BryLni.Mid(src, data.Capt_rhs_bgn(), data.Capt_rhs_end()) : BryUtl.Empty;
	...
	public void AddToBfr(BryWtr bfr) {
		...
		if (section_len != 0)	// guard against -1 index; should not happen
			section_mgr.Set_content(section_len - 1, bfr.Bry(), bfr.Len() - 1);	// close previous section; -1 to skip "\n" before "<h2>"
		fmtr.BldToBfrMany(bfr, hdr_num, hdr_id, hdr_content, hdr_capt_rhs);
