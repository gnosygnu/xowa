...
public class Xoh_hdr_html {
	private final BryWtr hdr_text_bfr = BryWtr.New();
	private final Xoh_toc_itm invalid_toc_itm = new Xoh_toc_itm().Set__txt(BryUtl.Empty, BryUtl.Empty);

	public void Write_html(BryWtr bfr, Xoh_html_wtr wtr, Xowe_wiki wiki, Xoae_page page, Xop_ctx ctx, Xoh_wtr_ctx hctx, Xoh_html_wtr_cfg cfg, Xop_tkn_grp grp, int sub_idx, byte[] src, Xop_hdr_tkn hdr) {
		// init
		int hdr_num = hdr.Num();
		...
		if (hdr_is_valid) {													// NOTE: need to check hdr_num b/c it could be dangling
			...
			bfr.Add(Bry__hdr_lhs_bgn).AddInt(hdr_num, 1, 1);				// '<h', '2'
		...
		if (hdr.Manual_bgn() > 0) bfr.AddByteRepeat(AsciiByte.Eq, hdr.Manual_bgn());	// '='
		...
		// write </span></h#>
		if (hdr_is_valid) {													// NOTE: need to check hdr_num b/c it could be dangling
			if (hdr.Manual_end() > 0) bfr.AddByteRepeat(AsciiByte.Eq, hdr.Manual_end());	// '='
			if (cfg.Toc__show())
			...
			bfr.Add(Bry__hdr_rhs_bgn).AddInt(hdr_num, 1, 1);				// '</h', '2'
			bfr.Add(Bry__hdr_rhs_end);										// '>\n'
		}
	}

	private static final byte[] Bry__hdr_lhs_bgn = BryUtl.NewA7("<h"), Bry__hdr_rhs_bgn = BryUtl.NewA7("</h"), Bry__hdr_rhs_end = BryUtl.NewA7(">\n")
	, Bry__span_lhs_bgn = BryUtl.NewA7("<span class=\"mw-headline\" id=\""), Bry__span_lhs_end = BryUtl.NewA7("\">")
	;
	public static byte[] Bld_hdr_html(BryWtr rv, Xoh_html_wtr wtr, Xoa_page page, Xop_ctx ctx, Xoh_wtr_ctx hctx, byte[] src, Xop_hdr_tkn hdr) {
