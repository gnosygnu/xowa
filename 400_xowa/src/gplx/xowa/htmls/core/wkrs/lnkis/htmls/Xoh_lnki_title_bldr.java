...
public class Xoh_lnki_title_bldr {
	public static void Add(BryWtr bfr, byte[] src, Xop_tkn_itm tkn) {Add_recurse(bfr, src, tkn);}
	private static void Add_recurse(BryWtr bfr, byte[] src, Xop_tkn_itm tkn) {
		switch (tkn.Tkn_tid()) {				
			...
			case Xop_tkn_itm_.Tid_lnki:
				...
				if (tkn_as_lnki.Tail_bgn() != -1)
					bfr.AddMid(src, tkn_as_lnki.Tail_bgn(), tkn_as_lnki.Tail_end());
				break;				
	...
	private static void Write_atr_text(BryWtr bfr, byte[] src, int bgn, int end) {
		for (int i = bgn; i < end; i++) {
			...
			switch (b) {
				...
				case AsciiByte.Lt:			bfr.Add(HtmlEntityCodes.LtBry); break;
