...
class Xoh_anch_capt_itm_fxt {
	...
	public void Test__match(String page_str, String capt_str, int expd_tid, int expd_trail_bgn) {
		byte[] page_bry = BryUtl.NewU8(page_str);
		byte[] capt_bry = BryUtl.NewU8(capt_str);
		...
		GfoTstr.Eq(expd_tid		, matcher.Parse(rdr.InitByPage(BryUtl.Empty, page_bry, page_bry.length), ns_is_cs, page_bry, 0, page_bry.length, capt_bry, 0, capt_bry.length));
		GfoTstr.Eq(expd_trail_bgn	, matcher.Split_pos());
	}
}
