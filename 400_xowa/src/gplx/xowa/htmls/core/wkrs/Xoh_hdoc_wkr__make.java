...
public class Xoh_hdoc_wkr__make implements Xoh_hdoc_wkr {
	private BryWtr bfr; private Xoh_page hpg; private Xoh_hdoc_ctx hctx; private byte[] src;
	...
	public void On_txt(int rng_bgn, int rng_end) {
		// text; just add it
		bfr.AddMid(src, rng_bgn, rng_end);
	}
	...
	public void On_lnki(gplx.xowa.htmls.core.wkrs.lnkis.Xoh_lnki_data data)	{
		...
		if (BryLni.Eq(src, src_bgn_lhs, src_bgn_rhs, Bry__a__bgn)) {
			bfr.Add(Bry__a__id);
			bfr.AddIntVariable(html_uid);
			bfr.AddByteQuote().AddByteSpace();
			bfr.AddMid(src, src_bgn_rhs, data.Src_end());
		}
		else {
			bfr.AddMid(src, data.Src_bgn(), data.Src_end());
	...
	public boolean On_thm(gplx.xowa.htmls.core.wkrs.thms.Xoh_thm_data data) {
		...
		bfr.AddMid(src, data.Src_bgn(), img_data.Src_bgn());
		// add img
		wkr__img.Init_by_parse(bfr, hpg, hctx, src, img_data);
		// add thm html after img
		bfr.AddMid(src, img_data.Src_end(), data.Src_end());
		return true;
	}
	...
	public static int Lnki_redlink_reg(Xoh_page hpg, Xoh_hdoc_ctx hctx, byte[] href_bry, int html_uid) {
		...
		try {
			...
			Gfo_log_.Instance.Warn("failed to add lnki to redlinks", "page", hpg.Url_bry_safe(), "href_bry", href_bry, "e", ErrUtl.ToStrLog(e));
			return html_uid;
		}
	}
	private static final byte[] Bry__a__bgn = BryUtl.NewA7("<a "), Bry__a__id = BryUtl.NewA7("<a id=\"xolnki_");
}
