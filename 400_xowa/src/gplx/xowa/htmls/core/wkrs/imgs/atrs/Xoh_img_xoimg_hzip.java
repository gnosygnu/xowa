...
public class Xoh_img_xoimg_hzip {
	public void Encode(BryWtr bfr, byte[] src, Xoh_img_xoimg_data arg) {
		...
		if (upright_exists)		bfr.AddDouble(arg.Lnki_upright()).AddByte(Xoh_hzip_dict_.Escape);
		if (time_exists)		bfr.AddDouble(arg.Lnki_time()).AddByte(Xoh_hzip_dict_.Escape);
		if (page_exists)		Gfo_hzip_int_.Encode(2, bfr, arg.Lnki_page());
	}
	public void Decode(BryWtr bfr, Xoh_hdoc_ctx hctx, Xoh_page hpg, BryRdr rdr, byte[] src, Xoh_img_xoimg_data arg) {
		int flag = rdr.ReadHzipInt(1);
		...
		int w				= width_exists		? rdr.ReadHzipInt(2)					: Xof_img_size.Size__neg1;
		int h				= height_exists		? rdr.ReadHzipInt(2)					: Xof_img_size.Size__neg1;
		double upright		= upright_exists	? rdr.ReadDoubleTo(Xoh_hzip_dict_.Escape) : Xof_img_size.Upright_null;
		double time			= time_exists		? rdr.ReadDoubleTo(Xoh_hzip_dict_.Escape) : Xof_lnki_time.Null;
		int page			= page_exists		? rdr.ReadHzipInt(2)					: Xof_lnki_page.Null;
