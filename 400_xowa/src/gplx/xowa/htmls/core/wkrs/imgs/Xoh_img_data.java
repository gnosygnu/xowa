...
import gplx.langs.htmls.*; import gplx.langs.htmls.docs.*; import gplx.xowa.htmls.core.wkrs.imgs.atrs.*; import gplx.xowa.htmls.core.wkrs.lnkis.anchs.*;
...
public class Xoh_img_data implements Xoh_data_itm {
	...
	public BryRef Anch_xo_ttl() {return anch_xo_ttl;} private BryRef anch_xo_ttl = BryRef.NewEmpty();
	...
	public boolean Init_by_parse(Xoh_hdoc_wkr hdoc_wkr, Xoh_hdoc_ctx hctx, Gfh_tag_rdr tag_rdr, byte[] src, Gfh_tag anch_head, Gfh_tag unused) {
		...
		BryRdrErrWkr err_wkr = tag_rdr.Err_wkr();
		...
		if (img_wo_anch) {
			...
			anch_xo_ttl.ValSet(xowa_title.Val());
		}
		else {
			...
			if (xowa_title.Val_dat_exists()) anch_xo_ttl.ValSet(xowa_title.Val());
		...
		img_alt__diff_anch_title = !BryLni.Eq(src, img_alt_bgn, img_alt_end, src, anch_title_bgn, anch_title_end);
		if (!img_src.Parse(err_wkr, hctx.Wiki__domain_bry(), img_tag)) return false;				// src='...'
		if (anch_xo_ttl.ValIsEmpty()) {
			anch_xo_ttl.ValSet(img_src.File_ttl_bry());
			if (anch_xo_ttl.ValIsEmpty())
				anch_xo_ttl.ValSet(anch_href.Ttl_page_db());
		}
		...
		if (anch_href.Site_exists()) {
			Xow_domain_itm itm = Xow_domain_itm_.parse(BryLni.Mid(src, anch_href.Site_bgn(), anch_href.Site_end()));
	...
	private static int Get_imap_idx(BryRdrErrWkr err_wkr, Gfh_tag img_tag) {
		byte[] val = img_tag.Atrs__get_as_bry(Imap_xtn_mgr.Bry__usemap__name); if (BryUtl.IsNullOrEmpty(val)) return -1;
		int rv = BryUtl.ToIntOr(val, Imap_xtn_mgr.Bry__usemap__prefix.length, val.length, -1);
		if (rv == -1) {
	...
	public static final byte[]
	  Bry__cls__anch__image			= BryUtl.NewA7("image")
	, Bry__cls__img__thumbimage		= BryUtl.NewA7("thumbimage")
	, Bry__atr__xowa_title			= BryUtl.NewA7("xowa_title")
	;
}
