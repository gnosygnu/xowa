...
import gplx.xowa.*;
import org.junit.*;
import gplx.xowa.htmls.portal.*; import gplx.xowa.wikis.pages.*;
public class Xoh_page_wtr_mgr_tst {
	...
	@Test public void Logo_has_correct_main_page() {	// PURPOSE: Logo href should be "/site/en.wikipedia.org/wiki/", not "/wiki/Main_Page"
		...
		Gfo_invk_.Invk_by_val(portal_mgr, Xow_portal_mgr.Invk_div_logo_, BryUtl.NewA7("~{portal_nav_main_href}"));
		...
		GfoTstr.EqObj(StringUtl.NewA7(portal_mgr.Div_logo_bry(true)), "/site/en.wikipedia.org/wiki/");
	}
	@Test public void Skip__math__basic() {
		Xop_fxt fxt = Xop_fxt.New_app_html();
		fxt.Init_lang_vnts("zh-hans", "zh-hant");

		fxt.Test__parse_to_html_mgr(StringUtl.ConcatLinesNlSkipLast
		( "<math>x_{1}-1</math>"
		, "<math>x-{1+2}-1</math>"
		, "-{zh-hans:A;zh-hant:B;}-"
