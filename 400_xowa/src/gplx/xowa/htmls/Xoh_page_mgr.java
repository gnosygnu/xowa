...
public class Xoh_page_mgr implements Gfo_invk {
	...
	private byte[] font_css_bry = BryUtl.Empty, custom_script = BryUtl.Empty;
	private final BryFmt font_css_fmt = BryFmt.Auto("body {font-family: ~{font_name}; font-size: ~{font_size}px;}");
	public float Font_size() {return font_size;} private float font_size = Font_size_default;
	private void Font_css_bry_() {
		font_css_bry = font_css_fmt.Bld_many_to_bry(BryWtr.New(), font_name, font_size);
	}
	public void Write_css(gplx.xowa.htmls.heads.Xoh_head_wtr wtr) {
		if (font_enabled)
			wtr.Write_css_style_itm(font_css_bry);
		if (BryUtl.IsNotNullOrEmpty(custom_script))
			wtr.Write_css_style_itm(custom_script);
	}

	public BryFmt Content_code_fmt() {return content_code_fmt;} private final BryFmt content_code_fmt = BryFmt.Auto("<pre>~{page_text}</pre>");
