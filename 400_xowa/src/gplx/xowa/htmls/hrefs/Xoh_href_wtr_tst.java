...
class Xoh_href_wtr_fxt {
	...
	public Xoh_href_wtr_fxt() {
		...
		wiki.Xwiki_mgr().Add_by_csv(BryUtl.NewA7("1|wikt|en.wiktionary.org"));
		app.Usere().Wiki().Xwiki_mgr().Add_by_csv(BryUtl.NewA7("1|en.wiktionary.org|en.wiktionary.org"));
	}
	public Xoae_app App() {return app;} private final Xoae_app app;
	public Xoh_href_wtr_fxt Prep_wiki_cs(String domain) {
		Xow_wiki wiki = app.Wiki_mgr().Get_by_or_make_init_n(BryUtl.NewU8(domain));
		wiki.Ns_mgr().Ns_main().Case_match_(Xow_ns_case_.Tid__all);
		return this;
	}
	public Xoh_href_wtr_fxt Prep_xwiki_by_many(String raw) {wiki.Xwiki_mgr().Add_by_csv(BryUtl.NewU8(raw)); return this;} // need to add to wiki's xwiki_mgr for ttl_parse
	public void Test_build(String raw, String expd) {
		Xoa_ttl ttl = Xoa_ttl.Parse(wiki, BryUtl.NewU8(raw));
