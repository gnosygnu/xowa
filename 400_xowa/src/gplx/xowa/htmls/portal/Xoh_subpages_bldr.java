...
public class Xoh_subpages_bldr implements BryBfrArg {
	private final BryWtr html_bfr = BryWtr.NewAndReset(255), path_bfr = BryWtr.NewAndReset(255), subpage_caption_bfr = BryWtr.NewAndReset(255);
	...
	public byte[] Bld(Xowe_wiki wiki, Xoa_ttl ttl) {
		...
		if (!	(	ns.Subpages_enabled()					// ns has subpages
				...
				&&	ttl.Leaf_bgn() != BryFind.NotFound    // ttl has leaf text; EX: Help:A/B
				)
			)
			return BryUtl.Empty; // doesn't match above; return empty;

		// split ttl by slashes; EX: "Help:A/B/C" -> "Help:A", "B", "C"
		byte[] raw = ttl.Raw();
		this.segs = BrySplit.Split(raw, AsciiByte.Slash);

		// build html
		this.wiki = wiki;
		fmtr_grp.BldToBfrArgs(html_bfr, this);
		return html_bfr.ToBryAndClear();
	}
	public void AddToBfr(BryWtr bfr) {
		...
		for (int i = 0; i < segs_len; i++) {
			...
			// build subpage_ttl
			byte[] subpage_ttl_bry = path_bfr.ToBry();
			...
			if (subpage_caption_bfr.HasSome()) subpage_caption_bfr.AddByteSlash();
			...
			byte[] subpage_caption
				...
				: subpage_caption_bfr.ToBry(); // nth seg is caption_bfr ("b", not "Help:A/b")
			...
			fmtr_itm.BldToBfr(bfr, delimiter
				, BryUtl.Add(Xoh_href_.Bry__wiki, subpage_ttl.Full_url()) // EX: /wiki/Help:A
				, Xoa_ttl.Replace_unders(subpage_ttl_escaped)
				, Xoa_ttl.Replace_unders(subpage_caption));
		}
		path_bfr.Clear();
	}
	private static final byte[] Delimiter__1st = BryUtl.NewA7("&lt; "), Delimiter__nth = BryUtl.NewA7("&lrm; | ");
