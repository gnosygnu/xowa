...
class Xow_popup_anchor_finder {
	...
	private boolean Find_hdr(int lhs_bgn) {
		int nl_rhs = Bry_find_.Find_fwd(src, AsciiByte.Nl, nl_lhs + 1, src_len);		// look for \n
		if (nl_rhs == Bry_find_.Not_found) nl_rhs = src_len - 1;							// no more \n; set to last idx
		nl_lhs = nl_rhs;																	// update nl_lhs for loop
		int lhs_end = Bry_find_.Find_fwd_while(src, lhs_bgn + 1, nl_rhs, AsciiByte.Eq);	// skip eq; EX: "\n==="; +1 to skip eq
		int rhs_end = Bry_find_.Trim_bwd_space_tab(src, nl_rhs, lhs_end);					// skip ws bwd; EX: "==   \n"
		int rhs_bgn = Bry_find_.Find_bwd_while(src, rhs_end, lhs_end, AsciiByte.Eq);		// skip eq; EX: "==\n" -> pos before =
	...
	private int Find_id(int bgn) {
		byte[] quoted = Bry_.Add(AsciiByte.QuoteBry, find, AsciiByte.QuoteBry);
	...
	private int Find_id_by_quoted(int bgn, byte[] quoted) {
		...
		rv = Bry_find_.Find_bwd(src, AsciiByte.Nl, id_bgn);
