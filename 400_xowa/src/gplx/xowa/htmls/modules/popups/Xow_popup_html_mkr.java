...
public class Xow_popup_html_mkr {
	...
	public BryFmtr Fmtr_popup()		{return fmtr_popup;}			private BryFmtr fmtr_popup				= BryFmtr.New(Dflt_html_fmtr_popup, Dflt_html_fmtr_popup_keys);
	public BryFmtr Fmtr_viewed()		{return fmtr_viewed;}			private BryFmtr fmtr_viewed			= BryFmtr.New(Dflt_html_fmtr_viewed, Dflt_html_fmtr_viewed_keys);
	public BryFmtr Fmtr_wiki()			{return fmtr_wiki;}				private BryFmtr fmtr_wiki				= BryFmtr.New(Dflt_html_fmtr_wiki, Dflt_html_fmtr_wiki_keys);
	public BryFmtr Fmtr_next_sect()	{return fmtr_next_sect;}		private BryFmtr fmtr_next_sect			= BryFmtr.New(Dflt_html_fmtr_next_sect, Dflt_html_fmtr_next_sect_keys);
	...
	public void Ctor(Xoae_app app, Xowe_wiki wiki) {
		...
		this.fmtr_popup = app.Mode().Tid_is_http() ?
				BryFmtr.New(Web_html_fmtr_popup, Dflt_html_fmtr_popup_keys) :
				BryFmtr.New(Dflt_html_fmtr_popup, Dflt_html_fmtr_popup_keys);
		wiki.Eval_mgr().Eval_mgr_(fmtr_popup, fmtr_viewed, fmtr_wiki, fmtr_next_sect);
	}
	public byte[] Bld(Xowe_wiki cur_wiki, Xoae_page page, Xow_popup_itm popup_itm, BryWtr wrdx_bfr) {
		...
		byte[] hdom_bry = wrdx_bfr.ToBryAndClear();
		String page_url = wrdx_bfr.Add(page.Wiki().Domain_bry()).Add(gplx.xowa.htmls.hrefs.Xoh_href_.Bry__wiki).Add(gplx.langs.htmls.encoders.Gfo_url_encoder_.Href
			...
			.ToStrAndClear()
		;
		fmtr_popup.BldToBfrMany
		( wrdx_bfr
		, hdom_bry
		, wiki.Lang().Dir_ltr_bry()
		, page_url
		, StringUtl.NewU8(page.Ttl().Full_txt())
		, popup_itm.Popup_id()
		...
		, page.Db().Page().Modified_on().ToStrFmt_yyyy_MM_dd_HH_mm_ss()
		, Xow_popup_html_bldr_.Bld_fmtr_viewed(fmtr_viewed, app, wiki, wrdx_bfr, page.Ttl())
		, app.Fsys_mgr().Root_dir().To_http_file_bry()
		);
		return wrdx_bfr.ToBryAndClear();
	}

	private static final byte[]
	  Dflt_html_fmtr_popup = BryUtl.NewA7(StringUtl.ConcatLinesNlSkipLast
...
//		, "    <span><a href='javascript:xowa.popups.FetchMore(\"~{popup_id}\");' title='~{<>msgs.get('api-xowa.html.modules.popups.show_more-tip');<>}'><img src='~{xowa_root_dir}bin/any/xowa/html/res/src/xowa/popups/imgs/show_more.png'></a></span>"
	...
	private static final String[]
	  Dflt_html_fmtr_popup_keys			= StringUtl.Ary("content", "page_lang_ltr", "page_url", "page_title", "popup_id", "wiki_item", "page_size", "edit_time", "view_time_item", "xowa_root_dir")
	, Dflt_html_fmtr_viewed_keys		= StringUtl.Ary("viewed_val")
	, Dflt_html_fmtr_wiki_keys			= StringUtl.Ary("wiki_val")
	, Dflt_html_fmtr_next_sect_keys		= StringUtl.Ary("next_sect_val")
	;
}
class Xow_popup_html_bldr_ {
	public static byte[] Bld_fmtr_wiki(BryFmtr fmtr, BryWtr wrdx_bfr, byte[] wiki_domain, byte[] page_domain) {
		return BryLni.Eq(wiki_domain, page_domain)
			? BryUtl.Empty	// same domain; return "";
			: fmtr.BldToBryMany(wrdx_bfr, page_domain);
	}
	public static byte[] Bld_fmtr_viewed(BryFmtr fmtr, Xoae_app app, Xowe_wiki wiki, BryWtr wrdx_bfr, Xoa_ttl ttl) {
		byte[] view_time_item = BryUtl.Empty;
		gplx.xowa.users.history.Xou_history_itm history_itm = app.Usere().History_mgr().Get_or_null(wiki.Domain_bry(), ttl.Full_txt());
		if (history_itm != null)
			view_time_item = fmtr.BldToBryMany(wrdx_bfr, history_itm.View_end().ToStrFmt_yyyy_MM_dd_HH_mm_ss());
