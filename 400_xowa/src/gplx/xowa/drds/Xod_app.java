...
import gplx.xowa.wikis.nss.*; import gplx.xowa.guis.cbks.js.*;
...
public class Xod_app {
	...
	public Xow_wiki Wikis__get_by_domain(String wiki_domain) {
		Xow_wiki rv = app.Wiki_mgri().Get_by_or_make_init_y(BryUtl.NewU8(wiki_domain));
	...
	public void Wiki__search(Cancelable cxl, Srch_rslt_cbk cbk, Xow_wiki wiki, String search, int bgn, int end) {
		...
		Srch_search_qry qry = Srch_search_qry.New__drd(wiki, ns_mgr, BryUtl.NewU8(search), bgn, end);
	...
	public static byte[] To_page_url(Xow_wiki wiki, String canonical_str) {// NOTE: need canonical_url to handle "A:B" where "A:" is not a ns, even though PageTitle treats "A:" as a namespace
		byte[] canonical_bry = BryUtl.NewU8(canonical_str);
		int page_bgn = BryFind.MoveFwd(canonical_bry, Xoh_href_.Bry__wiki, 0); if (page_bgn == BryFind.NotFound) throw ErrUtl.NewArgs("uknown url format: no '/wiki/'", "url", canonical_bry);
		byte[] page_bry = BryLni.Mid(canonical_bry, page_bgn, canonical_bry.length);	// get bry; EX: https://en.wikipedia.org/wiki/A -> A
