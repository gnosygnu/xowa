...
public class Xoa_url {
	...
	public String			Anch_str() {return anch_bry == null ? null : StringUtl.NewU8(anch_bry);}
	...
	public byte[] Page_for_lnki() {
		...
		if (page_bgn == BryFind.NotFound)	// no /wiki/ found; return page
			return page_bry == null ? BryUtl.Empty : page_bry;	// guard against null ref
		else
			return BryLni.Mid(raw, page_bgn, raw_len);// else take everything after "/wiki/";
	}
	private int Page_bgn(int raw_len) {
		int wiki_pos = BryFind.FindFwd(raw, Xoh_href_.Bry__wiki, 0, raw_len);	 // look for /wiki/
		return wiki_pos == BryFind.NotFound ? BryFind.NotFound : wiki_pos + Xoh_href_.Bry__wiki.length;
	}
	public boolean Eq_page(Xoa_url comp) {return BryLni.Eq(wiki_bry, comp.wiki_bry) && BryLni.Eq(page_bry, comp.page_bry) && this.Qargs_mgr().Match(Xoa_url_.Qarg__redirect, Xoa_url_.Qarg__redirect__no) == comp.Qargs_mgr().Match(Xoa_url_.Qarg__redirect, Xoa_url_.Qarg__redirect__no);}
	public String To_str()					{return StringUtl.NewU8(To_bry(BoolUtl.Y, BoolUtl.Y));}
	public byte[] To_bry_page_w_anch()		{
		byte[] page = page_bry, anch = anch_bry;
		byte[] anch_spr = anch == null ? null : AsciiByte.HashBry;
		return BryUtl.Add(page, anch_spr, anch);
	}
	...
	public byte[] To_bry(boolean full, boolean show_qargs) {							// currently used for status bar; not embedded in any html
		switch (tid) {
			case Xoa_url_.Tid_unknown:											// unknown; should not occur?
				return BryUtl.IsNullOrEmpty(raw) ? BryUtl.Add(wiki_bry, Xoh_href_.Bry__wiki, page_bry) : raw;	// raw is empty when using new_();
		...
		byte[] wiki_spr = vnt_bry == null ? Xoh_href_.Bry__wiki : BryUtl.Add(AsciiByte.SlashBry, vnt_bry, AsciiByte.SlashBry);
		byte[] anch_spr = anch == null ? null : AsciiByte.HashBry;
		if (!full) {
		...
		bfr.AddSafe(wiki).AddSafe(wiki_spr);							// add wiki_key;	EX: "en.wikipedia.org", "/wiki/"
		bfr.AddSafe(page);												// add page;		EX: "A"
	...
	public static Xoa_url Test() {return New(BryUtl.NewA7("test_wiki"), BryUtl.NewA7("test_page"));}
