...
public class Xof_meta_mgr implements Gfo_invk {
	...
	public void Save(boolean clear) {
		...
		for (int i = 0; i < dirty_len; i++) {
			Xof_meta_fil fil = (Xof_meta_fil)dirty_fils.GetAt(i);
	...
	Xof_meta_fil Load(byte[] md5) {
		...
		byte[] bry = Io_mgr.Instance.LoadFilBry(fil_url); if (bry == BryUtl.Empty) return null;
		rdr.Ini(bry, 0);
	...
	private static Xof_meta_fil Get_fil_or_null_recur(Object[] ary, int depth_max, byte[] md5, int md5_idx) {
		int ary_idx = IntUtl.ByHexByte(md5[md5_idx]);
		if (ary_idx < 0 || ary_idx > 15) throw ErrUtl.NewArgs("md5_not_valid", "md5", StringUtl.NewU8(md5), "idx", md5_idx);
		Object o = ary[ary_idx];
		if (o == null) return null;
		if (md5_idx == depth_max) {	// leaf; return fil
			try {return (Xof_meta_fil)o;} catch (Exception exc) {throw ErrUtl.NewCast(exc, Xof_meta_fil.class, o);}
		}
		Object[] nxt = null; try {nxt = (Object[])o;} catch(Exception exc) {throw ErrUtl.NewCast(exc, Object[].class, o);}
	...
	private static Xof_meta_fil Bld_new(Object[] ary, int depth_max, Xof_meta_mgr regy_mgr, byte[] md5, int md5_idx) {
		int ary_idx = IntUtl.ByHexByte(md5[md5_idx]);
		if (ary_idx < 0 || ary_idx > 15) throw ErrUtl.NewArgs("md5_not_valid", "md", StringUtl.NewU8(md5), "idx", md5_idx);
		Object o = ary[ary_idx];
