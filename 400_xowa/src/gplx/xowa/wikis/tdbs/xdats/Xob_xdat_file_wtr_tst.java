...
public class Xob_xdat_file_wtr_tst {
	@Test public void Write() {
		...
		tst_Add_idx(wtr, 7);
		wtr.Bfr().Add(BryUtl.NewA7("<2>b>cc"));
		tst_Add_idx(wtr, 15);
		wtr.Bfr().Add(BryUtl.NewA7("<3>c>dd"));
		tst_Add_idx(wtr, 23);
		tst_Flush(wtr, StringUtl.Concat
			( "!!!!(|!!!!)|!!!!)|\n"
			, "<1>a>b\n"
			, "<2>b>cc\n"
			, "<3>c>dd\n"
	...
	private void tst_Write(Xob_xdat_file_wtr wtr, String val, String expd) {
		wtr.Bfr().Add(BryUtl.NewU8(val));
		GfoTstr.EqObj(expd, StringUtl.NewU8(wtr.Bfr().Bry(), 0, wtr.Bfr().Len()));
	}
	private void tst_Add_idx(Xob_xdat_file_wtr wtr, int expd) {
		wtr.Add_idx(AsciiByte.Nl);
		GfoTstr.EqObj(expd, wtr.Idx()[wtr.Idx_pos() - 1]);
	}
