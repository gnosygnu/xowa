...
public class Xowd_regy_mgr_ {
	public static int FindSlot(ComparerAble comparer, Object[] ary, Object itm) {if (itm == null) throw ErrUtl.NewNull();
		...
		while (true) {
			...
			int comp = curSeg == null ? CompareAbleUtl.More : comparer.compare(itm, curSeg);	// nulls should only happen for lastAry
//				if (dbg) {
//					Tfds.Write(curPos, itm.toString(), comp, comp.toString(), curSeg.toString());
//				}
			if		(comp == CompareAbleUtl.Same) return curPos;
			else if	(comp >  CompareAbleUtl.Same) {lo = curPos; delta = 1;}
			else if	(comp <  CompareAbleUtl.Same) {hi = curPos; delta = -1;}
			int dif = hi - lo;
