...
class Xowd_cat_core_tbl__in_wkr extends Db_in_wkr__base {
	...
	private String tbl_name, fld_cat_id; private Db_meta_fld_list flds; private OrderedHash hash;
	...
	@Override protected void Fill_stmt(Db_stmt stmt, int bgn, int end) {
		for (int i = bgn; i < end; i++) {
			Xowd_page_itm itm = (Xowd_page_itm)hash.FetchAt(i);
	...
	@Override protected void Read_data(Cancelable cancelable, Db_rdr rdr) {
		while (rdr.Move_next()) {
			...
			Xowd_page_itm page = (Xowd_page_itm)hash.Fetch(ctg_data.Id_val());
			page.Xtn_(ctg_data);
		}
	}
}
