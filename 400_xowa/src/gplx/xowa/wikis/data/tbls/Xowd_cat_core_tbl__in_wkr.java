...
class Xowd_cat_core_tbl__in_wkr extends Db_in_wkr__base {
	...
	public void Ctor(Xowd_cat_core_tbl tbl, String tbl_name, DbmetaFldList flds, String fld_cat_id) {
	...
	@Override protected Db_qry Make_qry(int bgn, int end) {
		...
		return Db_qry_.select_cols_(tbl_name, Db_crt_.New_in(fld_cat_id, part_ary), flds.ToStrAry());
	}
	...
	@Override protected void Read_data(Cancelable cancelable, Db_rdr rdr) {
		while (rdr.Move_next()) {
			...
			Xowd_page_itm page = (Xowd_page_itm)hash.GetByOrNull(ctg_data.Id_val());
			page.Xtn_(ctg_data);
		}
	}
}
