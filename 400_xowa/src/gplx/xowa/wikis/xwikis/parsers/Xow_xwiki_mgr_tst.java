...
class Xow_xwiki_mgr_fxt {
	...
	public Xow_xwiki_itm xwiki_null_(String key) {return Xow_xwiki_itm.new_(BryUtl.NewU8(key), BryUtl.Empty, Xol_lang_stub_.Id__unknown, Xow_domain_tid_.Tid__other, BryUtl.Empty, BryUtl.Empty);}
	public Xow_xwiki_itm xwiki_(String key, String domain_str, String url_fmt) {
		...
		return Xow_xwiki_itm.new_(BryUtl.NewU8(key), BryUtl.NewU8(url_fmt), domain.Lang_actl_itm().Id(), domain.Domain_type_id(), domain.Domain_bry(), domain.Abrv_wm());
	}
	...
	public Xow_xwiki_mgr_fxt Init_peers() {
		wiki.Xwiki_mgr().Add_by_csv(BryUtl.NewU8(StringUtl.ConcatLinesNl
	...
	public Xow_xwiki_mgr_fxt Test_add_bulk_langs(Xow_xwiki_itm... itms) {
		...
		GfoTstr.EqLines(Xto_str(itms), Xto_str(To_ary(itms)));
		return this;
	}
	...
	public Xow_xwiki_mgr_fxt Test_len(int expd)			{GfoTstr.EqObj(expd, xwiki_mgr.Len()); return this;}
	Xow_xwiki_itm[] To_ary(Xow_xwiki_itm[] itms) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (itm == null) itm = xwiki_null_(StringUtl.NewU8(alias)); // "null", ignore
			rv.Add(itm);
		}
	...
	String Xto_str(Xow_xwiki_itm[] itms) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (BryUtl.IsNullOrEmpty(itm.Domain_bry()))	// "null", ignore
				sb.Add(itm.Key_bry()).AddCharNl();
			else {
				sb.Add(itm.Key_bry()).AddCharPipe().Add(itm.Domain_bry()).AddCharPipe().Add(itm.Url_fmt()).AddCharPipe().Add(itm.Domain_tid()).AddCharNl();
			}
		}
