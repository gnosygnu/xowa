...
public class Xow_xwiki_itm implements CompareAble {
	...
	public Xow_xwiki_itm(byte[] key_bry, byte[] url_fmt, int lang_id, int domain_tid, byte[] domain_bry, byte[] domain_name, byte[] abrv_wm) {
		this.key_bry = key_bry; this.key_str = StringUtl.NewU8(key_bry);
		this.url_fmt = url_fmt; this.lang_id = lang_id;
		this.url_fmtr = BryUtl.IsNullOrEmpty(url_fmt) ? null : BryFmtr.New(url_fmt, "0");
	...
	public BryFmtr Url_fmtr(){return url_fmtr;} private final BryFmtr url_fmtr;
	...
	public int compareTo(Object obj) {Xow_xwiki_itm comp = (Xow_xwiki_itm)obj; return BryUtl.Compare(key_bry, comp.key_bry);}
	public boolean Show_in_sitelangs(byte[] cur_lang_key) {
		return	show_in_sitelangs_base
			&&	!BryLni.Eq(key_bry, cur_lang_key) 			// lang is different than current; EX: [[en:A]] in en.wikipedia.org shouldn't link back to self
			;
	}
	...
	private static boolean Calc_show_in_sitelangs(byte[] key_bry, byte[] url_fmt, int lang_id, int domain_tid, byte[] domain_bry) {
		...
		boolean key_matches_domain_bgn = BryLni.Eq(domain_bry, 0, key_len, key_bry) && key_len + 1 < domain_bry.length && domain_bry[key_len] == AsciiByte.Dot;	// key + . matches start of domain; EX: "en" and "en.wikipedia.org"
		return	lang_id != Xol_lang_stub_.Id__unknown		// valid lang code
			...
			&&	BryUtl.IsNotNullOrEmpty(url_fmt)						// url_fmt exists
