...
public class Xoa_sitelink_div_wtr {
	...
	public void Write(BryWtr bfr, Xowe_wiki wiki, Xoa_sitelink_mgr mgr, List_adp slink_list, byte[] qid) {
		...
		for (int i = 0; i < slink_len; i++) {
			Wdata_sitelink_itm slink = (Wdata_sitelink_itm)slink_list.GetAt(i);
			...
			if (BryUtl.IsNullOrEmpty(ttl_bry)) {	// NOTE: handles ttls like [[fr:]] and [[:fr;]] which have an empty Page_txt, but a valued Full_txt_raw
		...
		try {
			byte[] wikidata_link = BryUtl.IsNullOrEmpty(qid) ? BryUtl.Empty : wbase_fmtr.BldToBryMany(tmp_bfr, qid);
			div_fmtr.BldToBfrMany(bfr, slink_len, wikidata_link, toggle_itm.Html_toggle_btn(), toggle_itm.Html_toggle_hdr(), grp_wtr.Fmt__init(grp_mgr));
		} finally {tmp_bfr.MkrRls();}
	}
	private static final BryFmtr wbase_fmtr = BryFmtr.New(" (<a href=\"/site/www.wikidata.org/wiki/~{qid}\">wikidata</a>)", "qid");
	private static final BryFmtr div_fmtr = BryFmtr.New(StringUtl.ConcatLinesNlSkipLast
	( "<div id=\"xowa-lang\">"
