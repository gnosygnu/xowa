...
public class Xow_wiki_props {
	...
	public byte[]            Site_name()          {return site_name;}          private byte[] site_name = BryUtl.Empty;
	public byte[]            Server_name()        {return server_name;}        private byte[] server_name = BryUtl.NewA7("localhost");
	public byte[]            Server()             {return server;}             private byte[] server = BryUtl.NewA7("http://localhost");
	public byte[]            Article_path()       {return article_path;}       private byte[] article_path = Xoh_href_.Bry__wiki;
	public byte[]            Script_path()        {return script_path;}        private byte[] script_path = BryUtl.NewA7("/wiki");
	public byte[]            Style_path()         {return style_path;}         private byte[] style_path = BryUtl.NewA7("/wiki/skins");
	public byte[]            Content_language()   {return content_language;}   private byte[] content_language = BryUtl.Empty;
	public byte[]            Direction_mark()     {return direction_mark;}     private byte[] direction_mark = BryUtl.Empty;
	public byte[]            Current_version()    {return CURRENT_VERSION;}    private static final byte[] CURRENT_VERSION = BryUtl.NewA7("1.21wmf11"); // approximate level of compatibility
	...
	public byte[]            Bldr_version()       {return bldr_version;}       private byte[] bldr_version = BryUtl.Empty;
	public int               Css_version()        {return css_version;}        private int css_version = 1;
	public byte[]            Siteinfo_misc()      {return siteinfo_misc;}      private byte[] siteinfo_misc = BryUtl.Empty;
	public byte[]            Siteinfo_mainpage()  {return siteinfo_mainpage;}  private byte[] siteinfo_mainpage = BryUtl.Empty;
	public GfoDate Modified_latest()    {return modified_latest;}    private GfoDate modified_latest;
	public String            Modified_latest__yyyy_MM_dd() {return modified_latest == null ? "" : modified_latest.ToStrFmt_yyyyMMdd();}
	...
	public void Siteinfo_misc_(byte[] v) {
		...
		int pipe_0 = BryFind.FindFwd(v, AsciiByte.Pipe);
		if (pipe_0 != BryFind.NotFound)
			this.site_name = BryLni.Mid(siteinfo_misc, 0, pipe_0);
	}
	...
	public void Init_by_ctor(int domain_tid, byte[] domain_bry) {
		...
		this.site_name = BryUtl.NewA7(StringUtl.UpperFirst(StringUtl.NewA7(Xow_domain_tid_.Get_type_as_bry(domain_tid))));
		...
		this.server = BryUtl.Add(gplx.core.net.Gfo_protocol_itm.Itm_https.Text_bry(), domain_bry);
	}
	...
	public void Init_by_load_2(byte[] main_page, byte[] bldr_version, byte[] siteinfo_misc, byte[] siteinfo_mainpage, GfoDate modified_latest) {
