...
public class Xow_cache_mgr {
	...
	private void Scrib_lang_names_grps(ListAdp list, Cfg_nde_obj nde) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (lang_nde.Nde_typ_is_grp())
				...
				KeyVal kv = KeyVal_.new_(String_.new_utf8_(lang_itm.Key_bry()), String_.new_utf8_(lang_itm.Local_name_bry()));
				list.Add(kv);
			}
		}
	}
