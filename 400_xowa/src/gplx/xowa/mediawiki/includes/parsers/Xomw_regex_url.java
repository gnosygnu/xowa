...
public class Xomw_regex_url {
	...
	public Xomw_regex_url(Xomw_regex_space regex_space) {
		...
		trie.Add_str_byte__many(ByteUtl.Zero, "[", "]", "<", ">", "\"");
		for (byte i = 0; i < 33; i++) {
			trie.Add_bry_byte(new byte[] {i}, ByteUtl.Zero);
		}
		trie.Add_bry_byte(BryUtl.NewByInts(127), ByteUtl.Zero);	// x7F

		byte[][] zs_ary = regex_space.Zs();
		for (byte[] zs : zs_ary) {
			trie.Add_bry_byte(zs, ByteUtl.Zero);
		}
	}
