...
class Xomw_lnki_wkr__fxt {
	...
	public Xomw_lnki_wkr__fxt() {
		...
		env.Magic_word_mgr().Add(BryUtl.NewU8("img_thumbnail"), BoolUtl.Y, BryUtl.Ary("thumb"));
		env.Magic_word_mgr().Add(BryUtl.NewU8("img_width"), BoolUtl.Y, BryUtl.Ary("$1px"));
		env.Message_mgr().Old_add("thumbnail-more", "enlarge", env.Language());
		parser.Init_by_wiki(wiki);

		// ctx
		pctx = new XomwParserCtx();
		pctx.Init_by_page(XomwTitleOld.newFromText(env, BryUtl.NewA7("Page_1")));
	}
	public void Clear() {
		wkr.Clear_state();
	}
	...
	public void Test__to_html(String src_str, String expd) {
		if (apos) expd = gplx.langs.htmls.Gfh_utl.Replace_apos(expd);
		GfoTstr.EqLines(expd, Exec__to_html(src_str), src_str);
	}
	public void Test__to_html__has(String src_str, String expd) {
		if (apos) expd = gplx.langs.htmls.Gfh_utl.Replace_apos(expd);
		GfoTstr.EqBoolY(StringUtl.Has(Exec__to_html(src_str), expd));
	}
	private String Exec__to_html(String src_str) {
		byte[] src_bry = BryUtl.NewU8(src_str);
		wkr.replaceInternalLinks(pbfr.Init(src_bry), env, pctx);
		wkr.replaceLinkHolders(pbfr);
		return pbfr.Rslt().ToStrAndClear();
	}
	public void Test__parseWidthParam(int[] img_size, String src_str, int expd_w, int expd_h) {
		wkr.parseWidthParam(img_size, BryUtl.NewU8(src_str));
		GfoTstr.Eq(expd_w, img_size[0], "w");
		GfoTstr.Eq(expd_h, img_size[1], "h");
	}
}
