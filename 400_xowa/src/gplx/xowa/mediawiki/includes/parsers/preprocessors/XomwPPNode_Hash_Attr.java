...
public class XomwPPNode_Hash_Attr extends XomwPPNode { 	public String name, value;
	...
	*/
	public XomwPPNode_Hash_Attr(XophpArray store, int index) {
		...
		if (!(StringUtl.CharAt(descriptor_name, 0) ==  '@')) {
			throw ErrUtl.NewArgs("XomwPPNode_Hash_Attr.CTOR: invalid name in attribute descriptor");
		}
		this.name = StringUtl.NewU8(XophpString_.substr(BryUtl.NewU8(descriptor_name), 1));
		...
		this.value = ClassUtl.EqByObj(byte[].class, value_obj) ? StringUtl.NewU8((byte[])value_obj): value_obj.toString();
		this.store = store;
		this.index = index;
	}

	@Override public String toString() {
		return StringUtl.Format("<@{0}>{1}</@{0}>", this.name, HtmlBryBfr.EscapeHtml(BryUtl.NewU8(this.value)));
	}
