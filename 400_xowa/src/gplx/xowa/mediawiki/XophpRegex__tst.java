...
public class XophpRegex__tst {
	...
	@Test public void preg_quote() {
		GfoTstr.Eq("abc", XophpRegex_.preg_quote("abc", "/"));
		GfoTstr.Eq("\\.\\\\\\+\\*\\?\\[\\^\\]\\$\\(\\)\\{\\}\\=\\!\\<\\>\\|\\:\\-\\#", XophpRegex_.preg_quote(".\\+*?[^]$(){}=!<>|:-#", "/"));
	}
}
class XophpRegex__fxt {
	...
	public void Test__preg_match(String pattern, String str, int flags, int offset, XophpRegex__expd rslt) {
		...
		GfoTstr.Eq(rslt.Pos(), actl_pos);
		XophpArray expd_matches = rslt.Matches();
		if (expd_matches != null) {
			GfoTstr.EqLines(expd_matches.To_str(), actl_matches.To_str());
		}
	}
	public void Term() {
		if (print_php != null)
			GfoTstr.Write(print_php.AddCharNl().ToStrAndClear());
	}
}
