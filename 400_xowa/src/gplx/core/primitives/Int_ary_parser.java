...
public class Int_ary_parser extends Obj_ary_parser_base {
	...
	public int[] Parse_ary(String str, byte dlm) {byte[] bry = BryUtl.NewU8(str); return Parse_ary(bry, 0, bry.length, dlm);}
	...
	@Override protected void Parse_itm(byte[] bry, int bgn, int end) {
		parser.Parse(bry, bgn, end); if (parser.Has_err() || parser.Has_frac()) throw ErrUtl.NewArgs("failed to parse number", "val", StringUtl.NewU8(bry, bgn, end));
