...
public class Gfh_class__tst {
	...
	@Test public void Cls__has__hash() {
		...
		fxt.Test__find_1st(hash,  ByteUtl.MaxValue127, "a");
		fxt.Test__find_1st(hash,  ByteUtl.MaxValue127, "xyz");
	}
}
class Gfh_class__fxt {
	...
	public void Test__has(boolean expd, String src, String... ary) {
		...
		for (String itm : ary) {
			...
			GfoTstr.Eq(expd, Gfh_class_.Has(src_bry, 0, src_bry.length, itm_bry), itm);
		}
	}
	public Hash_adp_bry Make_hash(String... ary) {
		...
		int len = ary.length;
		for (int i = 0; i < len; ++i)
			rv.Add_bry_byte(BryUtl.NewU8(ary[i]), (byte)i);
		return rv;
	}
	public void Test__find_1st(Hash_adp_bry hash, int expd, String src) {
		byte[] src_bry = BryUtl.NewU8(src);
		GfoTstr.EqByte((byte)expd, Gfh_class_.Find_1st(src_bry, 0, src_bry.length, hash), src);
	}
}
