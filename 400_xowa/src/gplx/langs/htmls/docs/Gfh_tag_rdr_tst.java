...
class Gfh_tag_rdr_fxt {
	...
	public void Init(String src_str) {
		...
		rdr.Init(BryUtl.Empty, src_bry, 0, src_bry.length);
	}
	...
	public void Test__move_fwd_head(int match_name_id, String expd) {
		...
		GfoTstr.Eq(expd, StringUtl.NewU8(rdr.Src(), actl_tag.Src_bgn(), actl_tag.Src_end()));
	}
	...
	public void Test__peek_bwd_tail(String expd_str) {
		byte[] expd_bry = BryUtl.NewU8(expd_str);
		Gfh_tag actl_tag = rdr.Tag__peek_bwd_tail(-1);
		GfoTstr.Eq(expd_bry, BryLni.Mid(rdr.Src(), actl_tag.Src_bgn(), actl_tag.Src_bgn() + expd_bry.length));
	}
	public void Test__pos(String expd_str) {
		byte[] expd_bry = BryUtl.NewU8(expd_str);
		GfoTstr.Eq(expd_bry, BryLni.Mid(rdr.Src(), rdr.Pos(), rdr.Pos() + expd_bry.length));
	}
}
