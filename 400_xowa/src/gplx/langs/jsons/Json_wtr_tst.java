...
class Json_wtr_fxt {
	private final Json_wtr wtr = new Json_wtr().Opt_quote_byte_(AsciiByte.Apos);
	public void Clear() {wtr.Clear();}
	public Json_wtr Wtr() {return wtr;}
	public void Test(String... expd) {
		GfoTstr.EqLines
		( StringUtl.AryAdd(expd, StringUtl.Ary(""))		// json_wtr always ends with "}\n"; rather than add "\n" to each test, just add it here
		, StringUtl.SplitLinesNl(StringUtl.NewU8(wtr.To_bry_and_clear()))
		);
	}
}
