...
public class Xoav_url_parser {
	...
	private int Parse_wiki(Xoav_url rv, byte[] src, int src_len, int pos) {
		...
		int wiki_end = Bry_finder.Find_fwd(src, Byte_ascii.Slash, wiki_bgn, src_len); if (wiki_end == Bry_.NotFound) throw Exc_.new_("could not find wiki_end", "src", String_.new_u8(src));
