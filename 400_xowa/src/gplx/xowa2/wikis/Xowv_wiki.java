...
import gplx.xowa2.apps.*; import gplx.xowa2.wikis.specials.*; import gplx.xowa2.wikis.data.*; import gplx.xowa2.wikis.data.tbls.*; import gplx.xowa2.gui.*;
public class Xowv_wiki implements Xoa_ttl_parser {
	public Xowv_wiki(Xoav_app app, byte[] domain_bry, Io_url wiki_root_dir) {
		...
		this.db_mgr = new Xodata_db_mgr(Bool_.Y, domain_str, wiki_root_dir);
	...
	public Xodata_db_mgr Db_mgr() {return db_mgr;} private final Xodata_db_mgr db_mgr;
	...
	public Xowv_wiki Init_assert() {
		...
		init_done = true;
		if (String_.Eq(domain_str, "xowa")) return this;
		Db_conn core_conn = Db_conn_pool.I.Get_or_new(db_mgr.Key__core());
		Xodata_ns_regy_tbl ns_tbl = db_mgr.Tbl__ns();
		ns_tbl.Conn_(core_conn);
		ns_tbl.Select_all(ns_mgr);
		db_mgr.Init_by_conn(core_conn);
		return this;
	}
