...
class Xob_xdat_file_ {
	public static int BinarySearch(int itm_0_bgn, byte[] src, int[] itm_ends, byte[] lkp, int lkp_bgn, byte lkp_dlm, int itm_end_adj, boolean exact, Xob_xdat_itm xdat_itm) {if (lkp == null) throw Err_.null_("lkp is null");
		int itm_ends_len = itm_ends.length; if (itm_ends_len == 0) throw Err_.new_("itm_ends_len cannot have 0 itms");
