	...
	public void Cmd_run() {
		...
		while (rdr.Read_next()) {
			try {
				...
				Xof_meta_itm meta_itm = trg_meta_mgr.Get_itm_or_new(xfer_itm.Lnki_ttl(), xfer_itm.Lnki_md5());
				meta_itm.Load_orig_(row.Orig_w(), row.Orig_h());
				xfer_itm.Atrs_by_meta(meta_itm, trg_repo, wiki.Html_mgr().Img_thumb_width());
