...
public class Sql_file_parser {
	...
	private void Identify_flds(byte[] raw) {
		...
		for (int i = 0; i < len; i++) {
			byte[] fld = flds_req[i];
			Sql_fld_itm itm = fld_mgr.Get_by_key(fld); if (itm == null) throw Err_.new_fmt_("could not find field: ~{0}", fld);
			...
			while (reading_file) {
				...
				switch (mode) {
					case Mode_sql_bgn:
						...
						if (cur_pos == Bry_.NotFound || cur_pos > bfr_len) throw Err_.new_("VALUES not found");	// something went wrong;
						mode = Mode_fld;
	...
	private static final byte[] Bry_insert_into = Bry_.new_utf8_("INSERT INTO "), Bry_values = Bry_.new_utf8_(" VALUES (");
