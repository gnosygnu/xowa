...
public abstract class Xop_tkn_itm_base implements Xop_tkn_itm {
	...
	@gplx.Virtual public Xop_tkn_itm Tkn_clone(Xop_ctx ctx, int bgn, int end) {throw Err_mgr._.fmt_(GRP_KEY, "tkn_clone_unhandled", "tkn_clone not implemented for ~{0}", Xop_tkn_itm_.Tid__names[this.Tkn_tid()]);}
	...
	public int Subs_src_bgn(int sub_idx) {if (subs_len == 0) throw Err_mgr._.fmt_(GRP_KEY, "subs_src_bgn_invalid", "no subs available: ~{0}", sub_idx); return subs_pos_ary[ sub_idx * 2];}
	public int Subs_src_end(int sub_idx) {if (subs_len == 0) throw Err_mgr._.fmt_(GRP_KEY, "subs_src_bgn_invalid", "no subs available: ~{0}", sub_idx); return subs_pos_ary[(sub_idx * 2) + 1];}
